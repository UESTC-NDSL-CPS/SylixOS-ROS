...
extern VOID                  _sigGetAction(PLW_CLASS_TCB  ptcb, INT  iSigIndex,
...
INT  kill (LW_OBJECT_HANDLE  ulId, INT  iSigNo)
{
    ...
    if (iSigNo == 0) {                                                  /*  测试目标是否存在            */
        __KERNEL_MODE_PROC(bInvalid = _Thread_Invalid(usIndex););
        if (bInvalid) {
            _ErrorHandle(ESRCH);
            return  (PX_ERROR);
        } else {
...
static INT  __sigqueue (LW_OBJECT_HANDLE  ulId, INT   iSigNo, PVOID  psigvalue)
{
    ...
    sendval = _doSigQueue(ptcb, iSigNo, sigvalue);                      /*  发送信号                    */
    ...
    return  (sendval == SEND_ERROR ? PX_ERROR : ERROR_NONE);
}
/*********************************************************************************************************
** 函数名称: sigqueue
** 功能描述: 发送队列类型信号, 如果是进程, 将发送给其主线程.
...
INT  sigqueue (LW_OBJECT_HANDLE  ulId, INT   iSigNo, const union sigval  sigvalue)
{
    ...
    if ((iSigNo != SIGUSR1) && (iSigNo != SIGUSR2) && (iSigNo != SIGTERM)) {
...
__again:
    __KERNEL_ENTER();                                                   /*  进入内核                    */
    ulError = _ThreadStop(ptcbCur);
    __KERNEL_EXIT();                                                    /*  退出内核 (异常中, 不调度)   */
...
INT  sigGetAction (LW_OBJECT_HANDLE  ulId, INT  iSigNo, struct sigaction *psigaction)
{
    ...
    ptcb = __GET_TCB_FROM_INDEX(usIndex);
    _sigGetAction(ptcb, __sigindex(iSigNo), psigaction);
    __KERNEL_EXIT();                                                    /*  退出内核                    */
