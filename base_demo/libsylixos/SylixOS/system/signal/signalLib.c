...
static PLW_CLASS_SIGPEND        _sigPendAlloc(VOID);
       ...
       BOOL                     _sigPendRun(PLW_CLASS_TCB  ptcbCur);
static BOOL                     _sigPendRunSelf(PLW_CLASS_TCB         ptcbCur,
...
static VOID  __signalExitHandle (PLW_CLASS_TCB  ptcbCur, INT  iSigNo, struct siginfo *psiginfo)
{
    ...
    if (pid > 0) {
        vprocSetImmediatelyTerm(pid, iSigNo);                           /*  立即退出模式                */
    }
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */
...
static VOID  __sigTaskCreateHook (LW_OBJECT_HANDLE  ulId)
{
    ...
    /*
     * SylixOS 为保持兼容性, 这里不将 SIGCHLD 默认设置为 SIG_IGN
     * 而是将 SIGCHLD 默认设置为 SIG_DFL
                             ...
                             INT            iSaType)
{
    ...
    *piSchedRet = ERROR_NONE;                                           /*  默认为就绪状态              */
    ...
    if (iSigNo == SIGKILL) {                                            /*  不可屏蔽的 KILL             */
        if (ptcb->TCB_usStatus & LW_THREAD_STATUS_STOP) {
            ptcb->TCB_ulStopNesting = 0ul;
            ptcb->TCB_usStatus &= ~LW_THREAD_STATUS_STOP;               /*  允许执行                    */
        }
    }
                              ...
                              sigset_t             *psigsetMask)
{
    ...
    psigctlmsg->SIGCTLMSG_archRegCtx      = (psigpend && psigpend->SIGPEND_pabtctx) ?
                                            ((PLW_VMM_ABORT_CTX)psigpend->SIGPEND_pabtctx)->ABTCTX_archRegCtx :
                                            ptcb->TCB_archRegCtx;
                          ...
                          PLW_CLASS_SIGCTLMSG   psigctlmsg)
{
    ...
    psigctx->SIGCTX_sigsetMask = psigctlmsg->SIGCTLMSG_sigsetMask;
                                                                        /*  恢复原先的掩码              */
    _sigPendRunSelf(ptcbCur, psigctlmsg);                               /*  检查并运行需要运行的信号    */
                             ...
                             PLW_CLASS_SIGCTLMSG   psigctlmsg)
{
...
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */
    {
        if ((pfuncHandle != SIG_IGN)   &&
            ...
            pregctx  = &psigctlmsg->SIGCTLMSG_archRegCtx;
            regOldPc = ARCH_REG_CTX_GET_PC(pregctx);

            if (psigaction->sa_flags & SA_SIGINFO) {                    /*  需要 siginfo_t 信息         */
                LW_SOFUNC_PREPARE(pfuncHandle);
                pfuncHandle(iSigNo, psiginfo, pregctx);                 /*  执行信号句柄                */

            } else {
                LW_SOFUNC_PREPARE(pfuncHandle);
                pfuncHandle(iSigNo, pregctx);                           /*  XXX 是否传入 pvCtx 参数 ?   */
            }

            regNewPc = ARCH_REG_CTX_GET_PC(pregctx);
            if ((regNewPc == regOldPc) &&                               /*  信号句柄未作调整            */
                (__SIGNO_MUST_EXIT & __sigmask(iSigNo))) {              /*  必须退出                    */
...
PLW_CLASS_SIGCONTEXT  _signalGetCtx (PLW_CLASS_TCB  ptcb)
{
    return  (&_K_sigctxTable[ptcb->TCB_usIndex]);
}
/*********************************************************************************************************
** 函数名称: _sigGetAction
...
** 调用模块:
*********************************************************************************************************/
VOID  _sigGetAction (PLW_CLASS_TCB  ptcb, INT  iSigIndex, struct sigaction *psigaction)
{
    PLW_CLASS_SIGCONTEXT  psigctx;

    psigctx = _signalGetCtx(ptcb);

    psigaction->sa_handler  = psigctx->SIGCTX_sigaction[iSigIndex].sa_handler;
    psigaction->sa_flags    = psigctx->SIGCTX_sigaction[iSigIndex].sa_flags;
    psigaction->sa_mask     = psigctx->SIGCTX_sigaction[iSigIndex].sa_mask;
    psigaction->sa_restorer = psigctx->SIGCTX_sigaction[iSigIndex].sa_restorer;
}
/*********************************************************************************************************
** 函数名称: _sigPendAlloc
...
VOID  _sigPendInit (PLW_CLASS_SIGPEND  psigpend)
{
    ...
    psigpend->SIGPEND_pabtctx = LW_NULL;
}
/*********************************************************************************************************
** 函数名称: _sigPendGet
...
** 全局变量: 
** 调用模块: 
** 注  意  : 该函数由 __sigReturn 函数调用，并传入当前任务控制块
*********************************************************************************************************/
static BOOL  _sigPendRunSelf (PLW_CLASS_TCB         ptcbCur,
                              PLW_CLASS_SIGCTLMSG   psigctlmsg)
{
    ...
    sigset_t                sigsetOld;                                  /*  信号执行完毕需要恢复的掩码  */
    ...
    do {
        ...
        if (__issig(iSigNo)) {
            ...
            __KERNEL_EXIT();                                            /*  退出内核                    */
            __sigRunHandle(psigctx, iSigNo, &siginfo, psigctlmsg);      /*  直接运行信号句柄            */
...
/*********************************************************************************************************
** 函数名称: __sigCtlCreateSelf
** 功能描述: 当前线程在堆栈中创造一个用于执行信号和信号退出的环境
** 输　入  : ptcbCur                任务控制块
**           psigctx                信号任务相关信息
**           psiginfo               信号信息
**           psigsetMask            执行完信号句柄后需要重新设置的掩码
...
** 调用模块:
*********************************************************************************************************/
static VOID  __sigCtlCreateSelf (PLW_CLASS_TCB         ptcbCur,
                                 PLW_CLASS_SIGCONTEXT  psigctx,
                                 struct siginfo       *psiginfo,
                                 sigset_t             *psigsetMask)
{
    ...
    if (psigctx && (psigctx->SIGCTX_stack.ss_flags == 0)) {             /*  使用用户指定的信号堆栈      */
        ...
        pucStkNow = (BYTE *)pstkStackNow;
    }

    if (pucStkNow != (BYTE *)pstkStackNow) {
#if CPU_STK_GROWTH == 0                                                 /*  CPU_STK_GROWTH == 0         */
        pucStkNow  += sizeof(LW_STACK);                                 /*  向空栈方向移动一个堆栈空间  */
        pucStkNow   = (BYTE *)ROUND_UP(pucStkNow, ARCH_STK_ALIGN_SIZE);
        psigctlmsg  = (PLW_CLASS_SIGCTLMSG)pucStkNow;                   /*  记录 signal contrl msg 位置 */
        ...
        if (ptcbCur->TCB_ulOption & LW_OPTION_THREAD_USED_FP) {
            pucStkNow  -= __SIGFPUCTX_SIZE_ALIGN;                       /*  让出 signal fpu ctx 空间    */
            pucStkNow   = (BYTE *)ROUND_DOWN(pucStkNow, ARCH_FPU_CTX_ALIGN);
            psigctlmsg->SIGCTLMSG_pfpuctx = (LW_FPU_CONTEXT *)pucStkNow;
            pucStkNow  -= sizeof(LW_STACK);                             /*  向空栈方向移动一个堆栈空间  */
        ...
        if (ptcbCur->TCB_ulOption & LW_OPTION_THREAD_USED_DSP) {
            pucStkNow  -= __SIGDSPCTX_SIZE_ALIGN;                       /*  让出 signal dsp ctx 空间    */
            pucStkNow   = (BYTE *)ROUND_DOWN(pucStkNow, ARCH_DSP_CTX_ALIGN);
            psigctlmsg->SIGCTLMSG_pdspctx = (LW_DSP_CONTEXT *)pucStkNow;
            pucStkNow  -= sizeof(LW_STACK);                             /*  向空栈方向移动一个堆栈空间  */
        ...
        psigctlmsg = alloca(sizeof(LW_CLASS_SIGCTLMSG));                /*  记录 signal contrl msg 位置 */

#if LW_CFG_CPU_FPU_EN > 0
        if (ptcbCur->TCB_ulOption & LW_OPTION_THREAD_USED_FP) {
            psigctlmsg->SIGCTLMSG_pfpuctx = alloca_with_align(sizeof(LW_FPU_CONTEXT),
                                                              ARCH_FPU_CTX_ALIGN * 8);
        } else {
            psigctlmsg->SIGCTLMSG_pfpuctx = LW_NULL;
        }
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
...
#if LW_CFG_CPU_DSP_EN > 0
        if (ptcbCur->TCB_ulOption & LW_OPTION_THREAD_USED_DSP) {
            psigctlmsg->SIGCTLMSG_pdspctx = alloca_with_align(sizeof(LW_DSP_CONTEXT),
                                                              ARCH_DSP_CTX_ALIGN * 8);
        } else {
            psigctlmsg->SIGCTLMSG_pdspctx = LW_NULL;
        }
#endif                                                                  /*  LW_CFG_CPU_DSP_EN > 0       */

        pucStkNow = alloca(3 * sizeof(ARCH_FP_CTX));
#if CPU_STK_GROWTH == 1
        pucStkNow = pucStkNow + 3 * sizeof(ARCH_FP_CTX) - 1;
#endif                                                                  /*  CPU_STK_GROWTH == 1         */
    }

    if (archSigCtxSave(&psigctlmsg->SIGCTLMSG_archRegCtx)) {            /*  信号运行完毕后会返回这里    */
        __KERNEL_ENTER();                                               /*  重新进入内核                */
        return;
    }

    psigctlmsg->SIGCTLMSG_iSchedRet       = ERROR_NONE;
    psigctlmsg->SIGCTLMSG_iKernelSpace    = __KERNEL_SPACE_GET2(ptcbCur);
    psigctlmsg->SIGCTLMSG_siginfo         = *psiginfo;
    psigctlmsg->SIGCTLMSG_sigsetMask      = *psigsetMask;
    psigctlmsg->SIGCTLMSG_ulLastError     = ptcbCur->TCB_ulLastError;   /*  记录当前错误号              */
    psigctlmsg->SIGCTLMSG_ucWaitTimeout   = ptcbCur->TCB_ucWaitTimeout; /*  记录 timeout 标志           */
    psigctlmsg->SIGCTLMSG_ucIsEventDelete = ptcbCur->TCB_ucIsEventDelete;

    pstkSignalShell = archTaskCtxCreate(&archRegCtx,
                                        (PTHREAD_START_ROUTINE)__sigShell,
                                        (PVOID)psigctlmsg,
                                        ptcbCur, (PLW_STACK)pucStkNow,
                                        ptcbCur->TCB_ulOption);         /*  建立信号外壳环境            */
    ...
    __KERNEL_SPACE_SET2(ptcbCur, 0);                                    /*  信号句柄运行在任务状态下    */
    __KERNEL_EXIT();                                                    /*  退出内核                    */

    iregInterLevel = KN_INT_DISABLE();                                  /*  关闭当前 CPU 中断           */
    KN_SMP_MB();
    archSigCtxLoad(&archRegCtx);
    KN_INT_ENABLE(iregInterLevel);                                      /*  运行不到这里                */

#else                                                                   /*  LW_CFG_CPU_ARCH_C6X         */
    /*
     * 由于 TI C6X DSP 编译器不支持 alloca, 直接在当前任务栈运行信号句柄
     */
    LW_CLASS_SIGCTLMSG  sigctlmsg;
    INTREG              iregInterLevel;

    archSigCtxSave(&sigctlmsg.SIGCTLMSG_archRegCtx);                    /*  保存现场上下文              */

    sigctlmsg.SIGCTLMSG_iSchedRet       = ERROR_NONE;
    sigctlmsg.SIGCTLMSG_iKernelSpace    = __KERNEL_SPACE_GET2(ptcbCur);
    sigctlmsg.SIGCTLMSG_siginfo         = *psiginfo;
    sigctlmsg.SIGCTLMSG_sigsetMask      = *psigsetMask;
    sigctlmsg.SIGCTLMSG_ulLastError     = ptcbCur->TCB_ulLastError;     /*  记录当前错误号              */
    sigctlmsg.SIGCTLMSG_ucWaitTimeout   = ptcbCur->TCB_ucWaitTimeout;   /*  记录 timeout 标志           */
    sigctlmsg.SIGCTLMSG_ucIsEventDelete = ptcbCur->TCB_ucIsEventDelete;

    __KERNEL_SPACE_SET2(ptcbCur, 0);                                    /*  信号句柄运行在任务状态下    */
    __KERNEL_EXIT();                                                    /*  退出内核                    */

    __sigRunHandle(psigctx, psiginfo->si_signo, psiginfo, &sigctlmsg);  /*  直接运行信号句柄            */

    __KERNEL_ENTER();                                                   /*  重新进入内核                */
    psigctx->SIGCTX_sigsetMask = sigctlmsg.SIGCTLMSG_sigsetMask;        /*  恢复原先的掩码              */

    _sigPendRunSelf(ptcbCur, &sigctlmsg);                               /*  检查并运行需要运行的信号    */
    __KERNEL_SPACE_SET(sigctlmsg.SIGCTLMSG_iKernelSpace);               /*  恢复成进入信号前的状态      */
    __KERNEL_EXIT();                                                    /*  退出内核                    */

    iregInterLevel = KN_INT_DISABLE();                                  /*  关闭当前 CPU 中断           */
    _SchedSetRet(sigctlmsg.SIGCTLMSG_iSchedRet);                        /*  通知调度器返回的情况        */
    ptcbCur->TCB_ulLastError     = sigctlmsg.SIGCTLMSG_ulLastError;     /*  恢复错误号                  */
    ptcbCur->TCB_ucWaitTimeout   = sigctlmsg.SIGCTLMSG_ucWaitTimeout;   /*  恢复 timeout 标志           */
    ptcbCur->TCB_ucIsEventDelete = sigctlmsg.SIGCTLMSG_ucIsEventDelete;
...
** 调用模块: 
**
** 注  意  : 该函数由 sigprocmask 和 sigsuspend 函数调用, 并传入当前任务控制块,
**           系统将在当前线程内构造信号环境, 当该信号执行时, 尾部会自动执行所有可以运行的 pend 信号.
*********************************************************************************************************/
BOOL  _sigPendRun (PLW_CLASS_TCB    ptcbCur)
{
    PLW_CLASS_SIGCONTEXT    psigctx;
...
** 调用模块:
*********************************************************************************************************/
static BOOL  _sigPendPut (PLW_CLASS_SIGCONTEXT psigctx, INT iSigIndex, PLW_CLASS_SIGPEND psigpend)
{
    PLW_CLASS_SIGPEND    psigpendNew  = _sigPendAlloc();                /*  从缓冲区中获取一个空闲的    */
    LW_LIST_RING_HEADER *ppringHeader = &psigctx->SIGCTX_pringSigQ[iSigIndex];

    if (psigpendNew == LW_NULL) {
        _DebugHandle(__ERRORMESSAGE_LEVEL, "no node can allocate from free sigqueue.\r\n");
        _ErrorHandle(ERROR_SIGNAL_SIGQUEUE_NODES_NULL);                 /*  没有空闲的 SIGQUEUE 节点    */
        return  (LW_FALSE);
    }

    *psigpendNew = *psigpend;                                           /*  拷贝信息                    */
    _List_Ring_Add_Last(&psigpendNew->SIGPEND_ringSigQ, ppringHeader);  /*  加入队列链表                */
...
** 全局变量: 
** 调用模块: 
** 注  意  : 由于 signalfd 可能等待 SIG_IGN 的信号, 所以这里需要首先判断入队情况.
*********************************************************************************************************/
LW_SEND_VAL  _doSignal (PLW_CLASS_TCB  ptcb, PLW_CLASS_SIGPEND   psigpend)
{
    ...
    if (sigsetSigMaskBit & psigctx->SIGCTX_sigsetMask) {                /*  被屏蔽了                    */
        if (psiginfo->si_code == SI_KILL) {                             /*  kill 产生了信号, 不能排队   */
            ...
            return  (SEND_BLOCK);                                       /*  已经存在在队列中            */

        } else if (psigpend->SIGPEND_iNotify == SIGEV_SIGNAL) {         /*  需要排队信号                */
            if (_sigPendPut(psigctx, iSigIndex, psigpend)) {
                psigctx->SIGCTX_sigsetPending |= sigsetSigMaskBit;      /*  iSigNo 由于屏蔽等待运行     */
                return  (SEND_BLOCK);
            } else {
                return  (SEND_ERROR);                                   /*  创建排队节点失败            */
            }
    ...
    if ((psigaction->sa_handler == SIG_ERR) ||
        ...
        if (iSigNo != SIGCHLD) {                                        /*  SIGCHLD IGN 需要回收资源    */
            return  (SEND_IGN);                                         /*  如果在设置 IGN 之前子进程已 */
        }                                                               /*  发送信号, 则会执行到这里    */
    }

#if LW_CFG_MODULELOADER_EN > 0
    if (iSigNo == SIGTERM) {                                            /*  exit 信号                   */
        pvproc = __LW_VP_GET_TCB_PROC(ptcb);
        if (pvproc && pvproc->VP_bExiting &&
            pvproc->VP_ulMainThread == ptcb->TCB_ulId) {                /*  已经处于 exiting 模式忽略   */
            return  (SEND_IGN);
        }
    }
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */
...
#if LW_CFG_MODULELOADER_EN > 0
    psiginfo->si_pid   = __LW_VP_TCB_PID_NOLOCK(ptcbCur);
#else
    psiginfo->si_pid   = 0;
#endif

    sigpend.SIGPEND_iNotify = SIGEV_SIGNAL;
    sigpend.SIGPEND_pabtctx = LW_NULL;
