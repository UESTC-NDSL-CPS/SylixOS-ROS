...
** 调用模块: 
*********************************************************************************************************/
static INT  _doSigEventInternal (LW_OBJECT_HANDLE   ulId,
                                 PSIGNAL_EVENT_ARG  psigea,
                                 BOOL               bNeedFree,
                                 PVOID              pabtctx)
{
             BOOL                bNeedUnsafe = LW_FALSE;
    REGISTER UINT16              usIndex;
    REGISTER PLW_CLASS_TCB       ptcb, ptcbCur;
    ...
    LW_TCB_GET_CUR_SAFE(ptcbCur);                                       /*  当前线程控制块              */
    ...
    sigpend.SIGPEND_pabtctx          = pabtctx;

#if LW_CFG_MODULELOADER_EN > 0
    if ((psigevent->sigev_signo == SIGKILL) && (pid > 0)) {
        vprocKillPrepare(pid, ptcbCur->TCB_ulId);                       /*  进程 KILL 预处理            */
    }
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */
    ...
    sendval = _doSignal(ptcb, &sigpend);                                /*  产生信号                    */
    ...
    LW_THREAD_SAFE_INKERN(ptcbCur);                                     /*  确保退出内核时不被删除      */
    bNeedUnsafe = LW_TRUE;
    __KERNEL_EXIT();                                                    /*  退出内核                    */
    ...
    iError = sendval == SEND_ERROR ? PX_ERROR : ERROR_NONE;
...
__out:
    ...
    if (bNeedUnsafe) {
        LW_THREAD_UNSAFE_EX(ptcbCur);                                   /*  退出安全模式                */
    }
...
INT  _doSigEvent (LW_OBJECT_HANDLE  ulId, struct sigevent *psigevent, INT  iSigCode)
{
    ...
    if (bExc) {
        if (_excJobAdd((VOIDFUNCPTR)_doSigEventInternal, (PVOID)ulId, (PVOID)psigea, 
                       (PVOID)LW_TRUE, LW_NULL, 0, 0) < ERROR_NONE) {
            __sigEventArgFree(psigea);
            return  (PX_ERROR);
        }
        return  (ERROR_NONE);
        ...
        return  (_doSigEventInternal(ulId, psigea, LW_FALSE, LW_NULL));
    }
}
/*********************************************************************************************************
** 函数名称: _doSigEventVmmAbort
** 功能描述: VMM 终止时用 sigevent 发送一个信号
** 输　入  : ulId                   目标线程 ID
**           psigevent              信号 event
**           iSigCode               si_code
**           pabtctx                VMM 终止上下文
...
** 调用模块:
*********************************************************************************************************/
INT  _doSigEventVmmAbort (LW_OBJECT_HANDLE  ulId,
                          ...
                          PVOID             pabtctx)
{
    ...
    if (bExc) {
        if (_excJobAdd((VOIDFUNCPTR)_doSigEventInternal, (PVOID)ulId, (PVOID)psigea,
                       (PVOID)LW_TRUE, pabtctx, 0, 0) < ERROR_NONE) {
            __sigEventArgFree(psigea);
            return  (PX_ERROR);
        }
        return  (ERROR_NONE);
        ...
        return  (_doSigEventInternal(ulId, psigea, LW_FALSE, pabtctx));
    }
}
/*********************************************************************************************************
** 函数名称: _doSigEventEx
