...
2016.10.22  简化系统重启设计.
2023.03.04  /dev/null, /dev/zero 支持 select 操作.
*********************************************************************************************************/
...
*********************************************************************************************************/
typedef struct {
    LW_DEV_HDR          NZ_devHdr;
    LW_SEL_WAKEUPLIST   NZ_selwulList;
} LW_NULLZERO_DEV;
/*********************************************************************************************************
  内部全局变量
*********************************************************************************************************/
static  LW_NULLZERO_DEV       _G_devhdrNull;                            /*  空闲设备头                  */
static  LW_NULLZERO_DEV       _G_devhdrZero;                            /*  zero设备头                  */
                           ...
                           LONG        lArg)
{
    ...
    LW_NULLZERO_DEV     *pnzDev = (LW_NULLZERO_DEV *)pdevhdr;
    ...
    case FIOFSTATGET:
        ...
        if (pdevhdr == &_G_devhdrNull.NZ_devHdr) {
    ...
    case FIOSELECT:
        ...
        SEL_WAKE_NODE_ADD(&pnzDev->NZ_selwulList, pselwunNode);

        switch (pselwunNode->SELWUN_seltypType) {

        case SELREAD:                                                   /*  等待数据可读                */
            if (pdevhdr == &_G_devhdrZero.NZ_devHdr) {
    ...
    case FIOUNSELECT:
        SEL_WAKE_NODE_DELETE(&pnzDev->NZ_selwulList, (PLW_SEL_WAKEUPNODE)lArg);
        break;
...
** 调用模块:
*********************************************************************************************************/
static ssize_t  _IosNullRead (LW_DEV_HDR  *pdevhdr, PCHAR  pcBuffer, size_t  stSize)
{
    if (!pcBuffer) {
        _ErrorHandle(EINVAL);
    }
