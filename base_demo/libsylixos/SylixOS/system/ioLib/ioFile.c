...
/*********************************************************************************************************
** 函数名称: _IosFileSync
** 功能描述: 使一个 fd_entry 调用驱动 sync 操作
** 输　入  : pfdentry          fd_entry
** 输　出  : 驱动返回值
** 全局变量:
** 调用模块:
...
INT  API_IosFdUnlink (PLW_DEV_HDR  pdevhdrHdr, CPCHAR  pcName)
{
    ...
    stDevNameLen = lib_strlen(pdevhdrHdr->DEVHDR_pcName);
    if (stDevNameLen > 1) {
        bIsMatchDev = pcName[0] == PX_EOS;
        lib_strcpy(cUnlinkName, pdevhdrHdr->DEVHDR_pcName);
        lib_strcpy(&cUnlinkName[stDevNameLen], pcName);                 /*  完整路径名                  */

    } else {
        bIsMatchDev = LW_FALSE;
        lib_strcpy(cUnlinkName, pcName);                                /*  如果是根设备 "/" 则不拷贝   */
    }
    ...
    for (plineFdEntry  = _S_plineFileEntryHeader;
        ...
        if ((pfdentry->FDENTRY_state != FDSTAT_CLOSED) && 
            ...
            if (pfdentry->FDENTRY_pcRealName &&
                ...
                if (pfdentry->FDENTRY_iType == LW_DRV_TYPE_NEW_1) {     /*  NEW 1 型驱动                */
                    pfdnode = (PLW_FD_NODE)pfdentry->FDENTRY_pfdnode;
                    if (pfdnode->FDNODE_ulLock) {
                        _IosUnlock();                                   /*  退出 IO 临界区              */
                        return  (1);                                    /*  文件被锁定, 不能删除        */

                    } else if (bIsMatchDev) {                           /*  如果为设备根目录            */
                        break;                                          /*  可以删除完整设备            */
...
** 功能描述: 查看文件是否打开
** 输　入  : pdevhdrHdr                    设备头, 如果为 NULL 则完全匹配文件名
**           pcRealName                    文件绝对路径 或相对于设备头的相对路径
** 输　出  : PLW_FD_ENTRY 存在则有打开, NULL 表示没有打开
** 全局变量:
** 调用模块:
** 注  意  : 这里的遍历使用 IO 锁, 中段没有打开此锁, 所以不用加 file list 锁.
                                           API 函数
*********************************************************************************************************/
LW_API
PLW_FD_ENTRY  API_IosFdIsBusy (PLW_DEV_HDR  pdevhdrHdr, CPCHAR  pcRealName)
{
    REGISTER PLW_FD_ENTRY    pfdentry;
    REGISTER PLW_LIST_LINE   plineFdEntry;
             PLW_FD_ENTRY    pfdentryRet = LW_NULL;
             size_t          stHdrNameLen;

    _IosLock();                                                         /*  进入 IO 临界区              */
    if (pdevhdrHdr) {
        stHdrNameLen = lib_strlen(pdevhdrHdr->DEVHDR_pcName);
    } else {
        stHdrNameLen = 0;
    }
    ...
    for (plineFdEntry  = _S_plineFileEntryHeader;
        ...
        pfdentry = _LIST_ENTRY(plineFdEntry, LW_FD_ENTRY, FDENTRY_lineManage);
        if (pfdentry->FDENTRY_state == FDSTAT_CLOSED) {                 /*  文件已关闭                  */
            continue;
        }
        if (pdevhdrHdr) {
            if (pfdentry->FDENTRY_pdevhdrHdr != pdevhdrHdr) {           /*  设备不相同                  */
                continue;
            }
            if (!pfdentry->FDENTRY_pcRealName ||
                (lib_strnlen(pfdentry->FDENTRY_pcRealName, stHdrNameLen) < stHdrNameLen)) {
                continue;                                               /*  文件名长度错误              */
            }
            if (lib_strcmp(&pfdentry->FDENTRY_pcRealName[stHdrNameLen], pcRealName) == 0) {
                pfdentryRet = pfdentry;                                 /*  文件正在被使用              */
                break;
            }
