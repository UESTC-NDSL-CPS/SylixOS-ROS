...
INT  API_AhciCtrlImpPortGet (AHCI_CTRL_HANDLE  hCtrl)
{
    ...
    if (!hCtrl->AHCICTRL_uiImpPortNum) {
        hCtrl->AHCICTRL_uiImpPortNum = hCtrl->AHCICTRL_uiPortNum;
        hCtrl->AHCICTRL_uiImpPortMap = (1 << hCtrl->AHCICTRL_uiImpPortNum) - 1;

        AHCI_CTRL_WRITE(hCtrl, AHCI_PI, hCtrl->AHCICTRL_uiImpPortMap);
        AHCI_CTRL_REG_MSG(hCtrl, AHCI_PI);
    }
