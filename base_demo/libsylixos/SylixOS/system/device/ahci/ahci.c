...
/*********************************************************************************************************
** 函数名称: API_AhciDiskAtaParamGet
** 功能描述: 获取 ATA 参数
** 输　入  : hCtrl      控制器句柄
**           uiDrive    驱动器号
**           pvBuf      参数缓冲区 (大小端由上层进行处理)
** 输　出  : ERROR or OK
** 全局变量:
** 调用模块:
*********************************************************************************************************/
...
/*********************************************************************************************************
** 函数名称: API_AhciDiskAtapiParamGet
** 功能描述: 获取 ATAPI 参数
** 输　入  : hCtrl      控制器句柄
**           uiDrive    驱动器号
**           pvBuf      参数缓冲区 (大小端由上层进行处理)
...
static ULONG  __ahciLbaRangeEntriesSet (PVOID  pvBuffer,
                                        ULONG  ulBuffSize,
                                        ULONG  ulSector,
                                        ULONG  ulCount)
{
    ...
    for (i = 0; i < (UINT32)ulBlkCount; i += ulSector) {                /* 循环处理所有扇区             */
...
#ifdef AHCI_CACHE_MEMORY
        if (uiDirection == O_WRONLY) {
            if ((cacheGetMode(DATA_CACHE) & CACHE_SNOOP_ENABLE) == 0) {
                cacheFlush(DATA_CACHE, pvBuffer, hBlkDev->BLKD_ulBytesPerSector * ulSector);
            }
        }
#endif                                                                  /* AHCI_CACHE_MEMORY            */
        ...
        if (uiDirection == O_RDONLY) {
            if ((cacheGetMode(DATA_CACHE) & CACHE_SNOOP_ENABLE) == 0) { /* RBuf 只读, 这里只需要无效    */
                cacheInvalidate(DATA_CACHE, pvBuffer, hBlkDev->BLKD_ulBytesPerSector * ulSector);
            }
        }
#endif                                                                  /* AHCI_CACHE_MEMORY            */
...
static INT  __ahciBlkIoctl (AHCI_DEV_HANDLE  hDev, INT  iCmd, LONG  lArg)
{
...
#if AHCI_SMART_EN > 0                                                   /* 使能 SMART 操作              */
    AHCI_CMD_HANDLE     hCmd;                                           /* 命令控制句柄                 */
#endif                                                                  /* AHCI_SMART_EN                */
    ...
    case FIOSYNCMETA:
#if AHCI_CACHE_EN > 0                                                   /* 使能 cache 回写              */
...
/*********************************************************************************************************
** 函数名称: __ahciPiCommandSend
** 功能描述: 向 ATAPI 驱动器发送命令
** 输　入  : hDev       设备句柄
**           hCmd       命令句柄
** 输　出  : ERROR or OK
** 全局变量:
...
static INT  __ahciPiCommandSend (AHCI_DEV_HANDLE  hDev, AHCI_CMD_HANDLE  hCmd)
{
    ...
    AHCI_CTRL_HANDLE        hCtrl;                                      /* 控制器句柄                   */
    AHCI_DRIVE_HANDLE       hDrive;                                     /* 驱动器句柄                   */
    UINT8                   ucError = 0;                                /* 错误信息                     */
    INT                     iRetryNum = 0;                              /* 重试计数                     */
...
/*********************************************************************************************************
** 函数名称: __ahciPiStartStopUnit
** 功能描述: 启动或停止设备
** 输　入  : hDev       设备句柄
**           iArg       打开关闭参数 (打开 0x02, 关闭 0x03)
** 输　出  : ERROR or OK
** 全局变量:
** 调用模块:
*********************************************************************************************************/
...
static INT  __ahciPiBlkReset (AHCI_DEV_HANDLE  hDev)
{
    ...
    if (hDrive->AHCIDRIVE_ucState == AHCI_DEV_OK) {                     /* 设备状态正常无需复位         */
        return  (ERROR_NONE);
    }
...
/*********************************************************************************************************
** 函数名称: __ahciPiBlkStatusChk
** 功能描述: ATAPI 块设备状态及参数检查
** 输　入  : hDev       设备句柄
** 输　出  : ERROR or OK
** 全局变量:
** 调用模块:
...
static INT  __ahciPiBlkStatusChk (AHCI_DEV_HANDLE  hDev)
{
    ...
    if ((hDrive->AHCIDRIVE_ucState != AHCI_DEV_OK) &&
        (hDrive->AHCIDRIVE_ucState != AHCI_DEV_MED_CH)) {               /* 状态错误                     */
...
/*********************************************************************************************************
** 函数名称: __ahciPiBlkIoctl
** 功能描述: ATAPI 块设备 ioctl
** 输　入  : hDev       设备句柄
**           iCmd       控制命令
**           lArg       控制参数
** 输　出  : ERROR or OK
...
#ifdef AHCI_CACHE_MEMORY
    if ((cacheGetMode(DATA_CACHE) & CACHE_SNOOP_ENABLE) == 0) {         /* RBuf 只读, 这里只需要无效    */
        cacheInvalidate(DATA_CACHE, pvBuffer, hBlkDev->BLKD_ulBytesPerSector * ulBlkCount);
    }
#endif                                                                  /* AHCI_CACHE_MEMORY            */
...
static INT  __ahciPiBlkDevCreate (AHCI_DEV_HANDLE  hDev)
{
    ...
    iRet = __ahciPiBlkDevDelete(hDev);                                  /* 删除已经存在的设备           */
    if (iRet != ERROR_NONE) {
        return  (PX_ERROR);
    }

    iRet = __ahciPiBlkStatusChk(hDev);                                  /* 获取状态信息                 */
    if (iRet != ERROR_NONE) {
        return  (PX_ERROR);
    }
    ...
    dcattrl.DCATTR_iPipeline        = 1;                                /* PI 为只读, 所以这里为 1      */
    ...
    API_AhciDevAdd(hCtrl, hDev->AHCIDEV_uiDrive);                       /* 添加设备                     */

    return  (ERROR_NONE);
}
/*********************************************************************************************************
** 函数名称: __ahciPiBlkDevManage
** 功能描述: ATAPI 块设备动态管理
** 输　入  : hDev       设备句柄
** 输　出  : ERROR or OK
** 全局变量:
...
static VOID  __ahciPiBlkDevManage (AHCI_DEV_HANDLE  hDev)
{
    ...
    if (hDrive->AHCIDRIVE_ucState == AHCI_DEV_INIT) {                   /* 对异常中断进行处理           */
    ...
    iRet = __ahciPiMediaEventGet(hDev, ucEventBuff, 8);                 /* 获取仓门状态和光盘状态       */
    if ((iRet == ERROR_NONE) &&
        ((ucEventBuff[4] == 0x00)) &&
        ((ucEventBuff[5] == 0x02))) {                                   /* 仓门关闭并且有光盘           */
    ...
    if (hDrive->AHCIDRIVE_ucState == AHCI_DEV_OK) {                     /* 设备状态正常                 */
        AHCI_ATAPI_LOG(AHCI_LOG_PRT, "ctrl %d drive %d atapi blk dev mange drive state ok.\r\n",
                       hDev->AHCIDEV_uiCtrl, hDev->AHCIDEV_uiDrive);
        return;
    }

    iRet = __ahciPiBlkStatusChk(hDev);                                  /* 获取最终状态和能力信息       */
    if ((iRet == ERROR_NONE) &&
        (hDrive->AHCIDRIVE_ucState == AHCI_DEV_MED_CH)) {
                                        ...
                                        ULONG             ulBlkCount)
{
    ...
    case AHCI_TYPE_ATA:                                                 /* ATA 设备                     */
        ...
        if (hDrive->AHCIDRIVE_bTrim) {                                  /* 是否支持 TRIM 的 SSD         */
            dcattrl.DCATTR_iBurstOpt |= LW_DCATTR_BOPT_HIGH_SPEED;
            dcattrl.DCATTR_stDirectThreshold = hDrive->AHCIDRIVE_ulByteSector
                                             * 16;                      /* 超过 16 扇区直接存储         */
        } else {
            dcattrl.DCATTR_iBurstOpt |= LW_DCATTR_BOPT_META_ONE_BURST;  /* 磁碟磁盘                     */
        }
                                                                        /* 挂载设备                     */
        if (!hDev->AHCIDEV_pvOemdisk) {
...
static INT  __ahciDiskDriveInit (AHCI_CTRL_HANDLE  hCtrl, UINT  uiDrive)
{
    ...
    if (hDrive->AHCIDRIVE_ucType == AHCI_TYPE_ATA) {                    /* ATA 模式                     */
        ...
        iRet = API_AhciDiskAtapiParamGet(hCtrl, uiDrive, (PVOID)hParam);/* 获取 ATAPI 参数              */
        if (iRet != ERROR_NONE) {
    ...
    if (hDrive->AHCIDRIVE_ucState == AHCI_DEV_INIT) {                   /* 设备初始状态时打印详细信息   */
...
#define AHCI_RELEASE_SEM(x, mask)  \
    ...
    for (i = 0; i < hCtrl->AHCICTRL_uiImpPortNum; i++) {
        hDrive = &hCtrl->AHCICTRL_hDrive[i];
        LW_SPIN_LOCK_QUICK(&hDrive->AHCIDRIVE_slLock, &iregInterLevel);
        uiPortIntMask = AHCI_BIT_MASK(hDrive->AHCIDRIVE_uiPort);
        if (uiReg & uiPortIntMask) {
            hDrive->AHCIDRIVE_uiIntCount++;
            ...
            if ((uiPortIntr & AHCI_PIS_PRCS) &&
                ...
                if (hDrive->AHCIDRIVE_iInitActive == LW_FALSE) {
                    ...
                    AHCI_CTRL_WRITE(hCtrl, AHCI_IS, uiPortIntMask);
                    LW_SPIN_UNLOCK_QUICK(&hDrive->AHCIDRIVE_slLock, iregInterLevel);
                    uiReg &= ~uiPortIntMask;
