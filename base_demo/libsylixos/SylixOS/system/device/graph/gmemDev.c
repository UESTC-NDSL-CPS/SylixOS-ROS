...
/*********************************************************************************************************
  获取使用计数
*********************************************************************************************************/
#define __GMEM_DEV_USED_COUNT(pgmdev)   \
        (LW_DEV_GET_USE_COUNT((PLW_DEV_HDR)pgmdev) - __GMEM_DEV_PEEKING_GET_COUNT(pgmdev))
/*********************************************************************************************************
  内部函数声明
*********************************************************************************************************/
...
static LONG  __gmemOpen (PLW_GM_DEVICE  pgmdev, PCHAR  pcName, INT  iFlag, INT  iMode)
{
    ...
    if (pgmdev->GMDEV_ulOptFlags & LW_GM_DEV_OPT_EXCL) {
        lOwner = (LONG)__PROC_GET_PID_CUR();                            /*  记录占用者                  */

        __KERNEL_ENTER();                                               /*  进入内核                    */
        if (pgmdev->GMDEV_lOwner >= 0) {
            __KERNEL_EXIT();                                            /*  退出内核                    */
            LW_DEV_DEC_USE_COUNT((PLW_DEV_HDR)pgmdev);
...
LW_API time_t  API_RootFsTime(time_t  *time);
    ...
    case LW_GM_GET_DEV_CNT:
        if (lArg) {
            *(INT *)lArg = __GMEM_DEV_USED_COUNT(pgmdev);
            iError = ERROR_NONE;
        } else {
            _ErrorHandle(EINVAL);
        }
