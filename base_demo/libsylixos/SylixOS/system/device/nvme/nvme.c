...
/*********************************************************************************************************
  NVME cache mode
*********************************************************************************************************/
#if !defined(LW_CFG_CPU_ARCH_MIPS) && !defined(LW_CFG_CPU_ARCH_LOONGARCH)
#if LW_CFG_CACHE_EN > 0
#define NVME_CACHE_MEMORY       1
#endif
#endif
/*********************************************************************************************************
  控制器默认页面大小
*********************************************************************************************************/
#define NVME_CTRL_PAGE_SHIFT    (LW_CFG_VMM_PAGE_SHIFT)
/*********************************************************************************************************
  事件操作
*********************************************************************************************************/
...
static INT  __nvmeCtrlEnable (NVME_CTRL_HANDLE  hCtrl, UINT64  ullCap)
{
    ...
    UINT  uiPageMax   = NVME_CAP_MPSMAX(ullCap) + 12;
    UINT  uiPageShift = NVME_CTRL_PAGE_SHIFT;
    ...
    if (uiPageShift > uiPageMax) {
        NVME_LOG(NVME_LOG_PRT, "maximum device page size %u smaller than host (%u)\n\r",
                 1 << uiPageMax, 1 << uiPageShift);
        uiPageShift = uiPageMax;
    }
                          ...
                          UINT                   uiTimeout)
{
    ...
    NVME_CMD_INFO_HANDLE   hCmdInfo     = __nvmeCmdInfo(hNvmeQueue);
    SYNC_CMD_INFO_HANDLE  *hSyncCmdInfo = (SYNC_CMD_INFO_HANDLE *)pvCtx;
    SYNC_CMD_INFO_HANDLE   hSync;
    ...
    hSync = hNvmeQueue->NVMEQUEUE_hCmdInfo[iCmdId];

    /*
     *  填充回调函数参数
     */
    hCmdInfo[iCmdId].NVMECMDINFO_pfCompletion = pfHandler;
    hCmdInfo[iCmdId].NVMECMDINFO_pvCtx        = (PVOID)hSync;
...
{
    INT                   iCmdId;
    SYNC_CMD_INFO_HANDLE  hCmdInfo;

    /*
     *  申请空闲的 cmdid，并注册命令完成回调函数
     */
    iCmdId = __cmdIdAllocKillable(hNvmeQueue, (PVOID)&hCmdInfo, __syncCompletion, uiTimeout);
    if (iCmdId < 0) {
        return  (iCmdId);
    }

    hCmdInfo->SYNCCMDINFO_iStatus = -EINTR;
    ...
    if (hCmdInfo->SYNCCMDINFO_iStatus == -EINTR) {                      /*  命令返回状态错误处理        */
    ...
    if (puiResult) {                                                    /*  获取回调函数参数中的返回结果*/
        *puiResult = hCmdInfo->SYNCCMDINFO_uiResult;
    }

    return  (hCmdInfo->SYNCCMDINFO_iStatus);
}
/*********************************************************************************************************
                             ...
                             INT                      iTotalLen)
{
    ...
    INT               iOffset = ullAddr & (hCtrl->NVMECTRL_uiPageSize - 1);
    UINT64           *pPrpList;

    hRwCmd->NVMERWCMD_ullPrp1 = cpu_to_le64(ullAddr);

    iLength -= (hCtrl->NVMECTRL_uiPageSize - iOffset);
    if (iLength <= 0) {
        return  (iTotalLen);
    }

    ullAddr += (hCtrl->NVMECTRL_uiPageSize - iOffset);
    if (iLength <= hCtrl->NVMECTRL_uiPageSize) {
        hRwCmd->NVMERWCMD_ullPrp2 = cpu_to_le64(ullAddr);
        return  (iTotalLen);
    }

    pPrpList = (UINT64 *)((addr_t)hNvmeQueue->NVMEQUEUE_pvPrpBuf + iCmdId * hCtrl->NVMECTRL_uiPageSize);
    hRwCmd->NVMERWCMD_ullPrp2 = cpu_to_le64((dma_addr_t)pPrpList);

    do {
        pPrpList[i++] = cpu_to_le64(ullAddr);
        ullAddr      += hCtrl->NVMECTRL_uiPageSize;
        iLength      -= hCtrl->NVMECTRL_uiPageSize;
    } while (iLength > 0);
                                 ...
                                 UINT              uiDirection)
{
    ...
    iCmdId = __cmdIdAllocKillable(hNvmeQueue, (PVOID)&hCmdInfo, __syncCompletion, (UINT)NVME_IO_TIMEOUT);
    if (iCmdId < 0) {
        return  (iCmdId);
    }
    ...
    if (hCmdInfo->SYNCCMDINFO_iStatus == -EINTR) {
                         ...
                         ULONG             ulBlkCount)
{
    ...
    for (i = 0; i < ulBlkCount; i += ulSecNum) {
        ...
        ulBytes  = ulSecNum * hDev->NVMEDEV_tBlkDev.BLKD_ulBytesPerSector;

        if (__nvmeUserCmdSubmit(hDev, ulHandle, ulStartBlk, ulSecNum, O_RDONLY)) {
            goto    __done;
        }

#ifdef NVME_CACHE_MEMORY                                                /* CPU 有随机预取 INV 在 DMA 后 */
        if ((cacheGetMode(DATA_CACHE) & CACHE_SNOOP_ENABLE) == 0) {     /* RBuf 只读, 这里只需要无效    */
            cacheInvalidate(DATA_CACHE, (PVOID)ulHandle, ulBytes);      /*  无效 D-Cache 对应空间       */
        }
#endif                                                                  /*  NVME_CACHE_MEMORY           */

        ulStartBlk += ulSecNum;
        ulHandle    = ulHandle + ulBytes;
    }
...
#ifdef NVME_CACHE_MEMORY
        if ((cacheGetMode(DATA_CACHE) & CACHE_SNOOP_ENABLE) == 0) {
            cacheFlush(DATA_CACHE, (PVOID)ulHandle, ulBytes);           /*  无效 D-Cache 对应空间       */
        }
#endif                                                                  /*  NVME_CACHE_MEMORY           */
                                    ...
                                    UINT                  uiNsid)
{
    ...
    pcBlkName = (PCHAR)__SHEAP_ALLOC(NVME_BLK_NAME_MAX);
    if (!pcBlkName) {
        goto    __free;
    }
    snprintf(pcBlkName, NVME_BLK_NAME_MAX, "%s-%d:%d",
             hCtrl->NVMECTRL_cCtrlName, hCtrl->NVMECTRL_uiUnitIndex, uiNsid);
     ...
     *  配置块设备参数
     */
    hBlkDev->BLKD_pcName            = pcBlkName;                        /*  块设备名                    */
    ...
    dcattrl.DCATTR_iBurstOpt         = LW_DCATTR_BOPT_HIGH_SPEED;       /*  高速存储设备                */
    ...
    dcattrl.DCATTR_stDirectThreshold = hBlkDev->BLKD_ulBytesPerSector
                                     * 16;                              /*  直接存储阀值                */

#ifdef NVME_CACHE_MEMORY
    dcattrl.DCATTR_iBurstOpt |= LW_DCATTR_BOPT_PAGE_ALIGN;              /*  需要页面对齐方便 cache 操作 */
#else
    dcattrl.DCATTR_iBurstOpt |= LW_DCATTR_BOPT_CACHE_COHERENCE;         /*  MIPS or LA 需要使用 DMA 空间*/
#endif
...
static VOID  __nvmeQueueFree (NVME_QUEUE_HANDLE   hNvmeQueue)
{
    ...
    for (i = 0; i < hNvmeQueue->NVMEQUEUE_usDepth; i++) {
        ...
        __SHEAP_FREE(hNvmeQueue->NVMEQUEUE_hCmdInfo[i]);                /*  释放同步命令信息结构        */
    }
...
static VOID  __nvmeQueuesFree (NVME_CTRL_HANDLE  hCtrl, INT  iLowest)
{
    ...
    for (i = hCtrl->NVMECTRL_uiQueueCount - 1; i >= iLowest; i--) {     /*  释放当前所有的队列资源      */
        ...
        if (hNvmeQueue->NVMEQUEUE_pvPrpBuf) {
            API_CacheDmaFree(hNvmeQueue->NVMEQUEUE_pvPrpBuf);
        }
...
static NVME_QUEUE_HANDLE __nvmeQueueAlloc (NVME_CTRL_HANDLE hCtrl, INT  iQueueId,  INT  iQCmdDepth)
{
    ...
    for (i = 0; i < iQCmdDepth; i++) {                                  /*  创建发送命令同步信号量      */
        hNvmeQueue->NVMEQUEUE_hSyncBSem[i] = API_SemaphoreBCreate("nvme_sync",
                                                                  ...
                                                                  LW_NULL);
        if (hNvmeQueue->NVMEQUEUE_hSyncBSem[i] == LW_OBJECT_HANDLE_INVALID) {
            iQCmdDepth = i;
            goto   __free_syncq;
        }
                                                                        /*  提前申请命令同步信息结构    */
        hNvmeQueue->NVMEQUEUE_hCmdInfo[i] = __SHEAP_ZALLOC(sizeof(SYNC_CMD_INFO_CB));
        if (!hNvmeQueue->NVMEQUEUE_hCmdInfo[i]) {
            iQCmdDepth = i + 1;
            goto   __free_syncq;
        }
    }
...
__free_syncq:
    for (i = 0; i < iQCmdDepth; i++) {
        if (hNvmeQueue->NVMEQUEUE_hSyncBSem[i] != LW_OBJECT_HANDLE_INVALID) {
            API_SemaphoreBDelete(&hNvmeQueue->NVMEQUEUE_hSyncBSem[i]);
        }

        if (hNvmeQueue->NVMEQUEUE_hCmdInfo[i]) {
            __SHEAP_FREE(hNvmeQueue->NVMEQUEUE_hCmdInfo[i]);
        }
    }

    API_CacheDmaFree((PVOID)hNvmeQueue->NVMEQUEUE_hSubmissionQueue);
...
static INT  __nvmeAdminQueueConfigure (NVME_CTRL_HANDLE  hCtrl)
{
    ...
    if (!hQueue) {
        ...
        if (!hQueue) {
            NVME_LOG(NVME_LOG_ERR, "ctrl %s unit %d alloc queue failed.\r\n",
                     hCtrl->NVMECTRL_cCtrlName, hCtrl->NVMECTRL_uiUnitIndex);
            return  (PX_ERROR);
        }
    }
...
/*********************************************************************************************************
** 函数名称: __nvmeQueuePrpBufAlloc
** 功能描述: 根据控制器的 page 大小，分配 prp 内存
** 输　入  : hCtrl         控制器句柄
** 输　出  : ERROR or OK
** 全局变量:
** 调用模块:
*********************************************************************************************************/
static INT  __nvmeQueuePrpBufAlloc (NVME_CTRL_HANDLE  hCtrl)
{
    UINT               i;
    NVME_QUEUE_HANDLE  hNvmeQueue;
    UINT32             uiQCmdDepth  = hCtrl->NVMECTRL_uiQCmdDepth;
    UINT32             uiPageSize   = hCtrl->NVMECTRL_uiPageSize;
    size_t             stPrpBufSize = (size_t)(uiPageSize * uiQCmdDepth);

    for (i = 0; i < hCtrl->NVMECTRL_uiQueueCount; i++) {
        hNvmeQueue = hCtrl->NVMECTRL_hQueues[i];
        hNvmeQueue->NVMEQUEUE_pvPrpBuf = API_CacheDmaMallocAlign(stPrpBufSize, uiPageSize);
        if (!hNvmeQueue->NVMEQUEUE_pvPrpBuf) {
            __nvmeQueuesFree(hCtrl, 0);
            return  (PX_ERROR);
        }
...
static INT  __nvmeInit (NVME_CTRL_HANDLE  hCtrl)
{
    ...
    iRet = __nvmeQueuePrpBufAlloc(hCtrl);                              /*  分配 prp 内存                */
    if (iRet != ERROR_NONE) {
        NVME_LOG(NVME_LOG_ERR, "ctrl %s unit %d allocate prp buffer failed.\r\n",
                 hCtrl->NVMECTRL_cCtrlName, hCtrl->NVMECTRL_uiUnitIndex);
        return  (PX_ERROR);
    }
...
NVME_CTRL_HANDLE  API_NvmeCtrlCreate (CPCHAR  pcName, UINT  uiUnit, PVOID  pvArg, ULONG  ulData)
{
    ...
    lib_strlcpy(hCtrl->NVMECTRL_cCtrlName, hDrv->NVMEDRV_cDrvName, NVME_CTRL_NAME_MAX);
