...
*********************************************************************************************************/
LW_API
INT  API_NvmeDevIoctl (NVME_DEV_HANDLE  hDev, INT  iCmd, LONG  lArg)
{
    if (!hDev) {
    ...
    case NVME_DEV_IOCTL_CACHE_FLUSH_GET:
        if (lArg) {
            *(INT *)lArg = hDev->NVMEDEV_iCacheFlush;
        }
        break;

    case NVME_DEV_IOCTL_CACHE_FLUSH_SET:
        hDev->NVMEDEV_iCacheFlush = (INT)lArg;
        break;
...
static VOID  __tshellNvmeDevCmdShow (NVME_DEV_HANDLE  hDev)
{
    ...
    if (hDev) {
        ...
        printf("%5d %4d %6d %7lu %16lu %-15s %-15s %-15s\n",
               i,
               hDev->NVMEDEV_uiCtrl,
               hDev->NVMEDEV_uiNameSpaceId,
               hDev->NVMEDEV_ulBlkOffset,
               hDev->NVMEDEV_ulBlkCount,
