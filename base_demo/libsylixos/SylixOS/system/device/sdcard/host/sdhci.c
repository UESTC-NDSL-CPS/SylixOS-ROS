...
static INT __sdhciHs400esSet(__PSDHCI_HOST  psdhcihost, BOOL  bIsOn);
static INT __sdhciSignalVoltageSet(__PSDHCI_HOST  psdhcihost, UINT32 uiSetVoltage);
static INT __sdhciTimingSet(__PSDHCI_HOST  psdhcihost, UINT32 uiSetTiming);
                         ...
                         LONG            lArg)
{
    ...
    case SDBUS_CTRL_SETHS400ES:
        iError = __sdhciHs400esSet(psdhcihost, (UINT32)lArg);
        break;

    case SDBUS_CTRL_SETVOLTAGE:
        iError = __sdhciSignalVoltageSet(psdhcihost, (UINT32)lArg);
        break;

    case SDBUS_CTRL_SETTIMING:
        iError = __sdhciTimingSet(psdhcihost, (UINT32)lArg);
        break;
...
static INT __sdhciSendTuning (__PSDHCI_HOST  psdhcihost, UINT32 uiOpcode)
{
    ...
    psdhcihostattr = &psdhcihost->SDHCIHS_sdhcihostattr;
    psdadapter     = psdhcihost->SDHCIHS_psdadapter;
    ...
    if (SDADP_TXF(psdadapter)) {
        iRet = SDBUS_TRANSFER(psdadapter, NULL, &sdmsg, 1);
    }
...
/*********************************************************************************************************
** 函数名称: __sdhciHs400esSet
** 功能描述: HS400ES 设置
** 输    入: psdhcihost      SDHCI HOST 结构指针
**           bIsOn           设置之后 HS400ES 是否开启
** 输    出: ERROR CODE
** 全局变量:
** 调用模块:
*********************************************************************************************************/
static INT __sdhciHs400esSet (__PSDHCI_HOST  psdhcihost, BOOL  bIsOn)
{
    ...
    if (psdhcihostattr->SDHCIHOST_pquirkop &&
        psdhcihostattr->SDHCIHOST_pquirkop->SDHCIQOP_pfuncHs400EnhancedStrobe) {
        psdhcihostattr->SDHCIHOST_pquirkop->SDHCIQOP_pfuncHs400EnhancedStrobe(psdhcihostattr, bIsOn);
    }

    return  (ERROR_NONE);
}
/*********************************************************************************************************
** 函数名称: __sdhciSignalVoltageSet
** 功能描述: 信号电压设置
** 输    入: psdhcihost      SDHCI HOST 结构指针
**           uiSetVoltage    要设置的电压
** 输    出: ERROR CODE
...
static INT __sdhciSignalVoltageSet (__PSDHCI_HOST  psdhcihost, UINT32  uiSetVoltage)
{
    ...
    if (psdhcihostattr->SDHCIHOST_pquirkop &&
        psdhcihostattr->SDHCIHOST_pquirkop->SDHCIQOP_pfuncVoltageSet) {
        psdhcihostattr->SDHCIHOST_pquirkop->SDHCIQOP_pfuncVoltageSet(psdhcihostattr, uiSetVoltage);
        goto __ret;
    }

    /*
     *  信号电压切换只在 v3.00 以上控制器支持
     */
    if (psdhcihost->SDHCIHS_uiVersion < SDHCI_HVER_SPEC_300) {
        return  (ERROR_NONE);
    }
    ...
    case SDARG_SETVOLTAGE_330:
        __sdhciPowerSetVol(psdhcihost, SDHCI_POWCTL_330, LW_TRUE);

        /*
         *  设置 Host Control2 寄存器中 1.8V 信号使能为 0
         */
        usCtrl &= ~SDHCI_HCTRL_VDD_180;
        SDHCI_WRITEW(psdhcihostattr, SDHCI_HOST_CONTROL2, usCtrl);

        SDHCI_DELAYMS(5);

        /*
         *  3.3V 电压输出应该在 5ms 内稳定
         */
        usCtrl = SDHCI_READW(psdhcihostattr, SDHCI_HOST_CONTROL2);
        if (!(usCtrl & SDHCI_HCTRL_VDD_180)) {
            break;
        }

        SDCARD_DEBUG_MSG(__ERRORMESSAGE_LEVEL, "3.3V regulator output did not become stable\n");

        return  (PX_ERROR);

    case SDARG_SETVOLTAGE_180:
        __sdhciPowerSetVol(psdhcihost, SDHCI_POWCTL_180, LW_TRUE);

        /*
         *   在 Host Control2 寄存器中使能 1.8V 信号
         */
        usCtrl |= SDHCI_HCTRL_VDD_180;
        SDHCI_WRITEW(psdhcihostattr, SDHCI_HOST_CONTROL2, usCtrl);
        ...
        SDCARD_DEBUG_MSG(__ERRORMESSAGE_LEVEL, "1.8V regulator output did not become stable\n");

        return  (PX_ERROR);

    case SDARG_SETVOLTAGE_120:
        /*
         *  TODO: 暂不支持设置1.2v
         */
        return  (PX_ERROR);
...
/*********************************************************************************************************
** 函数名称: __sdhciTimingSet
** 功能描述: 用于 HS200,HS400,HS400ES 采样时序设置
** 输    入: psdhcihost      SDHCI HOST 结构指针
**           uiSetTiming     要设置的时序
** 输    出: ERROR CODE
** 全局变量:
...
static INT __sdhciTimingSet (__PSDHCI_HOST  psdhcihost, UINT32  uiSetTiming)
{
    ...
    if (psdhcihost->SDHCIHS_uiVersion >= SDHCI_HVER_SPEC_300) {
        usCtrl  = SDHCI_READW(psdhcihostattr, SDHCI_HOST_CONTROL2);
        usCtrl &= ~SDHCI_HCTRL_DRV_TYPE_MASK;
        usCtrl |= SDHCI_HCTRL_DRV_TYPE_B;
        SDHCI_WRITEW(psdhcihostattr, SDHCI_HOST_CONTROL2, usCtrl);
    }

    if (psdhcihostattr->SDHCIHOST_pquirkop &&
        psdhcihostattr->SDHCIHOST_pquirkop->SDHCIQOP_pfuncTimingSet) {
        psdhcihostattr->SDHCIHOST_pquirkop->SDHCIQOP_pfuncTimingSet(psdhcihostattr, uiSetTiming);
        goto __ret;
    }

    usCtrl  = SDHCI_READW(psdhcihostattr, SDHCI_HOST_CONTROL2);
    usCtrl &= ~SDHCI_HCTRL_UHS_MASK;
    if ((uiSetTiming == SDARG_SETTIMING_MMC_HS200) ||
        (uiSetTiming == SDARG_SETTIMING_UHS_SDR104)) {
        usCtrl |= SDHCI_HCTRL_UHS_SDR104;
    } else if (uiSetTiming == SDARG_SETTIMING_MMC_HS400) {
        usCtrl |= SDHCI_HCTRL_UHS_HS400;
    }
    SDHCI_WRITEW(psdhcihostattr, SDHCI_HOST_CONTROL2, usCtrl);
...
static __SDHCI_SDM_HOST *__sdhciSdmHostNew (__PSDHCI_HOST   psdhcihost)
{
    ...
    if (SDHCI_QUIRK_FLG(&psdhcihost->SDHCIHS_sdhcihostattr,
                        SDHCI_QUIRK_FLG_FORCE_HS400ES)) {
        iCapablity |= SDHOST_CAP_HS400ES;
    }
