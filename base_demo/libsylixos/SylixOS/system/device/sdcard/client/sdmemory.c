...
#define __SD_MIN_CACHE_EN_TIMEOUT   1600                                /*  CACHE 使能超时(毫秒)        */
#define __SD_CACHE_FLUSH_TIMEOUT    (30 * 1000)                         /*  CACHE FLUSH 超时(毫秒)      */
/*********************************************************************************************************
  忙检测函数的等待类型
*********************************************************************************************************/
...
static INT __sdMemMmcBusWidthChangeHS200(PLW_SDCORE_DEVICE psdcoredevice,
                                         INT               iCardCap,
                                         LW_SDDEV_EXT_CSD *psddevextcsd);
static INT __sdMemMmcBusWidthChangeHS400(PLW_SDCORE_DEVICE psdcoredevice,
                                         INT               iCardCap,
                                         LW_SDDEV_EXT_CSD *psddevextcsd);
static INT __sdMemMmcBusWidthChangeHS400ES(PLW_SDCORE_DEVICE psdcoredevice,
                                           INT               iCardCap,
                                           LW_SDDEV_EXT_CSD *psddevextcsd);
...
static INT __sdMemInit (PLW_SDCORE_DEVICE psdcoredevice)
{
    ...
    case SDADAPTER_TYPE_SD:
        ...
        if (ucType != SDDEV_TYPE_MMC) {
            ...
            if ((iCardCap & iHostCap) & SDHOST_CAP_HS400ES) {
                iError = __sdMemMmcBusWidthChangeHS400ES(psdcoredevice, iCardCap, &sddevextcsd);
            } else if ((iCardCap & iHostCap) & SDHOST_CAP_HS200) {
                iError = __sdMemMmcBusWidthChangeHS200(psdcoredevice, iCardCap, &sddevextcsd);
                if (iError == ERROR_NONE) {
                    iError = __sdMemMmcBusWidthChangeHS400(psdcoredevice, iCardCap, &sddevextcsd);
                }
            } else {
            ...
            if (sddevextcsd.DEVEXTCSD_uiCacheSize > 0) {
                UINT  uiTimeout = max(sddevextcsd.DEVEXTCSD_uiCmd6Timeout, __SD_MIN_CACHE_EN_TIMEOUT);
                ...
                iError = API_SdCoreDevSwitch(psdcoredevice,
                                             ...
                                             1);
                if (iError == ERROR_NONE) {
                    iError = __sdMemSwitchWait(psdcoredevice, uiTimeout, LW_TRUE);
                }

                if (iError == ERROR_NONE) {
                    psdcoredevice->COREDEV_iDevSta |= COREDEV_STA_CACHE_FLUSH;
                    SDCARD_DEBUG_MSG(__LOGMESSAGE_LEVEL, "mmc cache turn on success.\r\n");
                } else {
                    SDCARD_DEBUG_MSG(__ERRORMESSAGE_LEVEL, "mmc cache failed to turn on.\r\n");
                    return  (PX_ERROR);
                }
            }
        }
...
/*********************************************************************************************************
** 函数名称: __sdFlushCmdSubmit
** 功能描述: 发送 FLUSH 命令
** 输　入  : psdcoredevice  设备句柄
** 输　出  : ERROR or OK
** 全局变量:
** 调用模块:
*********************************************************************************************************/
static INT  __sdFlushCmdSubmit (PLW_SDCORE_DEVICE  psdcoredevice)
{
    INT  iError;

    if (psdcoredevice->COREDEV_iDevSta & COREDEV_STA_CACHE_FLUSH) {
        iError = API_SdCoreDevSwitch(psdcoredevice,
                                     EXT_CSD_CMD_SET_NORMAL,
                                     EXT_CSD_FLUSH_CACHE,
                                     1);
        if (iError == ERROR_NONE) {
            iError = __sdMemSwitchWait(psdcoredevice, __SD_CACHE_FLUSH_TIMEOUT, LW_TRUE);
        }

        if (iError != ERROR_NONE) {
            SDCARD_DEBUG_MSG(__ERRORMESSAGE_LEVEL, "mmc cache flush failed.\r\n");
            return  (PX_ERROR);
        }
    }
                         ...
                         LONG             lArg)
{
    ...
    case FIOFLUSH:                                                      /*  将缓存回写到磁盘            */
        iRet = __sdFlushCmdSubmit(psdblkdevice->SDBLKDEV_pcoreDev);
        if (iRet != ERROR_NONE) {
            return  (PX_ERROR);
        }
        break;
    ...
    case FIOSYNCMETA:
        API_SdmHostExtOptGet(psdblkdevice->SDBLKDEV_pcoreDev,
                             SDHOST_EXTOPT_META_CACHE_FLUSH_GET,
                             (LONG)&lCacheFlush);
        if (lCacheFlush) {
            iRet = __sdFlushCmdSubmit(psdblkdevice->SDBLKDEV_pcoreDev);
            if (iRet != ERROR_NONE) {
...
/*********************************************************************************************************
** 函数名称: __sdMemMmcBusWidthChangeHS400ES
** 功能描述: 设置 MMC 卡的总线位宽 (HS400ES 模式)
** 输    入: psdcoredevice   核心设备对象
                                            ...
                                            LW_SDDEV_EXT_CSD  *psddevextcsd)
{
    ...
    if (!(iHostCap & SDHOST_CAP_DATA_8BIT)) {
        return  (PX_ERROR);
    }
    ...
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "switch to 8bit buswidth for hs400es failed.\r\n");
        return  (PX_ERROR);
    }

    iError = API_SdCoreDevCtl(psdcoredevice, SDBUS_CTRL_SETBUSWIDTH, SDBUS_WIDTH_8);
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "set bus width failed.\r\n");
        return  (PX_ERROR);
    }
    ...
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "switch to hs mode for hs400es failed.\r\n");
        return  (PX_ERROR);
    }

    iError = API_SdCoreDevCtl(psdcoredevice, SDBUS_CTRL_SETCLK, SDARG_SETCLK_MAX);
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "set clock 50M failed.\r\n");
        return  (PX_ERROR);
    }

    /*
     *  切换设备至带 strobe 的 DDR 模式
     */
    iError = API_SdCoreDevSwitch(psdcoredevice,
                                 EXT_CSD_CMD_SET_NORMAL,
                                 EXT_CSD_BUS_WIDTH,
                                 EXT_CSD_BUS_WIDTH_8_DDR | EXT_CSD_BUS_WIDTH_STROBE);
    if (iError == ERROR_NONE) {
        iError = __sdMemSwitchWait(psdcoredevice,
                                   psddevextcsd->DEVEXTCSD_uiCmd6Timeout,
                                   LW_TRUE);
    ...
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "switch to bus width for hs400es failed.\r\n");
        return  (PX_ERROR);
    }
    ...
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "switch to hs400 mode failed.\r\n");
        return  (PX_ERROR);
    }
     ...
     *  设置信号电压为 1.8V
     */
    iError = API_SdCoreDevCtl(psdcoredevice, SDBUS_CTRL_SETVOLTAGE, SDARG_SETVOLTAGE_180);
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "set voltage 1.8v failed.\r\n");
        return  (PX_ERROR);
    }

    /*
     *  HS400 模式下时钟速率仍为 200M,使用双边采样的 DDR 模式
     */
    iError = API_SdCoreDevCtl(psdcoredevice, SDBUS_CTRL_SETTIMING, SDARG_SETTIMING_MMC_HS400);
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "set timing hs400 failed.\r\n");
        return  (PX_ERROR);
    }

    iError = API_SdCoreDevCtl(psdcoredevice, SDBUS_CTRL_SETCLK, SDARG_SETCLK_HS200);
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "set clock 200M failed.\r\n");
        return  (PX_ERROR);
    }

    iError = API_SdCoreDevCtl(psdcoredevice, SDBUS_CTRL_SETHS400ES, LW_TRUE);
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "set hs400 enhanced strobe failed.\r\n");
        return  (PX_ERROR);
    }
    ...
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL,
                          "hs400 select power class to bus width 8 error.\r\n");
    }

    return  (iError);
}
/*********************************************************************************************************
** 函数名称: __sdMemMmcBusWidthChangeHS400
** 功能描述: 设置 MMC 卡的总线位宽 (HS400 模式)
** 输    入: psdcoredevice   核心设备对象
**           iCardCap        MMC卡支持的位宽功能
**           psddevextcsd    扩展CSD信息
** 输    出: NONE
                                          ...
                                          LW_SDDEV_EXT_CSD  *psddevextcsd)
{
    ...
    /*
     *  检查主控制器和设备是否都支持 HS400 模式
     */
    iCardCap = iCardCap & iHostCap;
    if (!(iCardCap & SDHOST_CAP_HS400)) {
        return  (ERROR_NONE);
    }
    ...
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "switch to hs mode for hs400 failed.\r\n");
        return  (PX_ERROR);
    }
    ...
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "switch to bus width for hs400 failed.\r\n");
        return  (PX_ERROR);
    }
                                          ...
                                          LW_SDDEV_EXT_CSD  *psddevextcsd)
{
    ...
    iError = API_SdCoreDevCtl(psdcoredevice, SDBUS_CTRL_SETTIMING, SDARG_SETTIMING_MMC_HS200);
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL, "set timing hs200 failed.\r\n");
        return  (PX_ERROR);
    }
    ...
    if (iError != ERROR_NONE) {
        SDCARD_DEBUG_MSGX(__ERRORMESSAGE_LEVEL,
                          ...
                          (puiBusWidth[uiIndex] == SDBUS_WIDTH_8) ? 8 : 4);
    }
