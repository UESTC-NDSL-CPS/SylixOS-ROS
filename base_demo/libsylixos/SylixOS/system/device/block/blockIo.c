                       ...
                       ULONG   ulRemainCount)
{
    ...
    if (LW_UNLIKELY(pblkd->BLKD_iFlag & O_WRONLY)) {
        return  (PX_ERROR);
    }
    
    if (LW_UNLIKELY(pblkd->BLKD_bRemovable && pblkd->BLKD_bDiskChange)) {
        return  (PX_ERROR);                                             /*  设备改变不可操作            */
    }
    ...
    if (LW_UNLIKELY(i >= pblkd->BLKD_iRetry)) {
        if (pblkd->BLKD_bRemovable) {
                        ...
                        ULONG   ulSectorCount)
{
    ...
    if (LW_UNLIKELY(pblkd->BLKD_iFlag == O_RDONLY)) {
        return  (PX_ERROR);
    }
