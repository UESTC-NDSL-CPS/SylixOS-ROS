...
INT  API_VutexPendEx (INT  *piVar, INT  iCompare, INT  iDesired, ULONG  ulTimeout)
{
    ...
    if (iCompare &   LW_OPTION_VUTEX_SIGNAL_INTER) {
        bSigIntr  =  LW_TRUE;
        iCompare &= ~LW_OPTION_VUTEX_SIGNAL_INTER;
    } else {
        bSigIntr  =  LW_FALSE;
    }
...
__wait_again:
    ...
    __KERNEL_ENTER();                                                   /*  进入内核                    */
    if (_VutexWakeIsMatch(LW_ACCESS_ONCE_PTR(INT, piVar),
                          iCompare, iDesired)) {                        /*  再次确认是否已经满足条件    */
        __KERNEL_EXIT();
        return  (ERROR_NONE);
    }
    ...
    if (iSchedRet) {
        if ((iSchedRet == LW_SIGNAL_EINTR) && bSigIntr) {
...
INT  API_VutexPostEx (INT  *piVar, INT  iValue, INT  iFlags)
{
    ...
    if (!(iFlags & LW_OPTION_VUTEX_FLAG_DONTSET)) {                     /*  设置变量值                  */
