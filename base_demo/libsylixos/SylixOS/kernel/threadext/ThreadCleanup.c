...
ULONG  API_ThreadCleanupPush (VOIDFUNCPTR  pfuncRoutine, PVOID  pvArg)
{
    ...
    pcurNode->CUR_iKernNesting = __KERNEL_SPACE_GET2(ptcbCur);
...
ULONG  API_ThreadCleanupPushEx (LW_OBJECT_HANDLE  ulId, VOIDFUNCPTR  pfuncRoutine, PVOID  pvArg)
{
    ...
    pcurNode->CUR_iKernNesting = __KERNEL_SPACE_GET2(ptcb);
    pcurNode->CUR_pfuncClean   = pfuncRoutine;
    pcurNode->CUR_pvArg        = pvArg;
...
VOID  API_ThreadCleanupPop (BOOL  bRun)
{
    ...
    LW_TCB_GET_CUR(ptcbCur);
    if (bRun) {
        iOrigKernNesting = __KERNEL_SPACE_GET2(ptcbCur);                /*  保存当前 KS 值              */
    }
    ...
    if (pcurNode) {
        ...
        KN_INT_ENABLE(iregInterLevel);                                  /*  打开中断                    */
        if (bRun) {
            __KERNEL_SPACE_SET2(ptcbCur, pcurNode->CUR_iKernNesting);   /*  使用注册时的 KS 值          */
    ...
    if (bRun) {
        __KERNEL_SPACE_SET2(ptcbCur, iOrigKernNesting);                 /*  恢复当前 KS 值              */
    }
}
