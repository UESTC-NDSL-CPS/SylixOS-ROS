...
2023.05.05  修复极低可能性循环等待错误.
*********************************************************************************************************/
#define  __SYLIXOS_KERNEL
#include "../SylixOS/kernel/include/k_kernel.h"
/*********************************************************************************************************
** 函数名称: _ThreadWaitIsLoop
** 功能描述: 是否进入循环等待状态.(进入内核并关中断状态被调用)
** 输　入  : ptcbCur    当前线程控制块
**           ptcbDest   目标线程控制块
** 输　出  : 是否存在循环
** 全局变量:
...
static BOOL  _ThreadWaitIsLoop (PLW_CLASS_TCB  ptcbCur, PLW_CLASS_TCB  ptcbDest)
{
    while (ptcbDest && ptcbDest->TCB_ptcbWaitStatus) {
        if (ptcbDest->TCB_ptcbWaitStatus == ptcbCur) {
            return  (LW_TRUE);
        } else {
            ptcbDest = ptcbDest->TCB_ptcbWaitStatus;
        }
    }
...
** 全局变量: 
** 调用模块: 
** 注  意  : 1: 此函数已经预置的阻塞点, 最后一次退出内核时, 将阻塞等待目标线程状态改变完毕.
**
...
ULONG  _ThreadStatusChange (PLW_CLASS_TCB  ptcb, UINT  uiStatusReq)
{
    ...
    if (ptcb == ptcbCur) {                                              /*  改变自己的状态              */
        ...
        if (_ThreadWaitIsLoop(ptcbCur, ptcb)) {
            KN_INT_ENABLE(iregInterLevel);                              /*  打开中断                    */
            return  (EAGAIN);                                           /*  有循环, 需要重试            */
        }
        goto    __change2;
