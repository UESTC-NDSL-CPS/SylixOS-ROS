                 ...
                 PVOID                    pvArg)
{
    ...
    if (((ulOption & LW_OPTION_THREAD_NUMA) ||
        ((ulOption & (LW_OPTION_THREAD_STK_MAIN | LW_OPTION_OBJECT_GLOBAL)) == 0)) &&
        (ptcbCur->TCB_iHetrccMode == LW_HETRCC_STRONG_AFFINITY)) {      /*  NUMA 或进程内线程继承强亲和 */
        ptcb->TCB_iHetrccMode  = LW_HETRCC_STRONG_AFFINITY;
        ptcb->TCB_ulHetrccLock = ptcbCur->TCB_ulHetrccLock;
    } else {
        ptcb->TCB_iHetrccMode  = LW_HETRCC_NON_AFFINITY;
        ptcb->TCB_ulHetrccLock = 0ul;
    }

#if LW_CFG_SMP_NUMA_EN > 0
    if ((ulOption & (LW_OPTION_THREAD_STK_MAIN | LW_OPTION_OBJECT_GLOBAL)) == 0) {
        if (ptcbCur->TCB_ulOption & LW_OPTION_THREAD_NUMA) {
            ptcb->TCB_ulOption |= LW_OPTION_THREAD_NUMA;                /*  进程内线程继承 NUMA 标志    */
        }
    }
    ...
    if (LW_SYS_STATUS_IS_RUNNING()) {
        ...
        ptcb->TCB_uiSecReg    = ptcbCur->TCB_uiSecReg;
    ...
    if (LW_CPU_FPU_IS_INITED()) {
        __ARCH_FPU_CTX_INIT(ptcb->TCB_pvStackFP,                        /*  初始化当前任务 FPU 上下文   */
                            (ulOption & LW_OPTION_THREAD_FPU_EXC_EN)
#if LW_CFG_MODULELOADER_EN > 0
                            || (pvproc && (pvproc->VP_ulFeatures & __LW_VP_FT_FPU_EXC_EN))
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */
                            );
    }
    ...
    if (LW_CPU_DSP_IS_INITED()) {
        __ARCH_DSP_CTX_INIT(ptcb->TCB_pvStackDSP,                       /*  初始化当前任务 DSP 上下文   */
                            (ulOption & LW_OPTION_THREAD_DSP_EXC_EN)
#if LW_CFG_MODULELOADER_EN > 0
                            || (pvproc && (pvproc->VP_ulFeatures & __LW_VP_FT_DSP_EXC_EN))
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */
                            );
    }
    ...
    __KERNEL_ENTER();                                                   /*  进入内核                    */
    _K_ptcbTCBIdTable[ptcb->TCB_usIndex] = ptcb;                        /*  保存TCB控制块               */
    _List_Line_Add_Ahead(&ptcb->TCB_lineManage, 
                         &_K_plineTCBHeader);                           /*  加入 TCB 管理链表           */
    __KERNEL_EXIT();                                                    /*  退出内核                    */

#if LW_CFG_MODULELOADER_EN > 0                                          /*  如果为 GLOBAL 则不属于进程  */
    if (pvproc) {                                                       /*  继承进程控制块              */
        ptcb->TCB_pvVProcessContext = (PVOID)pvproc;
        vprocThreadAdd(pvproc, ptcb);
    }
#endif
...
/*********************************************************************************************************
** 函数名称: _TCBDestroy
** 功能描述: 销毁一个TCB (不改变 join 和 detach 相关信息)
** 输　入  : ptcb              任务控制块
** 输　出  : NONE
** 全局变量: 
** 调用模块: 
...
VOID  _TCBDestroyExt (PLW_CLASS_TCB  ptcb)
{
    ...
    while (ptex->TEX_pmonoOnceHeader) {
        ...
        API_VutexPostEx(pcur->CUO_piOnce,
                        __ARCH_INT_MAX, LW_OPTION_VUTEX_FLAG_DONTSET);  /*  唤醒等待完成的任务          */
...
** 调用模块: 
*********************************************************************************************************/
VOID  _TCBCleanupPopExt (PLW_CLASS_TCB  ptcb, BOOL  bRun)
{
#if LW_CFG_THREAD_EXT_EN > 0
             INTREG                 iregInterLevel;
    REGISTER __PLW_THREAD_EXT       ptex = &ptcb->TCB_texExt;
    REGISTER __PLW_CLEANUP_ROUTINE  pcur;
             INT                    iOrigKernNesting;
    
    if (bRun) {
        iOrigKernNesting = __KERNEL_SPACE_GET2(ptcb);                   /*  保存当前 KS 值              */
    }
    ...
    while (ptex->TEX_pmonoCurHeader) {
        ...
        if (bRun) {
            __KERNEL_SPACE_SET2(ptcb, pcur->CUR_iKernNesting);          /*  使用注册时的 KS 值          */
    ...
    if (bRun) {
        __KERNEL_SPACE_SET2(ptcb, iOrigKernNesting);                    /*  恢复当前 KS 值              */
    }
#endif                                                                  /*  LW_CFG_THREAD_EXT_EN > 0    */
}
/*********************************************************************************************************
