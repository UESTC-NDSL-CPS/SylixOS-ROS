...
INT  _CpuActive (PLW_CLASS_CPU   pcpu)
{
    ...
    LW_HCCCPU_GET(pcpu->CPU_ulHccId)->HCCCPU_uiActive++;
#endif
    KN_SMP_MB();
...
INT  _CpuInactive (PLW_CLASS_CPU   pcpu)
{
    ...
    if (!ptcb->TCB_bCPULock) {                                          /*  此任务未设置亲和度          */
        _CandTableNotify(ptcb);                                         /*  重新尝试通知其他核心调度    */
    }

    _CpuBogoMipsClear(ulCPUId);                                         /*  清除之前计算的 BogoMIPS     */
...
/*********************************************************************************************************
** 函数名称: _CpuGetCur
** 功能描述: 获取当前 CPU 控制块 (驱动程序在中断状态下被调用)
** 输　入  : NONE
** 输　出  : 当前 CPU 控制块
** 全局变量: 
** 调用模块: 
