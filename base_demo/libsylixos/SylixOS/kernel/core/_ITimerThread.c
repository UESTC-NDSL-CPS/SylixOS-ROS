...
PVOID  _ITimerThread (PVOID  pvArg)
{
    ...
    for (;;) {
        ...
        __KERNEL_TIME_GET_IGNIRQ(i64CurTime, INT64);                    /*  获得函数执行后时间          */
        if (i64CurTime > _K_wuITmr.WU_i64LastTime) {
            ulCounter += i64CurTime - _K_wuITmr.WU_i64LastTime;         /*  进行花费时间修正            */
            _K_wuITmr.WU_i64LastTime = i64CurTime;
        }
