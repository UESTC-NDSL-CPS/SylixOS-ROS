...
VOID _SchedSwp (PLW_CLASS_CPU pcpuCur)
{
    ...
    if (bIsIntSwitch) {
        MONITOR_EVT_LONG7(MONITOR_EVENT_ID_SCHED, MONITOR_EVENT_SCHED_INT,
                          ptcbCur->TCB_cThreadName, ptcbCur->TCB_ulId,
                          ptcbCur->TCB_ucPriority, ptcbCur->TCB_usStatus,
                          ptcbHigh->TCB_cThreadName, ptcbHigh->TCB_ulId,
                          ptcbHigh->TCB_ucPriority, LW_NULL);
    } else {
        MONITOR_EVT_LONG7(MONITOR_EVENT_ID_SCHED, MONITOR_EVENT_SCHED_TASK,
                          ptcbCur->TCB_cThreadName, ptcbCur->TCB_ulId,
                          ptcbCur->TCB_ucPriority, ptcbCur->TCB_usStatus,
                          ptcbHigh->TCB_cThreadName, ptcbHigh->TCB_ulId,
                          ptcbHigh->TCB_ucPriority, LW_NULL);
...
#if ((LW_CFG_CPU_FPU_EN > 0) && defined(__ARCH_CRT_CTX_ENABLE_FPU)) || \
    ((LW_CFG_CPU_DSP_EN > 0) && defined(__ARCH_CRT_CTX_ENABLE_DSP))
    REGISTER ARCH_REG_CTX  *pregctx  = &pcpuCur->CPU_pcrcbNext->COROUTINE_archRegCtx;
    REGISTER ULONG          ulOption = pcpuCur->CPU_ptcbTCBCur->TCB_ulOption;
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */

    pcpuCur->CPU_pcrcbCur = pcpuCur->CPU_pcrcbNext;                     /*  切换协程                    */

#if (LW_CFG_CPU_FPU_EN > 0) && defined(__ARCH_CRT_CTX_ENABLE_FPU)
    if (ulOption & LW_OPTION_THREAD_USED_FP) {
        __ARCH_CRT_CTX_ENABLE_FPU(pregctx);
    }
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */

#if (LW_CFG_CPU_DSP_EN > 0) && defined(__ARCH_CRT_CTX_ENABLE_DSP)
    if (ulOption & LW_OPTION_THREAD_USED_DSP) {
        __ARCH_CRT_CTX_ENABLE_DSP(pregctx);
    }
