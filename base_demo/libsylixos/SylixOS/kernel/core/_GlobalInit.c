...
{
    REGISTER INT            i;
#if (LW_CFG_SMP_EN > 0) && ((LW_CFG_CPU_ARCH_SMT > 0) || (LW_CFG_CPU_ARCH_HETRC > 0))
             ULONG          ulMaxId;
    ...
    LW_CPU_FOREACH (i) {
        ...
        if (ulMaxId < pcpu->CPU_ulPhyId) {
            ulMaxId = pcpu->CPU_ulPhyId;
        }
    }
    _K_ulNPhyCpus = ulMaxId + 1;                                        /*  统计物理 CPU 个数           */
#endif                                                                  /*  LW_CFG_CPU_ARCH_SMT > 0     */
    ...
    LW_CPU_FOREACH (i) {
        pcpu = LW_CPU_GET(i);
        pcpu->CPU_ulHccId = bspCpuLogic2Hetrcc((ULONG)i);               /*  获得物理 CPU ID             */
        _BugFormat(pcpu->CPU_ulHccId >= LW_NCPUS, LW_TRUE,
                   "Heterogeneous Computing Cluster ID error: %lu\r\n", pcpu->CPU_ulHccId);
        if (ulMaxId < pcpu->CPU_ulHccId) {
            ulMaxId = pcpu->CPU_ulHccId;
        }
    }
    _K_ulNHetrccs = ulMaxId + 1;                                        /*  统计算力簇个数              */
