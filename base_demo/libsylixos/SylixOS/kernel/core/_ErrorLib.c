...
/*********************************************************************************************************
  任务上下文信息是否包含调用栈
*********************************************************************************************************/
#define LW_CONTEXT_SHOW_INC_BACKTRACE   0
#define LW_CONTEXT_BACKTRACE_BUF_SIZE   2048
#define LW_CONTEXT_BACKTRACE_DEPTH      20
/*********************************************************************************************************
  函数声明
*********************************************************************************************************/
static VOID  _DebugFmtRaw(VOIDFUNCPTR pfuncPrint, CPCHAR  pcFmt, ...);
/*********************************************************************************************************
** 函数名称: __context_show
** 功能描述: 显示当前上下文信息
** 输　入  : pfuncPrint  打印函数
...
** 调用模块:
*********************************************************************************************************/
VOID  __context_show (VOIDFUNCPTR  pfuncPrint)
{
...
#if LW_CONTEXT_SHOW_INC_BACKTRACE > 0
             CHAR            cBacktraceBuf[LW_CONTEXT_BACKTRACE_BUF_SIZE] = "";
#endif

    if (LW_CPU_GET_CUR_NESTING()) {
        pfuncPrint("in interrupt context.\r\n");
        return;
    }
...
#if LW_CONTEXT_SHOW_INC_BACKTRACE > 0
    API_BacktracePrint(cBacktraceBuf, LW_CONTEXT_BACKTRACE_BUF_SIZE, LW_CONTEXT_BACKTRACE_DEPTH);
    pfuncPrint(cBacktraceBuf);
#endif

#if LW_CFG_MODULELOADER_EN > 0
    pvproc = __LW_VP_GET_TCB_PROC(ptcb);
    _DebugFmtRaw(pfuncPrint, "in thread %lx[%s] proc %d[%s] context.\r\n",
                 ptcb->TCB_ulId, ptcb->TCB_cThreadName,
                 pvproc ? pvproc->VP_pid : 0, pvproc ? pvproc->VP_pcName : "kernel");
#else
    _DebugFmtRaw(pfuncPrint, "in thread %lx[%s] context.\r\n",
                 ptcb->TCB_ulId, ptcb->TCB_cThreadName);
#endif
}
/*********************************************************************************************************
...
VOID  _DebugMessage (INT  iLevel, CPCHAR  pcPosition, CPCHAR  pcString)
{
    ...
    if (_K_pfuncKernelDebugError && (iLevel & __BUGMESSAGE_LEVEL)) {
        ...
        __context_show(_K_pfuncKernelDebugError);
    }
#endif
...
static VOID  _DebugFmtRaw (VOIDFUNCPTR pfuncPrint, CPCHAR  pcFmt, ...)
{
    ...
    va_start(ap, pcFmt);
    _DebugFmtPrint(pfuncPrint, pcFmt, ap);
    va_end(ap);
}
/*********************************************************************************************************
** 函数名称: _DebugFmtMsg
