...
2022.10.13  修正 realloc 中间不能释放 heap lock.
*********************************************************************************************************/
...
/*********************************************************************************************************
** 函数名称: __heap_verify
** 功能描述: 检测这个内存地址是否在指定内存堆中并且已被占用(调用时已经进入护持状态)
** 输　入  : pheap                        堆控制块指针
...
** 调用模块:
*********************************************************************************************************/
static BOOL  __heap_verify (PLW_CLASS_HEAP     pheap,
...
** 调用模块:
*********************************************************************************************************/
static PVOID  __heap_free (PLW_CLASS_HEAP  pheap, PVOID  pvStartAddress, BOOL  bIsNeedVerify, CPCHAR  pcPurpose)
{
    ...
    if (bIsNeedVerify) {                                                /*  是否需要安全性检查          */
        if (!__heap_verify(pheap, pvStartAddress,
    ...
    MONITOR_EVT_LONG3(MONITOR_EVENT_ID_REGION, MONITOR_EVENT_REGION_FREE,
                      pheap, pvStartAddress, psegment->SEGMENT_stByteSize, LW_NULL);
...
PVOID  _HeapFree (PLW_CLASS_HEAP  pheap, PVOID  pvStartAddress, BOOL  bIsNeedVerify, CPCHAR  pcPurpose)
{
    ...
    pvRet = __heap_free(pheap, pvStartAddress, bIsNeedVerify, pcPurpose);
...
__split_segment:
    ...
    __heap_free(pheap, __HEAP_SEGMENT_DATA_PTR(psegmentNew),
