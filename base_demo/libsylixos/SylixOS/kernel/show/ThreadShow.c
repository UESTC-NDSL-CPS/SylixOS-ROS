...
VOID    API_ThreadShowEx (pid_t  pid)
{
    ...
    for (i = 0; i < LW_CFG_MAX_THREADS; i++) {
...
#if (LW_CFG_SMP_EN > 0) && (LW_CFG_CPU_ARCH_HETRC > 0)
        _ThreadGetHetrcc(ptcb, &iHetrMode, LW_NULL);
        if (iHetrMode == LW_HETRCC_WEAK_AFFINITY) {
            cHccMark = '*';                                             /*  弱亲和                      */
        } else if (iHetrMode == LW_HETRCC_STRONG_AFFINITY) {
            if (tcbdesc.TCBD_ulOption & LW_OPTION_THREAD_NUMA) {
                cHccMark = '#';                                         /*  NUMA 亲和                   */
            } else {
                cHccMark = '^';                                         /*  强亲和                      */
            }
        ...
        printf("%-16s %7lx %5ld %3d %-4s %4ld %-4s %10s %7llu %-3s %c %3ld\n",
