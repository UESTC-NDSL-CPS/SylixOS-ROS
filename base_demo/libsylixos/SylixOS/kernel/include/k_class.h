...
typedef struct {
    ...
    INT                   CUR_iKernNesting;                             /*  kernel space nesting        */
...
typedef struct __lw_tcb {
    ...
    PVOID                 TCB_pvTlsLowAddr;                             /*  线程 TLS(起始点)            */
    PVOID                 TCB_pvTlsHighAddr;                            /*  线程 TLS(结束点)            */
...
#define TCB_ulDelay       TCB_wunDelay.WUN_ulCounter
    ...
    INT                   TCB_iHetrccMode;                              /*  异构算力簇模式              */
    ULONG                 TCB_ulHetrccLock;                             /*  推荐或锁定运行的异构算力簇  */
    ULONG                 TCB_ulComputPow;                              /*  使用的算力统计              */
...
#endif
#define LW_SOCK_REF_SIZE    3
    PVOID                 TCB_pvSockRef[LW_SOCK_REF_SIZE];              /*  网络 socket 引用记录        */
#endif
...
typedef struct {
    LW_OBJECT_HANDLE      TCBS_ulId;                                    /*  ID 信息                     */
    BOOL                  TCBS_bIsProc;

    UINT16                TCBS_usIndex;                                 /*  任务基本信息                */
    UINT16                TCBS_usStatus;
    UINT8                 TCBS_ucPriority;
    ULONG                 TCBS_ulOption;

    ULONG                 TCBS_ulCPUId;                                 /*  动态运行信息                */
    BOOL                  TCBS_bIsCand;

    BOOL                  TCBS_bCPULock;                                /*  CPU 亲和调度状态            */
    ULONG                 TCBS_ulCPULock;

    INT                   TCBS_iHetrccMode;                             /*  算力簇亲和调度状态          */
    ULONG                 TCBS_ulHetrccLock;
    ULONG                 TCBS_ulComputPow;

    ULONG                 TCBS_ulReserved[8];                           /*  保留扩展                    */
} LW_CLASS_TCB_SCHED;
typedef LW_CLASS_TCB_SCHED  *PLW_CLASS_TCB_SCHED;
