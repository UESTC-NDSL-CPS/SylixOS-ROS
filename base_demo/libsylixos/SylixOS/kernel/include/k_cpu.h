...
/*********************************************************************************************************
  Heterogeneous computing cluster (异构大小核调度)
*********************************************************************************************************/
#if (LW_CFG_SMP_EN > 0) && (LW_CFG_CPU_ARCH_HETRC > 0)

typedef struct __lw_hcccpu {
    UINT                    HCCCPU_uiActive;                            /*  在线 CPU 数量               */
    UINT                    HCCCPU_uiComputPow;                         /*  计算能力指标                */
    LW_CLASS_PCBBMAP        HCCCPU_pcbbmapReady;                        /*  异构 CPU 簇就绪表           */
} LW_CLASS_HCCCPU;
typedef LW_CLASS_HCCCPU    *PLW_CLASS_HCCCPU;
...
#endif                                                                  /*  LW_CFG_CPU_ARCH_SMT > 0     */
#if LW_CFG_CPU_ARCH_HETRC > 0
#define LW_NHETRCCS          (_K_ulNHetrccs)
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
#else
...
#if (LW_CFG_SMP_EN > 0) && (LW_CFG_CPU_ARCH_HETRC > 0)
#define LW_HCCCPU_GET_CUR()  (&_K_hcccpuTable[LW_CPU_GET_CUR()->CPU_ulHccId])
#define LW_HCCCPU_GET(hccid) (&_K_hcccpuTable[(hccid)])
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
...
#if (LW_CFG_SMP_EN > 0) && (LW_CFG_CPU_ARCH_HETRC > 0)
#define LW_HCCCPU_FOREACH(i)            for (i = 0; i < LW_NHETRCCS; i++)
#define LW_HCCCPU_REVERSE_FOREACH(i)    for (i = (LW_NHETRCCS - 1); (i >= 0) && (i < LW_NHETRCCS); i--)
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
...
#if LW_CFG_CPU_ARCH_HETRC > 0
#define LW_HCC_RDY_PCBBMAP(hccid)       (&(LW_HCCCPU_GET(hccid)->HCCCPU_pcbbmapReady))
#define LW_HCC_RDY_BMAP(hccid)          (&(LW_HCCCPU_GET(hccid)->HCCCPU_pcbbmapReady.PCBM_bmap))
#define LW_HCC_RDY_PPCB(hccid, prio)    (&(LW_HCCCPU_GET(hccid)->HCCCPU_pcbbmapReady.PCBM_pcb[prio]))
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
#endif                                                                  /*  LW_CFG_SMP_EN > 0           */
