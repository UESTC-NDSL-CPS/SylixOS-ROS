    ...
    for (i = 0; pphydesc[i].PHYD_stSize; i++) {
        if ((pphydesc[i].PHYD_uiType == LW_PHYSICAL_MEM_BUSPOOL) ||
            ...
            continue;
                                                                        ...
                                                                        /*  BootSFR 可能内存页面不对齐  */
        adrStartAlign = ROUND_DOWN(pphydesc[i].PHYD_ulPhyAddr, LW_CFG_VMM_PAGE_SIZE);
        stLength      = pphydesc[i].PHYD_stSize + (pphydesc[i].PHYD_ulPhyAddr - adrStartAlign);

        ulPageNum = (ULONG)(stLength >> LW_CFG_VMM_PAGE_SHIFT);
        if (stLength & ~LW_CFG_VMM_PAGE_MASK) {
            ulPageNum++;
        }
        ...
        case LW_PHYSICAL_MEM_TEXT:
            ulError = __vmmLibPageMap(adrStartAlign,
