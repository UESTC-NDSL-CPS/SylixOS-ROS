...
** 功能描述: 共享区域发生改变时, 需要将改变的页面变为 private, 不再被共享.
                                    ...
                                    LW_OBJECT_HANDLE  *pulGuarder)
{
    ...
    PLW_VMM_PAGE_PRIVATE   pvmpagep         = (PLW_VMM_PAGE_PRIVATE)pvmpageVirtual->PAGE_pvAreaCb;

    if (pvmpagep &&
        (pvmpageVirtual->PAGE_ulFlags & LW_VMM_FLAG_WRITABLE)) {        /*  虚拟空间允许写操作          */
        ...
        if (pvmpagep && pvmpagep->PAGEP_stPtSize) {
            if ((ulAbortAddr >= pvmpagep->PAGEP_ulPtStart) ||
                ...
                if (ptcbCur->TCB_pvVProcessContext) {                   /*  进程内修改保护段            */
                    ...
                    return  (PX_ERROR);                                 /*  杀死内核任务, 内存不可写    */
                }
            }
        }
#endif                                                                  /*  LW_CFG_MODULELOADER_TEXT_RO */
...
static PCHAR  __vmmAbortTypeStr (PLW_VMM_ABORT  pabtInfo)
{
    ...
    case LW_VMM_ABORT_TYPE_FPE:
        ...
        case FPE_FLTDIV:
            return  ("float points divide by zero");

        case FPE_FLTOVF:
            return  ("float points overflow");

        case FPE_FLTUND:
            return  ("float points underflow");

        case FPE_FLTRES:
            return  ("float points inexact result");

        case FPE_FLTINV:
            return  ("float points invalid operate");

        case FPE_FLTSUB:
            return  ("float points subscript out of range");
...
static VOID  __vmmAbortKill (PLW_VMM_ABORT_CTX  pabtctx)
{
    ...
    UINT             uiAbortType = __ABTCTX_ABORT_TYPE(pabtctx);

#if LW_CFG_GDB_EN > 0
    if (!__KERNEL_ISENTER()) {
        if ((uiAbortType != LW_VMM_ABORT_TYPE_FPE) &&                   /*  FPU DSP 异常不通知调试器    */
            (uiAbortType != LW_VMM_ABORT_TYPE_DSPE)) {                  /*  由信号处理函数处理          */
    ...
    case LW_VMM_ABORT_TYPE_FPE:
    case LW_VMM_ABORT_TYPE_DSPE:                                        /*  与 FPE 共享分支             */
    ...
    _doSigEventVmmAbort(pabtctx->ABTCTX_ptcb->TCB_ulId, &sigeventAbort, iSigCode, pabtctx);
...
#if LW_CFG_VMM_EN > 0
    if (ptcb->TCB_ucStackAutoAllocFlag == LW_STACK_AUTO_ALLOC) {
