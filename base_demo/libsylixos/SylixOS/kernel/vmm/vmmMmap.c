                            ...
                            int  iFd, off_t  off, LW_OBJECT_HANDLE  *pulGuarder)
{
    ...
    if (iFd >= 0) {
        ...
        if (off > stat64Fd.st_size) {                                   /*  off 越界                    */
            ...
            stLen = ROUND_UP(stat64Fd.st_size - off,
...
** 功能描述: 修改一片虚拟内存属性
** 输　入  : stLen         映射长度
**           iFlags        LW_VMM_SHARED_CHANGE / LW_VMM_PRIVATE_CHANGE / LW_VMM_MAP_FIXED
                               ...
                               ULONG  ulFlag, int  iFd, off_t  off)
{
    PLW_VMM_MAP_NODE    pmapn;
    PLW_VMM_MAP_NODE    pmapnR;
    PLW_VMM_MAP_NODE    pmapnNew;
    PVOID               pvNextAddr = LW_NULL;
    PVOID               pvRetAddr  = pvAddr;

    iFlags &= ~LW_VMM_MAP_FIXED;                                        /*  去掉 LW_VMM_MAP_FIXED      */

__next_addr:
    pmapn = __vmmMapnFindCur(pvAddr);                                   /*  查找 MAP NODE 控制块       */
    if (pmapn == LW_NULL) {
        _ErrorHandle(EINVAL);
        return  (LW_VMM_MAP_FAILED);
    }
    ...
    if ((pvAddr + stLen) > (pmapn->MAPN_pvAddr + pmapn->MAPN_stLen)) {  /*  结束地址在本节点后面        */
        pvNextAddr = pmapn->MAPN_pvAddr + pmapn->MAPN_stLen;            /*  下一次处理的地址            */
    } else {
        pvNextAddr = LW_NULL;
    }

    if ((pmapn->MAPN_pvAddr != pvAddr) ||
        (pmapn->MAPN_stLen  != stLen)) {                                /*  部分修改, 需要拆分节点      */
        if ((pvAddr + stLen) <
            (pmapn->MAPN_pvAddr + pmapn->MAPN_stLen)) {                 /*  结束地址在本节点前          */
            pmapnR = __vmmMmapSplit(pmapn,                              /*  拆分右节点                  */
                                    pvAddr + stLen - pmapn->MAPN_pvAddr);
            if (pmapnR == LW_NULL) {
                return  (LW_VMM_MAP_FAILED);
            }
        } else {
        ...
        if (pvAddr > pmapn->MAPN_pvAddr) {                              /*  起始地址在本节点后          */
            pmapnNew = __vmmMmapSplit(pmapn,                            /*  拆分左节点                  */
                                      pvAddr - pmapn->MAPN_pvAddr);     /*  pmapnNew 为右节点           */
            if (pmapnNew == LW_NULL) {
                if (pmapnR) {
                    __vmmMmapMerge(pmapn, pmapnR);
                }
                return  (LW_VMM_MAP_FAILED);
            }

            pmapn = pmapnNew;                                           /*  使用 pmapnNew 作为目标节点  */
        }
    }

    if (pmapn->MAPN_ulFlag != ulFlag) {                                 /*  映射属性不相关, 需要修改    */
        pmapn->MAPN_ulFlag = ulFlag;
    ...
    if (pvNextAddr) {                                                   /*  还需要进行处理              */
        stLen -= (pvNextAddr - pvAddr);                                 /*  调整剩余长度                */
        if (iFd >= 0) {
            off += (pvNextAddr - pvAddr);                               /*  调整文件映射偏移量          */
        }
        pvAddr = pvNextAddr;                                            /*  继续处理                    */
        goto  __next_addr;
    }

    return  (pvRetAddr);                                                /*  成功, 返回原地址            */
}
/*********************************************************************************************************
** 函数名称: API_VmmMmap
** 功能描述: 内存文件映射.
...
INT  API_VmmMunmap (PVOID  pvAddr, size_t  stLen)
{
    ...
    if (API_VmmMsync(pvAddr, stLen, 0)) {                               /*  取消映射前需要回写文件      */
        return  (PX_ERROR);
    }
    ...
    if (iInval) {
        ...
        for (i = 0; i < ulPageNum; i++) {
            if ((API_VmmGetFlag((PVOID)ulAddr, &ulFlag) == ERROR_NONE) &&
                ...
                stWroteLen = pwrite(pmapn->MAPN_iFd, (CPVOID)ulAddr,
                                    LW_CFG_VMM_PAGE_SIZE, off);
                if ((stWroteLen <= 0) && (errno == ENOSYS)) {
                    return  (ERROR_NONE);                               /*  驱动不支持(如 shm)          */
                } else if (stWroteLen != LW_CFG_VMM_PAGE_SIZE) {
                    return  (PX_ERROR);
                }
            }
        ...
        if (stExcess) {
            if ((API_VmmGetFlag((PVOID)ulAddr, &ulFlag) == ERROR_NONE) &&
                ...
                if ((stWroteLen <= 0) && (errno == ENOSYS)) {
                    return  (ERROR_NONE);                               /*  驱动不支持(如 shm)          */
                } else if (stWroteLen != stExcess) {                    /*  写入文件                    */
                    return  (PX_ERROR);
                }
            }
        }
