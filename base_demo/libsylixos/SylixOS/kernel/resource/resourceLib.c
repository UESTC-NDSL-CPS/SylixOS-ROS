...
static LW_LIST_LINE_HEADER  _G_plineResFile;
/*********************************************************************************************************
  资源表操作锁
*********************************************************************************************************/                                   
...
/*********************************************************************************************************
** 函数名称: __resAddFileHook
** 功能描述: 资源管理器加入一个 FILE 资源
** 输　入  : presraw       FILE 资源缓冲
**           pvfunc        释放函数
**           pvFile        FILE 指针
**           bClose        释放资源时是否关闭文件
**           bFlush        释放资源时是否排干文件流缓冲
** 输　出  : NONE
** 全局变量:
** 调用模块:
*********************************************************************************************************/
VOID  __resAddFileHook (PLW_RESOURCE_RAW    presraw,
                        VOIDFUNCPTR         pvfunc,
                        PVOID               pvFile,
                        BOOL                bClose,
                        BOOL                bFlush)
{
    ...
    presraw->RESRAW_pvArg[0] = pvFile;
    presraw->RESRAW_pvArg[1] = (PVOID)(LONG)bClose;
    presraw->RESRAW_pvArg[2] = (PVOID)(LONG)bFlush;

    presraw->RESRAW_pfuncFree = pvfunc;

    __LW_RESRAW_LOCK();
    _List_Line_Add_Ahead(&presraw->RESRAW_lineManage, &_G_plineResFile);
    presraw->RESRAW_bIsInstall = LW_TRUE;                               /*  安装成功                    */
    __LW_RESRAW_UNLOCK();
}
/*********************************************************************************************************
** 函数名称: __resDelFileHook
** 功能描述: 从资源管理器去除一个 FILE 资源
** 输　入  : presraw       FILE 资源缓冲
** 输　出  : NONE
** 全局变量:
** 调用模块:
*********************************************************************************************************/
VOID  __resDelFileHook (PLW_RESOURCE_RAW    presraw)
{
    if (!presraw) {
        return;
    }
    ...
    __LW_RESRAW_LOCK();
    _List_Line_Del(&presraw->RESRAW_lineManage, &_G_plineResFile);
    presraw->RESRAW_bIsInstall = LW_FALSE;
    __LW_RESRAW_UNLOCK();
}
/*********************************************************************************************************
...
** 全局变量: 
** 调用模块: 
** 注  意  : FILE 资源以及 I/O FD 资源已在进程退出时回收过了, 所以这里仅仅是为了确保中间对应驱动 close 内
**           存在 open 或者 fopen 的情况, 所以再次进行回收操作.
*********************************************************************************************************/
    ...
    while (plineTemp) {
        ...
        if (presraw->RESRAW_pid == pid) {
            presraw->RESRAW_pfuncFree(presraw->RESRAW_pvArg[0],
                                      ...
                                      presraw->RESRAW_pvArg[2]);        /*  释放 FILE 资源              */
        }
    }
...
/*********************************************************************************************************
** 函数名称: __resPidReclaimOnlyFile
** 功能描述: 从资源管理器删除对应进程的 FILE 资源
** 输　入  : pid           进程号
**           bClose        是否关闭文件
**           bFlush        是否排干文件流缓冲
** 输　出  : 是否成功, 如果不成功证明这个进程还有线程在运行.
...
** 调用模块:
*********************************************************************************************************/
INT  __resPidReclaimOnlyFile (pid_t  pid, BOOL  bClose, BOOL  bFlush)
{
    PLW_RESOURCE_RAW    presraw;
    PLW_LIST_LINE       plineTemp;
