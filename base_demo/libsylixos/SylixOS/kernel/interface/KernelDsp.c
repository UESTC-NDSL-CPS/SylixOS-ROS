...
** 全局变量: 
** 调用模块: 
** 注  意  : 此函数在 CPU 主核心启动时运行, 所以此时存在的仅仅是系统内建的基础任务,
**           所以这里初始化为不需要异常
**
                                           API 函数
*********************************************************************************************************/
...
VOID  API_KernelDspPrimaryInit (CPCHAR  pcMachineName, CPCHAR  pcDspName)
{
    ...
    for (plineList  = _K_plineTCBHeader;
        ...
        __ARCH_DSP_CTX_INIT(ptcb->TCB_pvStackDSP, LW_FALSE);            /*  初始化该任务 DSP 上下文     */
    }
    ...
    for (i = 0; i < LW_CFG_MAX_PROCESSORS; i++) {
        for (j = 0; j < LW_CFG_MAX_INTER_SRC; j++) {                    /*  初始化, 但是不使能 DSP      */
            __ARCH_DSP_CTX_INIT((PVOID)&(LW_CPU_GET(i)->CPU_dspctxContext[j]), LW_FALSE);
        }
    }
#endif                                                                  /*  LW_CFG_INTER_DSP > 0        */
    
    _DebugHandle(__LOGMESSAGE_LEVEL, "DSP initialized.\r\n");
}
/*********************************************************************************************************
...
VOID  API_KernelDspSecondaryInit (CPCHAR  pcMachineName, CPCHAR  pcDspName)
{
    ...
    _DebugHandle(__LOGMESSAGE_LEVEL, "secondary DSP initialized.\r\n");
}
