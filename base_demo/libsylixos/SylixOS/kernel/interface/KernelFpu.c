...
VOID  API_KernelFpuPrimaryInit (CPCHAR  pcMachineName, CPCHAR  pcFpuName)
{
    ...
    for (plineList  = _K_plineTCBHeader;
        ...
        __ARCH_FPU_CTX_INIT(ptcb->TCB_pvStackFP, LW_FALSE);             /*  初始化该任务 FPU 上下文     */
    }
    ...
    for (i = 0; i < LW_CFG_MAX_PROCESSORS; i++) {
        for (j = 0; j < LW_CFG_MAX_INTER_SRC; j++) {                    /*  初始化, 但是不使能 FPU      */
            __ARCH_FPU_CTX_INIT((PVOID)&(LW_CPU_GET(i)->CPU_fpuctxContext[j]), LW_FALSE);
        }
    }
#endif                                                                  /*  LW_CFG_INTER_FPU > 0        */
    
    _DebugHandle(__LOGMESSAGE_LEVEL, "FPU initialized.\r\n");
}
/*********************************************************************************************************
...
VOID  API_KernelFpuSecondaryInit (CPCHAR  pcMachineName, CPCHAR  pcFpuName)
{
    ...
    _DebugHandle(__LOGMESSAGE_LEVEL, "secondary FPU initialized.\r\n");
}
