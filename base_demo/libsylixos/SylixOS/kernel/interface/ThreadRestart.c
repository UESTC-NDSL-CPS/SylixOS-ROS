...
ULONG  API_ThreadRestartEx (LW_OBJECT_HANDLE  ulId, PTHREAD_START_ROUTINE  pfuncThread, PVOID  pvArg)
{
    ...
    if ((ptcbCur->TCB_ulId == ulId) &&
        (ptcbCur->TCB_ulThreadSafeCounter == 0ul)) {                    /*  条件允许时 重启自己         */
    ...
    if (ptcb == ptcbCur) {
        _ThreadRestartProcHook(ptcb, LW_FALSE);                         /*  处理 hook 相关操作          */
        ...
        ulError = ERROR_NONE;                                           /*  不会运行到这里              */
    
    } else {                                                            /*  删除其他任务                */
        _ThreadDeleteWaitDeath(ptcb);                                   /*  将要删除的线程进入僵死状态  */
        
        _ThreadRestartProcHook(ptcb, LW_TRUE);                          /*  处理 hook 相关操作          */
