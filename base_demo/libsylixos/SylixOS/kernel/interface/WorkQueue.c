...
/*********************************************************************************************************
  无 Q 节点时默认等待时间
*********************************************************************************************************/
#define LW_DQUEUE_IDLE_TICK     (LW_TICK_HZ * 100)
/*********************************************************************************************************
  简单作业队列控制结构
*********************************************************************************************************/
typedef struct {
...
static PLW_WORK_QUEUE  __wqDCreate (PLW_WORK_QUEUE  pwq, UINT  uiQSize, ULONG  ulScanPeriod)
{
    ...
    __WAKEUP_INIT(&pwq->q.WQ_dq.DWQ_wakeup, LW_NULL, LW_NULL);
                           ...
                           PVOID           pvArg5)
{
    ...
    _WakeupAdd(&pwq->q.WQ_dq.DWQ_wakeup, &pwdn->DWQN_wun, LW_TRUE);
...
static VOID  __wqDExec (PLW_WORK_QUEUE  pwq)
{
    ...
    pwq->q.WQ_dq.DWQ_wakeup.WU_i64LastTime = API_TimeGet64();           /*  记录当前执行时间            */

    __WAKEUP_GET_FIRST(&pwq->q.WQ_dq.DWQ_wakeup, pwun);                 /*  获得第一个节点              */

    if (pwun) {
        ulSleep = pwun->WUN_ulCounter;                                  /*  已第一个节点等待时间 Sleep  */
    } else {
        ulSleep = LW_DQUEUE_IDLE_TICK;
    }

    API_SemaphoreMPost(pwq->q.WQ_dq.DWQ_ulLock);

    API_SemaphoreBPend(pwq->q.WQ_dq.DWQ_ulSem, ulSleep);                /*  等待节点时间到期            */
    ...
    i64Now  = API_TimeGet64();
    ulSleep = i64Now - pwq->q.WQ_dq.DWQ_wakeup.WU_i64LastTime;
    pwq->q.WQ_dq.DWQ_wakeup.WU_i64LastTime = i64Now;

    __WAKEUP_PASS_FIRST(&pwq->q.WQ_dq.DWQ_wakeup, pwun, ulSleep);

    pwdn = _LIST_ENTRY(pwun, LW_WORK_DNODE, DWQN_wun);                  /*  存在需要执行的节点          */

    _WakeupDel(&pwq->q.WQ_dq.DWQ_wakeup, pwun, LW_FALSE);
    pwq->q.WQ_dq.DWQ_uiCount--;                                         /*  删除此节点                  */
    ...
    _list_mono_free(&pwq->q.WQ_dq.DWQ_pmonoPool, &pwdn->DWQN_monoList); /*  是否节点到节点池            */

    API_SemaphoreMPost(pwq->q.WQ_dq.DWQ_ulLock);

    if (pfunc) {
        LW_SOFUNC_PREPARE(pfunc);
        pfunc(pvArg[0], pvArg[1], pvArg[2],
              pvArg[3], pvArg[4], pvArg[5]);                            /*  执行回调                    */
    }

    API_SemaphoreMPend(pwq->q.WQ_dq.DWQ_ulLock, LW_OPTION_WAIT_INFINITE);

    i64Now = API_TimeGet64();
    if (i64Now > pwq->q.WQ_dq.DWQ_wakeup.WU_i64LastTime) {              /*  进行花费时间修正            */
        ulSleep += i64Now - pwq->q.WQ_dq.DWQ_wakeup.WU_i64LastTime;
        pwq->q.WQ_dq.DWQ_wakeup.WU_i64LastTime = i64Now;
    }
