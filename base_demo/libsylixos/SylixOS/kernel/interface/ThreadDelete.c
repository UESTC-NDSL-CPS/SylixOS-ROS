                       ...
                       PVOID  pvRetVal, BOOL  bIsAlreadyWaitDeath)
{
...
#if LW_CFG_MODULELOADER_EN > 0
    vprocTlsFree(ptcbDel);                                              /*  释放 TLS 空间               */
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */
    ...
    if (__threadNeedJwt(bDetachFlag, iDetachCnt, ptcbDel)) {            /*  需要进入 JWT                */
        ...
        _Free_Tcb_Object(ptcbDel);                                      /*  释放 TCB                    */
    }
...
ULONG  API_ThreadDelete (LW_OBJECT_HANDLE  *pulId, PVOID  pvRetVal)
{
    ...
    if ((ptcbCur->TCB_ulId == ulId) &&
        (ptcbCur->TCB_ulThreadSafeCounter == 0ul)) {                    /*  条件允许时 删除自己         */
        ptcbCur->TCB_iCancelState = LW_THREAD_CANCEL_DISABLE;           /*  不允许 cancel               */
        KN_SMP_WMB();
        _TCBCleanupPopExt(ptcbCur, LW_TRUE);                            /*  提前执行                    */
    }
    ...
    ptcbDel = _K_ptcbTCBIdTable[usIndex];                               /*  获取目标 ptcb               */
    ...
    LW_THREAD_SAFE_INKERN(ptcbCur);                                     /*  进入安全模式                */
    ...
    if (ptcbDel == ptcbCur) {
        _ThreadDeleteProcHook(ptcbDel, pvRetVal, LW_FALSE);             /*  处理 hook 相关操作          */
        ...
        ulError = ERROR_NONE;                                           /*  不可能运行到这里            */
    
    } else {                                                            /*  删除其他任务                */
        _ThreadDeleteWaitDeath(ptcbDel);                                /*  将要删除的线程进入僵死状态  */
        
        _ThreadDeleteProcHook(ptcbDel, pvRetVal, LW_TRUE);              /*  处理 hook 相关操作          */
