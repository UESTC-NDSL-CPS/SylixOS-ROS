...
extern LW_API VOID  API_DiskCacheParam(CPCHAR  pcParam);
#endif                                                                  /*  (LW_CFG_MAX_VOLUMES > 0)    */
                                                                        /*  (LW_CFG_DISKCACHE_EN > 0)   */
#endif                                                                  /*  (LW_CFG_DEVICE_EN > 0)      */
#if LW_CFG_TPSFS_EN > 0
extern LW_API VOID  API_TpsTransModeParam(CPCHAR  pcParam);
...
** 输　入  : pcParam       启动参数, 是以空格分开的一个字符串列表，通常具有如下形式:
                           ...
                           hsdcwt=          高速存储设备驱动是否磁盘缓存写穿透 nvme, sata ...

                           tpstransmode=single          TPS 模式的事务提交模式, single 单次提交
                                                                                file 合并文件数据操作事务
                                                                                volume 合并所有文件系统元数据操作事务

                           rebootcacheen=no 内核 reboot 时是否使能 cache
                           rebootvmmen=no   内核 reboot 时是否使能 mmu
                                            如果此两项任意一项为 yes, 则 reboot 时不关闭中断
                                            bspReboot 函数需要自行处理这些情况
...
ULONG  API_KernelStartParam (CPCHAR  pcParam)
{
    ...
    while (pcTok) {
        if (lib_strncmp(pcTok, "ncpus=", 6) == 0) {                     /*  CPU 数量                    */
          ...
          else if (lib_strncmp(pcTok, "autorectcb=", 11) == 0) {        /*  自动删除 TCB                */
            if (pcTok[11] == 'n') {
...
#if (LW_CFG_MAX_VOLUMES > 0) && (LW_CFG_DISKCACHE_EN > 0)
          else if (lib_strncmp(pcTok, "hsdcwt=", 7) == 0) {             /*  高速储存 cache 写穿透模式   */
            API_DiskCacheParam(pcTok);
        }
#endif                                                                  /*  (LW_CFG_MAX_VOLUMES > 0)    */
                                                                        /*  (LW_CFG_DISKCACHE_EN > 0)   */
#endif                                                                  /*  (LW_CFG_DEVICE_EN > 0)      */
...
#if LW_CFG_TPSFS_EN > 0
          else if (lib_strncmp(pcTok, "tpstransmode=", 13) == 0) {      /*  TpsFs 事务模式              */
              API_TpsTransModeParam(&pcTok[13]);
        }
#endif                                                                  /*  (LW_CFG_TPSFS_EN > 0)       */
