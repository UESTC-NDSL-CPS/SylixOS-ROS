...
/*********************************************************************************************************
  使用文件结构的资源成员的第四个参数用于记录文件结构是否需要被释放
*********************************************************************************************************/
#define __FILE_NEED_FREE(fp)    ((fp)->resraw.RESRAW_pvArg[3])
/*********************************************************************************************************
** 函数名称: __lib_newfile
** 功能描述: 创建一个文件结构
...
FILE  *__lib_newfile (FILE  *pfFile)
{
    ...
    if (bNeedFree) {
        __FILE_NEED_FREE(pfFile) = (PVOID)(LONG)bNeedFree;              /* 记录是否需要释放内存         */
    }

#if LW_CFG_MODULELOADER_EN > 0
    __resAddFileHook(&pfFile->resraw, (VOIDFUNCPTR)fclose_ex,           /* 默认不需要 flush 和 close    */
                     pfFile, LW_FALSE, LW_FALSE);
...
{
#if LW_CFG_MODULELOADER_EN > 0
    __resDelFileHook(&pfFile->resraw);
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */

    if (__FILE_NEED_FREE(pfFile)) {                                     /* 需要释放内存                 */
        lib_free(pfFile);
    }
}
/*********************************************************************************************************
