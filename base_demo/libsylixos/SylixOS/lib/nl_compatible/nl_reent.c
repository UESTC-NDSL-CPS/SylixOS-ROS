...
static VOID  __nlreent_delete_hook (LW_OBJECT_HANDLE  ulThread)
{
    ...
    for (i = 0; i < 3; i++) {
        if (plwreent->_file[i]._flags) {
            fclose_ex(&plwreent->_file[i], LW_FALSE, LW_TRUE);
        }
    }
...
/*********************************************************************************************************
** 函数名称: lib_nlreent_make_local
** 功能描述: 将当前线程的 stdfile 结构重新加入资源管理器 (确定正确的回收 pid)
** 输　入  : NONE
** 输　出  : NONE
** 全局变量:
** 调用模块:
...
INT  lib_nlreent_make_local (VOID)
{
    REGISTER __LW_REENT    *plwreent;
    REGISTER PLW_CLASS_TCB  ptcbCur;
    REGISTER FILE          *files;
    ...
    plwreent = &_G_lwreentTbl[ptcbCur->TCB_usIndex];
    files    = plwreent->_file;

    for (i = 0; i < 3; i++) {
        __resDelFileHook(&files[i].resraw);
        __resAddFileHook(&files[i].resraw, (VOIDFUNCPTR)fclose_ex,      /* 默认不需要 flush 和 close    */
                         &files[i], LW_FALSE, LW_FALSE);
    }
