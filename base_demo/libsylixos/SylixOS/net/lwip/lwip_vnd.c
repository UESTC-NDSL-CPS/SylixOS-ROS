...
static ssize_t  _vndWrite (PLW_VND_FILE  pvndfil, PCHAR  pcBuffer, size_t  stNBytes)
{
    ...
    if (pvnd->VND_vnetdev.type == IF_VND_TYPE_ETHERNET) {
        ...
        p = netdev_pbuf_alloc_raw((u16_t)stNBytes, 0);                  /*  不需要偏移                  */
    }
    if (!p) {
        _ErrorHandle(ENOMEM);
        return  (PX_ERROR);
...
{
    BOOL          bChecksum, bGensum;
    PLW_VND_DEV   pvndNew;
    PLW_LIST_LINE pline;
    
    if (pifvnd->ifvnd_type & IF_VND_FLAG_NOGENSUM) {                    /*  不进行 gensum               */
        bGensum = LW_FALSE;
    } else {
        bGensum = LW_TRUE;
    }

    if (pifvnd->ifvnd_type & IF_VND_FLAG_NOCHKSUM) {                    /*  不进行 checksum             */
        bChecksum = LW_FALSE;
    } else {
        bChecksum = LW_TRUE;
    }

    pifvnd->ifvnd_type &= ~IF_VND_FLAGS_MASK;                           /*  去掉 mask                   */
    ...
    if (vnetdev_add(&pvndNew->VND_vnetdev, _vndNotifyDev, pifvnd->ifvnd_bsize,
                    pifvnd->ifvnd_id, pifvnd->ifvnd_type, bGensum, bChecksum, pvndNew)) {
        VND_LOCK();
...
static INT  __tshellVnd (INT  iArgC, PCHAR  *ppcArgV)
{
    ...
    if (!lib_strcmp(ppcArgV[1], "add")) {
        ...
        if (iArgC > 4) {
            if (ppcArgV[4][0] == 'y' || ppcArgV[4][0] == 'Y') {
                ifvnd.ifvnd_type |= IF_VND_FLAG_NOCHKSUM;
            }
        }
...
static VOID __tshellVndInit (VOID)
{
    ...
    API_TShellFormatAdd("vnd", " [add | del] [id] [hwaddr] [nochksum]");
