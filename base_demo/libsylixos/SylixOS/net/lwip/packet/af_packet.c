...
static AF_PACKET_T  *__packetCreate (INT  iType, INT  iProtocol)
{
    ...
    pafpacket->PACKET_uiSecReg = vprocSecRegionGetCur();
...
INT  packet_link_input (struct pbuf *p, struct netif *inp, BOOL bOutgo)
{
    ...
    for (plineTemp  = _G_plineAfPacket;
        ...
        if (netif_is_security(inp) &&
            (netif_get_security(inp) != pafpacket->PACKET_uiSecReg)) {  /*  不是一个安全域              */
            continue;
        }
#endif                                                                  /*  LW_CFG_LWIP_SEC_REGION     */
                        ...
                        const void *optval, socklen_t optlen)
{
    ...
    case SOL_SOCKET:
        ...
        case SO_SECREGION:
            if (optlen != sizeof(UINT)) {
                _ErrorHandle(EINVAL);
            } else if (*(UINT *)optval && geteuid()) {
                _ErrorHandle(EACCES);
            } else {
                pafpacket->PACKET_uiSecReg = *(UINT *)optval;
                iRet = ERROR_NONE;
            }
            break;
    ...
    case SOL_SOCKET:
        ...
        case SO_SECREGION:
            if (*optlen != sizeof(UINT)) {
                ...
                *(UINT *)optval = pafpacket->PACKET_uiSecReg;
                iRet = ERROR_NONE;
            }
            break;
