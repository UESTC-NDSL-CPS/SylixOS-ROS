...
** 调用模块:
*********************************************************************************************************/
static VOID  __unix_cloexec (INT  iFd, INT  iCloExec)
{
#if LW_CFG_MODULELOADER_EN > 0
    if (__PROC_GET_PID_CUR()) {
        vprocIoFileCloExec(iFd, iCloExec);
    } else
            ...
            if (pcmhdr->cmsg_type == SCM_RIGHTS) {                      /*  传送文件描述符              */
                ...
                for (i = 0; i < iNum; i++) {                            
                    ...
                    if ((flags & MSG_CMSG_CLOEXEC) && (iFdArry[i] >= 0)) {
                        __unix_cloexec(iFdArry[i], FD_CLOEXEC);
                    }
                }
            }
        }
