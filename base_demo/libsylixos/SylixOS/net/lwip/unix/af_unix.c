...
2023.05.05  增加 direct accept 支持.
*********************************************************************************************************/
...
/*********************************************************************************************************
** 函数名称: __unixDeleteAllMsg
** 功能描述: 删除所有 unix 节点的所有消息节点
** 输　入  : pafunix               unix 节点
** 输　出  : NONE
** 全局变量: 
** 调用模块: 
...
/*********************************************************************************************************
** 函数名称: __unixCanDirectAccept
** 功能描述: 查看直接 accept 的队列中是否有可直接 accept 的节点
** 输　入  : pafunix               控制块
** 输　出  : 是否有可直接 accept 的节点
** 全局变量:
** 调用模块:
*********************************************************************************************************/
static BOOL  __unixCanDirectAccept (AF_UNIX_T  *pafunix)
{
    if (pafunix->UNIX_pringAccept == LW_NULL) {
        return  (LW_FALSE);
...
** 调用模块:
*********************************************************************************************************/
static VOID __unixSockName (AF_UNIX_T  *pafunix,
                            struct sockaddr_un *paddrun, socklen_t *addrlen, INT  *msg_flags)
{
    size_t  stPathLen;
                                   ...
                                   INT  *msg_flags)
{
    ...
    if (from && fromlen && (*fromlen > __AF_UNIX_ADDROFFSET)) {         /*  是否需要获取地址信息        */
        if (__AF_UNIX_TYPE(pafunixRecver) == SOCK_DGRAM) {              /*  非面向连接需要从每个包中获取*/
            ...
            from->sun_len    = (uint8_t)(__AF_UNIX_ADDROFFSET + stPathLen);
            *fromlen = from->sun_len;                                   /*  不包括 \0 的总长度          */
        
        } else {
            __unixSockName(pafunixRecver->UNIX_pafunxPeer,
                           from, fromlen, msg_flags);                   /*  获得对端地址                */
        }
    }
...
** 输　入  : pafunix               控制块
**           bClearDirectAccept    是否删除所有可直接 accept 节点
** 输　出  : NONE
** 全局变量: 
** 调用模块: 
*********************************************************************************************************/
static VOID  __unixRefuseAll (AF_UNIX_T  *pafunix, BOOL  bClearDirectAccept)
{
    AF_UNIX_T      *pafunixConn;
    AF_UNIX_T      *pafunixPeer;
    PLW_LIST_RING   pringConn;
    ...
    if (bClearDirectAccept) {
        while (pafunix->UNIX_pringAccept) {
            pringConn = pafunix->UNIX_pringAccept;                      /*  从 accept 表头获取          */
            ...
            _List_Ring_Del(&pafunixConn->UNIX_ringConnect,
                           &pafunix->UNIX_pringAccept);                 /*  从 accept 连接表中删除      */
            ...
            __AF_UNIX_UNLOCK();                                         /*  删除操作需要解锁            */
            __unixDelete(pafunixConn);
            __AF_UNIX_LOCK();
        }
    }
}
/*********************************************************************************************************
** 函数名称: __unixPreAccept
** 功能描述: 从请求连接的队列中取出一个并进行预 accept
** 输　入  : pafunix               控制块
** 输　出  : NONE
** 全局变量:
** 调用模块:
*********************************************************************************************************/
static VOID  __unixPreAccept (AF_UNIX_T  *pafunix)
{
    ...
    if (pafunixNew == LW_NULL) {
       __AF_UNIX_LOCK();
       __unixRefuseAll(pafunix, LW_FALSE);                              /*  没有足够内存, 拒绝连接      */
       return;
    }
    __AF_UNIX_LOCK();
    ...
    if (pafunixConn) {
       ...
       _List_Ring_Add_Last(&pafunixNew->UNIX_ringConnect,
                           &pafunix->UNIX_pringAccept);
...
INT  unix_bind (AF_UNIX_T  *pafunix, const struct sockaddr *name, socklen_t namelen)
{
    ...
    if ((pcPath[0] == PX_EOS) ||
        !lib_strcmp(pcPath, PX_STR_ROOT) || !lib_strcmp(pcPath, "//")) {
        _ErrorHandle(EINVAL);                                           /*  地址无效                   */
        return  (PX_ERROR);
    }

    lib_strncpy(cPath, pcPath, iPathLen);
...
AF_UNIX_T  *unix_accept (AF_UNIX_T  *pafunix, struct sockaddr *addr, socklen_t *addrlen)
{
    ...
    __AF_UNIX_LOCK();
    do {
        if (__unixCanDirectAccept(pafunix)) {                           /*  有可直接 accept             */
            pafunixNew = _LIST_ENTRY(pafunix->UNIX_pringAccept, AF_UNIX_T, UNIX_ringConnect);

            if (paddrun && addrlen && (*addrlen > __AF_UNIX_ADDROFFSET)) {
                __unixSockName(pafunixNew, paddrun, addrlen, LW_NULL);
            }                                                           /*  记录请求连接节点的信息      */

            _List_Ring_Del(&pafunixNew->UNIX_ringConnect,
                           &pafunix->UNIX_pringAccept);
            __AF_UNIX_UNLOCK();
            return  (pafunixNew);
        }
        ...
        if (__unixCanAccept(pafunix)) {                                 /*  是否可以 accept             */
            ...
            if (pafunixConn) {
                ...
                if (paddrun && addrlen && (*addrlen > __AF_UNIX_ADDROFFSET)) {
                    __unixSockName(pafunixConn, paddrun, addrlen, LW_NULL);
                }                                                       /*  记录请求连接节点的信息      */
...
__try_send:
        if (__unixCanWrite(pafunixRecver)) {                            /*  可以发送                    */
            ...
            if (sstWriteNum <= 0) {
                if (sstTotal == 0) {
                    sstTotal =  sstWriteNum;                            /*  未实现正常发送, 返回 wnum   */
                }
...
INT  unix_close (AF_UNIX_T  *pafunix)
{
    ...
    if ((__AF_UNIX_TYPE(pafunix) == SOCK_STREAM) ||
        ...
        if (pafunix->UNIX_iStatus == __AF_UNIX_STATUS_CONNECT) {        /*  正在连接                    */
            ...
            __unixRefuseAll(pafunix, LW_TRUE);                          /*  拒绝所有连接请求            */
        }
        pafunixPeer = pafunix->UNIX_pafunxPeer;
        if (pafunixPeer) {
            /*
...
INT  unix_getsockname (AF_UNIX_T  *pafunix, struct sockaddr *name, socklen_t *namelen)
{
    ...
    if (paddrun && namelen && (*namelen > __AF_UNIX_ADDROFFSET)) {
        __AF_UNIX_LOCK();
        __unixSockName(pafunix, paddrun, namelen, LW_NULL);
        __AF_UNIX_UNLOCK();
    }
...
INT  unix_getpeername (AF_UNIX_T  *pafunix, struct sockaddr *name, socklen_t *namelen)
{
    ...
    if (paddrun && namelen && (*namelen > __AF_UNIX_ADDROFFSET)) {
        __AF_UNIX_LOCK();
        __unixSockName(pafunix->UNIX_pafunxPeer, paddrun, namelen, LW_NULL);
        __AF_UNIX_UNLOCK();
    }
        ...
        if ((__AF_UNIX_TYPE(pafunix) == SOCK_STREAM) ||
            ...
            case __AF_UNIX_STATUS_LISTEN:
                if (__unixCanDirectAccept(pafunix)) {                   /*  有可直接 accept             */
                    *piSoErr = ERROR_NONE;
                    iEvent   = 1;
                } else if (__unixCanAccept(pafunix)) {                  /*  可以 accept                 */
                    *piSoErr = ERROR_NONE;
                    iEvent   = 1;
                    __unixPreAccept(pafunix);                           /*  预 accept                   */
                }
                break;
