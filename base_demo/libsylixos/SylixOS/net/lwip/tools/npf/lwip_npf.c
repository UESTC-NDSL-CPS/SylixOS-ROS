...
2023.03.06  加入对 do not forword 类型进行限制.
*********************************************************************************************************/
...
*********************************************************************************************************/
static BOOL  __npfMacRuleCheck (__PNPF_NETIF_CB  pnpfni, BOOL  bIn,
                                struct eth_hdr *pethhdr, struct pbuf *p)
{
    BOOL                bAllow       = LW_TRUE;
    BOOL                bDontForward = LW_FALSE;
    BOOL                bDenied      = LW_FALSE;
    PLW_LIST_LINE       plineTemp;
    ...
    while (plineTemp) {
        ...
        if (pnpfrm->NPFRM_iFlags & LWIP_NPF_RULE_FLAG_ALLOW) {
            ...
            if (lib_memcmp(pnpfrm->NPFRM_ucMac,
                ...
                bAllow = LW_FALSE;                                      /*  禁止通过                    */
                if (!bDenied) {
                    bDontForward = (pnpfrm->NPFRM_iFlags
                                 & LWIP_NPF_RULE_FLAG_DONT_FORWARD)
                                 ? LW_TRUE : LW_FALSE;                  /*  是否仅为不允许转发          */
                    if (!bDontForward) {
                        bDenied = LW_TRUE;                              /*  有任意一个禁止项 DF 既不允许*/
                    }
                }
            }
        }
    ...
    if (!bAllow && bDontForward) {
        bAllow = LW_TRUE;
        p->ext_flags |= PBUF_EXTFLAG_DONT_FORWARD;                      /*  仅不允许路由转发            */
    }
...
** 调用模块:
*********************************************************************************************************/
static BOOL  __npfIpRuleCheck (__PNPF_NETIF_CB  pnpfni, BOOL  bIn, struct ip_hdr *piphdr, struct pbuf *p)
{
    BOOL                bAllow       = LW_TRUE;
    BOOL                bDontForward = LW_FALSE;
    BOOL                bDenied      = LW_FALSE;
    ...
    while (plineTemp) {
        ...
        if (pnpfri->NPFRI_iFlags & LWIP_NPF_RULE_FLAG_ALLOW) {
            ...
            if ((ipaddrHbo.addr >= pnpfri->NPFRI_ipaddrHboS.addr) &&
                ...
                bAllow = LW_FALSE;                                      /*  禁止通过                    */
                if (!bDenied) {
                    bDontForward = (pnpfri->NPFRI_iFlags
                                 & LWIP_NPF_RULE_FLAG_DONT_FORWARD)
                                 ? LW_TRUE : LW_FALSE;                  /*  是否仅为不允许转发          */
                    if (!bDontForward) {
                        bDenied = LW_TRUE;                              /*  有任意一个禁止项 DF 既不允许*/
...
static BOOL  __npfUdpRuleCheck (__PNPF_NETIF_CB  pnpfni, BOOL  bIn, 
                                struct ip_hdr *piphdr, struct udp_hdr *pudphdr, struct pbuf *p)
{
    BOOL                bAllow       = LW_TRUE;
    BOOL                bDontForward = LW_FALSE;
    BOOL                bDenied      = LW_FALSE;
    ...
    while (plineTemp) {
        ...
        if (pnpfru->NPFRU_iFlags & LWIP_NPF_RULE_FLAG_ALLOW) {
            ...
            if ((ipaddrHbo.addr >= pnpfru->NPFRU_ipaddrHboS.addr) &&
                ...
                bAllow = LW_FALSE;                                      /*  禁止通过                    */
                if (!bDenied) {
                    bDontForward = (pnpfru->NPFRU_iFlags
                                 & LWIP_NPF_RULE_FLAG_DONT_FORWARD)
                                 ? LW_TRUE : LW_FALSE;                  /*  是否仅为不允许转发          */
                    if (!bDontForward) {
                        bDenied = LW_TRUE;                              /*  有任意一个禁止项 DF 既不允许*/
...
static BOOL  __npfTcpRuleCheck (__PNPF_NETIF_CB  pnpfni, BOOL  bIn, 
                                struct ip_hdr *piphdr, struct tcp_hdr *ptcphdr, struct pbuf *p)
{
    BOOL                bAllow       = LW_TRUE;
    BOOL                bDontForward = LW_FALSE;
    BOOL                bDenied      = LW_FALSE;
    ...
    while (plineTemp) {
        ...
        if (pnpfrt->NPFRT_iFlags & LWIP_NPF_RULE_FLAG_ALLOW) {
            ...
            if ((ipaddrHbo.addr >= pnpfrt->NPFRT_ipaddrHboS.addr) &&
                ...
                bAllow = LW_FALSE;                                      /*  禁止通过                    */
                if (!bDenied) {
                    bDontForward = (pnpfrt->NPFRT_iFlags
                                 & LWIP_NPF_RULE_FLAG_DONT_FORWARD)
                                 ? LW_TRUE : LW_FALSE;                  /*  是否仅为不允许转发          */
                    if (!bDontForward) {
                        bDenied = LW_TRUE;                              /*  有任意一个禁止项 DF 既不允许*/
...
static INT  npf_netif_firewall (struct netif *pnetif, struct pbuf *p)
{
    ...
    if (pnetif->flags & (NETIF_FLAG_ETHARP | NETIF_FLAG_ETHERNET)) {    /*  以太网接口                  */
        ...
        if (__npfMacRuleCheck(pnpfni, LW_TRUE, pethhdr, p)) {           /*  开始检查相应的 MAC 过滤规则 */
    ...
    if (__npfIpRuleCheck(pnpfni, LW_TRUE, piphdr, p)) {                 /*  开始检查相应的 ip 过滤规则  */
    ...
    case IP_PROTO_UDPLITE:
        ...
        if (__npfUdpRuleCheck(pnpfni, LW_TRUE, piphdr, pudphdr, p) == LW_FALSE) {
            goto    __drop_input;
        }
        break;
    ...
    case IP_PROTO_TCP:                                                  /*  tcp 过滤处理                */
        ...
        if (__npfTcpRuleCheck(pnpfni, LW_TRUE, piphdr, ptcphdr, p) == LW_FALSE) {
            goto    __drop_input;
        }
        break;
    ...
    if (pnetif->flags & (NETIF_FLAG_ETHARP | NETIF_FLAG_ETHERNET)) {    /*  以太网接口                  */
        ...
        if (__npfMacRuleCheck(pnpfni, LW_TRUE, &ethhdrChk, p)) {        /*  开始检查相应的 MAC 过滤规则 */
            iOffset += sizeof(struct eth_hdr);                          /*  允许通过                    */
    ...
    if (__npfIpRuleCheck(pnpfni, LW_TRUE, &iphdrChk, p)) {              /*  开始检查相应的 ip 过滤规则  */
    ...
    case IP_PROTO_UDPLITE:
        ...
        if (__npfUdpRuleCheck(pnpfni, LW_TRUE, &iphdrChk, &udphdrChk, p) == LW_FALSE) {
            goto    __drop_input;
        }
        break;
    ...
    case IP_PROTO_TCP:                                                  /*  tcp 过滤处理                */
        ...
        if (__npfTcpRuleCheck(pnpfni, LW_TRUE, &iphdrChk, &tcphdrChk, p) == LW_FALSE) {
            goto    __drop_input;
        }
        break;
...
INT  API_INetNpfInit (VOID)
{
    ...
    API_TShellHelpAdd("npfruleadd",    "add a rule into net packet filter.\n"
                                       ...
                                       "allow : allowed\n"
                                       "deny  : not allowed\n"
                                       "df    : allowed access this machine and do not forwarding\n");
...
#define __NPF_TSHELL_RADD_ARG_PORTE     8
    ...
    if (!lib_strcmp(ppcArgV[__NPF_TSHELL_RADD_ARG_ALLOW], "allow")) {
        iFlags = LWIP_NPF_RULE_FLAG_ALLOW;
    } else if (!lib_strcmp(ppcArgV[__NPF_TSHELL_RADD_ARG_ALLOW], "df")) {
        iFlags = LWIP_NPF_RULE_FLAG_DONT_FORWARD;
    } else {
        iFlags = 0;
    }
        ...
        for (plineTempNpfni  = _G_plineNpfHash[i];
            ...
            while (plineTemp) {
                ...
                *pstOft = bnprintf(pcBuffer, stTotalSize, *pstOft, 
                         ...
                         iSeqNum, "MAC",
                         (pnpfrm->NPFRM_iFlags & LWIP_NPF_RULE_FLAG_ALLOW) ? "YES" : "NO",
                         (pnpfrm->NPFRM_iFlags & LWIP_NPF_RULE_FLAG_DONT_FORWARD) ? "*" : "",
            ...
            while (plineTemp) {
                ...
                *pstOft = bnprintf(pcBuffer, stTotalSize, *pstOft, 
                         "%-5s %-6s %6d %-4s %-5s %-2s %-17s %-15s %-15s %-6s %-6s\n",
                         pnpfni->NPFNI_cName, (pnpfni->NPFNI_bAttached) ? "YES" : "NO",
                         iSeqNum, "IP",
                         (pnpfri->NPFRI_iFlags & LWIP_NPF_RULE_FLAG_ALLOW) ? "YES" : "NO", "N/A",
                         (pnpfri->NPFRI_iFlags & LWIP_NPF_RULE_FLAG_DONT_FORWARD) ? "*" : "",
            ...
            while (plineTemp) {
                ...
                *pstOft = bnprintf(pcBuffer, stTotalSize, *pstOft, 
                         "%-5s %-6s %6d %-4s %-5s %-2s %-17s %-15s %-15s %-6d %-6d\n",
                         pnpfni->NPFNI_cName, (pnpfni->NPFNI_bAttached) ? "YES" : "NO",
                         iSeqNum, "UDP",
                         (pnpfru->NPFRU_iFlags & LWIP_NPF_RULE_FLAG_ALLOW) ? "YES" : "NO", "N/A",
                         (pnpfru->NPFRU_iFlags & LWIP_NPF_RULE_FLAG_DONT_FORWARD) ? "*" : "",
                         ip4addr_ntoa_r(&ipaddrS, cIpBuffer1, INET_ADDRSTRLEN),
                         ip4addr_ntoa_r(&ipaddrE, cIpBuffer2, INET_ADDRSTRLEN),
            ...
            while (plineTemp) {
                ...
                *pstOft = bnprintf(pcBuffer, stTotalSize, *pstOft, 
                         ...
                         pnpfni->NPFNI_cName, (pnpfni->NPFNI_bAttached) ? "YES" : "NO",
                         iSeqNum, "TCP",
                         (pnpfrt->NPFRT_iFlags & LWIP_NPF_RULE_FLAG_ALLOW) ? "YES" : "NO", "N/A",
                         (pnpfrt->NPFRT_iFlags & LWIP_NPF_RULE_FLAG_DONT_FORWARD) ? "*" : "",
                         ip4addr_ntoa_r(&ipaddrS, cIpBuffer1, INET_ADDRSTRLEN),
                         ip4addr_ntoa_r(&ipaddrE, cIpBuffer2, INET_ADDRSTRLEN),
                                       ...
                                       off_t            oft)
{
    ...
    "NETIF ATTACH SEQNUM RULE ALLOW DF MAC               IPs             IPe             PORTs  PORTe\n";
