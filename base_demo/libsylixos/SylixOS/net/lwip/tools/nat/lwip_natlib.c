...
static INT  __natApInput (struct pbuf *p, struct netif *netifIn)
{
    ...
    if ((ucProto != IP_PROTO_ICMP) &&
        ...
        if (pnatmap) {
            ...
            if (__NAT_TRAFFIC_IS_ON()) {
                __natTrafficInput((ip4_addr_t *)&ipaddr, p->tot_len);   /*  统计输入 Traffic 信息       */
            }
        ...
        if (pnatcb) {                                                   /*  如果找到控制块              */
            ...
            if (__NAT_TRAFFIC_IS_ON()) {
                __natTrafficInput(&pnatcb->NAT_ipaddrLocal, p->tot_len);/*  统计输入 Traffic 信息       */
            }
...
static INT  __natApOutput (struct pbuf *p, struct netif  *pnetifIn, struct netif *netifOut)
{
    ...
    if (pnatcb) {
        ...
        if (__NAT_TRAFFIC_IS_ON()) {
            __natTrafficOutput((ip4_addr_t *)&(iphdr->src), p->tot_len);/*  统计输出 Traffic 信息       */
        }
