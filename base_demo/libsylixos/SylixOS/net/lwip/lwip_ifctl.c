...
static INT  __ifSubIoctlIf (INT  iCmd, PVOID  pvArg)
{
    ...
    case SIOCSIFSECREG:                                                 /*  设置网卡安全域              */
        if (geteuid()) {
            ...
            pnetif->sec_region = pifreq->ifr_secreg;
            iRet = ERROR_NONE;
        }
        break;
...
static INT  __ifSubIoctl6 (INT  iCmd, PVOID  pvArg)
{
    ...
    case SIOCDIFADDR6:                                                  /*  删除 IPv6 地址              */
        if (iSize == 0) {                                               /*  清除                        */
            LOCK_TCPIP_CORE();                                          /*  必须 lock 协议栈            */
            netif_ip6_addr_clear(pnetif, 0);
            UNLOCK_TCPIP_CORE();
            iRet = ERROR_NONE;

        } else if (iSize == 1) {                                        /*  删除一个                    */
            inet6_addr_to_ip6addr(&ip6addr, &pifr6addr->ifr6a_addr);
            LOCK_TCPIP_CORE();                                          /*  必须 lock 协议栈            */
