...
#define MIB2_NETIF(netif)   (&((netif)->mib2_counters))
    ...
    if (!ip6_addr_cmp(ip_2_ip6(&netif->ip6_gw), IP6_ADDR_ANY6)) {       /*  是否存在 IPv6 网关地址      */
        CHAR        cBuffer[64];

        printf("%9s inet6 default gateway: %s\n", "",
               ip6addr_ntoa_r(ip_2_ip6(&netif->ip6_addr[i]), cBuffer, sizeof(cBuffer)));
    }
#endif                                                                  /*  LWIP_IPV6                   */
    ...
    uiSecReg = netif_get_security(netif);
#else
    uiSecReg = 0;
#endif
    ...
    printf("%9s collisions:%u txqueue:%u tcpaf:%u tcpwnd:%u sr:%u\n", "",
           MIB2_NETIF(netif)->ifcollisions,
           (netdev) ? netdev_txq_length(netdev) : 0,
           netif_get_tcp_ack_freq(netif), netif_get_tcp_wnd(netif), uiSecReg);
#else
    printf("%9s collisions:%u noproto:%u tcpaf:%u tcpwnd:%u sr:%u\n", "",
           MIB2_NETIF(netif)->ifcollisions,
