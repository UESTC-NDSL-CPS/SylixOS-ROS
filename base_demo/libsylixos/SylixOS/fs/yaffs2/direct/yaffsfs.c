...
#ifdef SYLIXOS
/* 
 * SylixOS add chown support 
 */
static int yaffsfs_DoChOwn(struct yaffs_obj *obj, uid_t uid, gid_t gid)
{
    int result = -1;
...
int yaffs_chown_reldir(struct yaffs_obj *reldir, const YCHAR *path, uid_t uid, gid_t gid)
{
    struct yaffs_obj *obj = NULL;
    struct yaffs_obj *dir = NULL;
    int retVal = -1;
    ...
    if (!dir && notDir)
        ...
        retVal = yaffsfs_DoChOwn(obj, uid, gid);
...
int yaffs_chown(const YCHAR *path, uid_t uid, gid_t gid)
{
    return yaffs_chown_reldir(NULL, path, uid, gid);
}
#endif
