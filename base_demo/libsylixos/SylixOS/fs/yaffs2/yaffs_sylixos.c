...
/*********************************************************************************************************
  检测路径字串是否为直接指向设备
*********************************************************************************************************/
#define __YAFFS_IS_VOLUME(pcName)       (lib_rindex(pcName, PX_DIVIDER) == pcName)
/*********************************************************************************************************
  内部操作宏
*********************************************************************************************************/
                          ...
                          INT             iMode)
{
    ...
    if (pcName == LW_NULL) {                                            /*  无文件名                    */
        ...
        __PATH_FIX_ROOT(pcName);                                        /*  正常化 path                 */
        ...
        if (iError < ERROR_NONE) {                                      /*  产生错误                    */
            if (__PATH_IS_ROOT(pyaffile->YAFFIL_cName)) {               /*  是否为 yaffs 文件系统设备   */
                           ...
                           PCHAR           pcName)
{
    ...
    if (pcName == LW_NULL) {
        ...
        if (__PATH_IS_ROOT(pcName)) {                                   /*  根目录或者 yaffs 设备文件   */
__re_umount_vol:
        ...
        } else if (__YAFFS_IS_VOLUME(pcName)) {                         /*  yaffs 挂载的设备            */
            struct yaffs_dev  *pyaffsdev;
...
static INT  __yaffsRename (PLW_FD_ENTRY  pfdentry, PCHAR  pcNewName)
{
    ...
    REGISTER PCHAR          pcNewPath = &cNewPath[0];
             PLW_FD_ENTRY   pfdentryNew;
             PLW_FD_NODE    pfdnode   = (PLW_FD_NODE)pfdentry->FDENTRY_pfdnode;
             PYAFFS_FILE    pyaffile  = (PYAFFS_FILE)pfdnode->FDNODE_pvFile;
             PYAFFS_FSLIB   pyaffsNew;

    if (__PATH_IS_ROOT(pyaffile->YAFFIL_cName)) {                       /*  检查是否为设备文件          */
    ...
    if (pfdnode->FDNODE_ulRef > 1) {
        _ErrorHandle(EBUSY);                                            /*  源文件有其他打开实例        */
        return  (PX_ERROR);
    }
    ...
    if ((pyaffile->YAFFIL_iFileType == __YAFFS_FILE_TYPE_NODE) ||
        ...
        pfdentryNew = iosFdIsBusy(pfdentry->FDENTRY_pdevhdrHdr, cNewPath);
        if (pfdentryNew) {
            if ((PLW_FD_NODE)pfdentryNew->FDENTRY_pfdnode != pfdnode) { /*  目标文件正在使用            */
                __YAFFS_OPUNLOCK();
                _ErrorHandle(EBUSY);
                return  (PX_ERROR);
...
** 调用模块:
*********************************************************************************************************/
static INT  __yaffsAllocate (PLW_FD_ENTRY  pfdentry, off_t *oftArg)
{
    INT                iError;
    PLW_FD_NODE        pfdnode  = (PLW_FD_NODE)pfdentry->FDENTRY_pfdnode;
    PYAFFS_FILE        pyaffile = (PYAFFS_FILE)pfdnode->FDNODE_pvFile;
    ...
    __YAFFS_OPLOCK();
    if (oftNewSize > pfdnode->FDNODE_oftSize) {
        iError = yaffs_ftruncate(pyaffile->YAFFIL_iFd, oftNewSize);
        yaffs_fstat(pyaffile->YAFFIL_iFd, &yafstat);
        pfdnode->FDNODE_oftSize = yafstat.st_size;                      /*  更新文件大小                */
    } else {
        iError = ERROR_NONE;
...
static INT  __yaffsStatGet (PLW_FD_ENTRY  pfdentry, struct stat *pstat)
{
    ...
    if (iError == ERROR_NONE) {
         ...
         *  格式化时使用.
         */
        if (__PATH_IS_ROOT(pyaffile->YAFFIL_cName)) {                   /* yaffs root                   */
...
static INT  __yaffsLStatGet (PYAFFS_FSLIB  pyaffs, PCHAR  pcName, struct stat *pstat)
{
    ...
    if (iError == ERROR_NONE) {
         ...
         *  格式化时使用.
         */
        if (__PATH_IS_ROOT(pcName)) {                                   /* yaffs root                   */
            pstat->st_mode |= S_IFDIR;
        }
...
** 调用模块:
*********************************************************************************************************/
static INT  __yaffsChown (PLW_FD_ENTRY  pfdentry, LW_IO_USR  *pusr)
{
    INT             iError   = PX_ERROR;
    PLW_FD_NODE     pfdnode  = (PLW_FD_NODE)pfdentry->FDENTRY_pfdnode;
    PYAFFS_FILE     pyaffile = (PYAFFS_FILE)pfdnode->FDNODE_pvFile;
    ...
    if ((pyaffile->YAFFIL_iFileType == __YAFFS_FILE_TYPE_NODE) ||
        (pyaffile->YAFFIL_iFileType == __YAFFS_FILE_TYPE_DIR)) {        /*  open 创建的普通文件或目录   */
        iError = yaffs_chown(pyaffile->YAFFIL_cName, pusr->IOU_uid, pusr->IOU_gid);
    }
    __YAFFS_OPUNLOCK();
...
static INT  __yaffsReadDir (PLW_FD_ENTRY  pfdentry, DIR  *dir)
{
    ...
    if (__PATH_IS_ROOT(pyaffile->YAFFIL_cName)) {
        /*
                          ...
                          LONG          lArg)
{
    ...
    case FIOALLOCATE:                                                   /*  文件预分配空间              */
        return  (__yaffsAllocate(pfdentry, (off_t *)lArg));
    ...
    case FIOCHMOD:                                                      /*  改变文件访问权限            */
        return  (__yaffsChmod(pfdentry, (INT)lArg));
    ...
    case FIOCHOWN:                                                      /*  修改文件所属关系            */
        return  (__yaffsChown(pfdentry, (LW_IO_USR *)lArg));
