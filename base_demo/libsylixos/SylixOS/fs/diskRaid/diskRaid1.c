...
#define RAID_BLK_READ(pdiskr, disk, buf, sect, num, remain) \
        pdiskr->DISKR_ppblkdDisk[disk]->BLKD_pfuncBlkRd(pdiskr->DISKR_ppblkdDisk[disk], buf, sect, num, remain)
                          ...
                          ULONG              ulRemainCount)
{
    ...
    for (i = 0; i < pdiskr->DISKR_uiNDisks; i++) {
        if (RAID_BLK_READ(pdiskr, i, pvBuffer, ulStartSector, ulSectorCount, ulRemainCount)) {
