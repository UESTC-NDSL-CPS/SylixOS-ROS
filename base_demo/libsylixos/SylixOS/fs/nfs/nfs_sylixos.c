                          ...
                          struct stat   *pstat)
{
    ...
    if (*pcPtr) {                                                       /*  没有查找到目标              */
        ...
        *pbIsDir = bFindIsDir;
        ...
        if (!bFindIsDir && iFlag & O_TRUNC) {                           /*  需要将文件清零              */
    ...
    if (pcName == LW_NULL) {                                            /*  无文件名                    */
        ...
        if (iFlags & O_CREAT) {
            ...
            if (__PATH_IS_ROOT(pcName)) {
                ...
                if (__fsCheckFileName(pcName)) {                        /*  检查文件名合法性            */
                    _ErrorHandle(ENOENT);
                    return  (PX_ERROR);
                }
            }
        ...
        if (iError < 0) {
            if (__PATH_IS_ROOT(pnfsfile->NFSFIL_cName)) {
                         ...
                         PCHAR           pcName)
{
    ...
    if (__PATH_IS_ROOT(pcName)) {                                       /*  根目录或者 nfs 设备文件     */
...
static INT  __nfsRename (PLW_FD_ENTRY  pfdentry, PCHAR  pcNewName)
{
    ...
    if (__PATH_IS_ROOT(pnfsfile->NFSFIL_cName)) {                       /*  检查是否为设备文件          */
        _ErrorHandle(ENOSYS);                                           /*  不支持设备重命名            */
        return  (PX_ERROR);
    }
    if ((pcNewName == LW_NULL) || __PATH_IS_ROOT(pcNewName)) {
        _ErrorHandle(EINVAL);
        return  (PX_ERROR);
    }
...
** 调用模块:
*********************************************************************************************************/
static INT  __nfsAllocate (PLW_FD_ENTRY  pfdentry, off_t *oftArg)
{
    enum clnt_stat  clntstat;
    ...
    if (oftNewSize > pnfsfile->NFSFIL_stat.st_size) {
        attrargs.object = pnfsfile->NFSFIL_handle;
        attrargs.new_attributes.size.set_it = LW_TRUE;
        attrargs.new_attributes.size.set_size3_u.size = oftNewSize;
        ...
        pnfsfile->NFSFIL_stat.st_size = oftNewSize;
                        ...
                        LONG          lArg)
{
    ...
    case FIOALLOCATE:                                                   /*  文件空间预分配              */
        return  (__nfsAllocate(pfdentry, (off_t *)lArg));
