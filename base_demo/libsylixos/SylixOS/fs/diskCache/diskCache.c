...
/*********************************************************************************************************
  磁盘 cache 参数表
*********************************************************************************************************/
static CHAR  _G_cDiskCacheWTParam[64] = { 0 };                          /*  写穿透参数配置              */
/*********************************************************************************************************
  函数声明
*********************************************************************************************************/
...
/*********************************************************************************************************
** 函数名称: API_DiskCacheParam
** 功能描述: 记录磁盘缓冲启动参数
** 输　入  : pcParam            DISKCACHE 参数
** 输　出  : NONE
** 全局变量: 
** 调用模块: 
*********************************************************************************************************/
LW_API
VOID  API_DiskCacheParam (CPCHAR pcParam)
{
    INT  i;

    if (lib_strncmp(pcParam, "hsdcwt=", 7) == 0) {
        lib_strlcpy(_G_cDiskCacheWTParam, &pcParam[7], sizeof(_G_cDiskCacheWTParam));
        for (i = 0; (i < sizeof(_G_cDiskCacheWTParam)) && (_G_cDiskCacheWTParam[i] != 0); i++) {
            if (_G_cDiskCacheWTParam[i] == ';') {
                _G_cDiskCacheWTParam[i] =  PX_EOS;
            }
        }
    }
}
                               ...
                               PLW_BLK_DEV         *ppblkDiskCache)
{
    ...
    pdiskcDiskCache->DISKC_iCacheOpt = pdcattrl->DCATTR_iBurstOpt;
    
    if (pdcattrl->DCATTR_iBurstOpt & LW_DCATTR_BOPT_HIGH_SPEED) {
        if (pdcattrl->DCATTR_stDirectThreshold == 0ul) {
            pdiskcDiskCache->DISKC_iDirectThreshold = pdcattrl->DCATTR_iMaxRBurstSector;
        } else {
            pdiskcDiskCache->DISKC_iDirectThreshold = pdcattrl->DCATTR_stDirectThreshold
                                                    / ulBytesPerSector;
        }
    }
     ...
     * 检查是否需要写穿透
     */
    if (*_G_cDiskCacheWTParam && *pblkdDisk->BLKD_pcName) {
        for (i = 0; i < sizeof(_G_cDiskCacheWTParam); i++) {
            if (_G_cDiskCacheWTParam[i] != '\0') {
                if (lib_strstr(pblkdDisk->BLKD_pcName, &_G_cDiskCacheWTParam[i])) {
                    break;
                }
                i += lib_strlen(&_G_cDiskCacheWTParam[i]);
            }
        }
        if (i < sizeof(_G_cDiskCacheWTParam)) {
