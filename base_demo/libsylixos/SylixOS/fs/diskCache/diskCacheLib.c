...
/*********************************************************************************************************
** 函数名称: __diskCacheNodeRemove
** 功能描述: 删除指定范围的 cache 节点
** 输　入  : pdiskcDiskCache    磁盘 CACHE 控制块
**           blkrange           扇区范围
** 输　出  : NONE
** 全局变量:
                                       ...
                                       PLW_BLK_RANGE      blkrange)
{
    ...
    for (i = 0; i < pdiskcDiskCache->DISKC_ulNCacheNode; i++) {
        ...
        __diskCacheHashRemove(pdiskcDiskCache, pdiskn);                 /*  移除有效性                  */

        if (_list_ring_get_next(&pdiskn->DISKN_ringLru) !=
            pdiskcDiskCache->DISKC_pringLruHeader) {                    /*  更新 LRU 中的位置, 方便分配 */
...
/*********************************************************************************************************
** 函数名称: __diskCacheNodeToBuffer
** 功能描述: 将扇区范围内缓存的脏 cache 同步到 buffer 内 (不更新 LRU)
** 输　入  : pdiskcDiskCache    磁盘 CACHE 控制块
**           pcBuffer           缓冲区
**           ulStartSector      起始扇区
...
** 输　出  : NONE
** 全局变量:
** 调用模块: 1: 如果 cache 存在且为脏, 则从 cache 拷贝, 否则需要查看管线待写数据是否存在.
*********************************************************************************************************/
...
/*********************************************************************************************************
** 函数名称: __diskCacheNodeFromBuffer
** 功能描述: 使用数据更新相应的 cache (不更新 LRU)
** 输　入  : pdiskcDiskCache    磁盘 CACHE 控制块
**           pcBuffer           缓冲区
**           ulStartSector      起始扇区
**           ulSectorCount      连续扇区数量
    ...
    for (i = 0; i < ulSectorCount; i++) {
        ...
        if (pdiskn) {
            ...
            if (__LW_DISKCACHE_IS_DIRTY(pdiskn)) {
                __LW_DISKCACHE_CLR_DIRTY(pdiskn);                       /*  清除脏位标志                */
                pdiskcDiskCache->DISKC_ulDirtyCounter--;
            }
            ...
            if (!bCallback) {
                bCallback = LW_TRUE;
                __LW_DISKCACHE_FS_CALLBACK(pdiskcDiskCache,
                                           pdiskn, ulSectorCount - i);  /*  执行文件系统回调            */
            }
        }
                                      ...
                                      ULONG              ulSectorCount)
{
    ...
    do {
        ...
        if (iError < 0) {
            ...
            __diskCacheMemcpy(pvBuffer, pcData, stBytes);               /*  拷贝到文件系统              */

            __diskCacheWpPutBuffer(&pdiskcDiskCache->DISKC_wpWrite, pcData);

            __diskCacheNodeToBuffer(pdiskcDiskCache, pvBuffer,
                                    ulStartSector, ulNSector);          /*  脏 cache 更新已读缓存       */
        }
                                       ...
                                       UINT64             u64FsKey)
{
    ...
    do {
        ...
        __diskCacheMemcpy(pcData, pvBuffer, stBytes);                   /*  拷贝到写入缓存              */

        iError = __diskCacheWpWrite(pdiskcDiskCache, pdiskcDiskCache->DISKC_pblkdDisk,
                                    pcData, ulStartSector, ulNSector);  /*  连续写入扇区数据            */
        if (iError < 0) {
            __diskCacheWpPutBuffer(&pdiskcDiskCache->DISKC_wpWrite, pcData);
            break;
            ...
            __diskCacheNodeFromBuffer(pdiskcDiskCache, pvBuffer,
                                      ulStartSector, ulNSector);        /*  更新相关 cache              */
        }
                      ...
                      ULONG              ulRemianCount)
{
    ...
    if (__LW_DISKCACHE_NEED_DIRECT(pdiskcDiskCache, ulSectorCount)) {   /*  直接读取                    */
        ...
        if (ulRemianCount &&
            ulRemianCount < pdiskcDiskCache->DISKC_iMaxRBurstSector) {  /*  使用推荐的连续读长度        */
        ...
        if (iMaxBurst) {
            pdiskcDiskCache->DISKC_iMaxRBurstSector = iMaxBurst;        /*  恢复设置                    */
        }
    }
                       ...
                       UINT64             u64FsKey)
{
    ...
    if (__LW_DISKCACHE_IS_WRITETHROUGH(pdiskcDiskCache) ||
        __LW_DISKCACHE_NEED_DIRECT(pdiskcDiskCache, ulSectorCount)) {   /*  写穿透模式                  */
...
INT  __diskCacheIoctl (PLW_DISKCACHE_CB   pdiskcDiskCache, INT  iCmd, LONG  lArg)
{
    ...
    case FIOSYNCMETA:                                                   /*  SYNCMETA 需要回写区域       */
        iError    = ERROR_NONE;
        bMutex    = LW_TRUE;
        pblkrange = (PLW_BLK_RANGE)lArg;
        if (!__LW_DISKCACHE_IS_WRITETHROUGH(pdiskcDiskCache)) {         /*  非写穿透模式                */
