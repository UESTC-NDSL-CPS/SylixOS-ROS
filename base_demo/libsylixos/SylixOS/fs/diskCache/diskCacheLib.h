...
/*********************************************************************************************************
  ...
  注意, 当读写管线使用 cached 缓存时:

  1: DISKCWP_iPipeline 为 0 时. DISKCWP_pvRBurstBuffer 与 DISKCWP_pvWBurstBuffer 为同一块内存, 所以
     如果设备驱动是 DMA 操作, 则在读操作之前需要 cache flush DMA 传输完成后, 需要 cache invalid.

  2: DISKCWP_iPipeline 大于 0 时, DISKCWP_pvRBurstBuffer 只负责读, 所以不会存在内存 dirty 所以如果
     设备驱动是 DMA 操作, 则只需要在 DMA 操作完成后进行 cache invalid 操作即可.

  需要说明: cache invalid 操作必须放在 DMA 操作之后, 因为有些处理器在 DMA 操作过程中会随机预取, 如果
            将 cache invalid 操作放在 DMA 操作之前, 会造成 DMA 操作后内存与 DMA 数据不一致的情况.
*********************************************************************************************************/
typedef struct {
...
typedef struct {
    ...
    INT                     DISKC_iDirectThreshold;                     /*  直接读写阀值                */
