                         ...
                         INT             iMode)
{
    ...
    if (pramn) {
        if (!S_ISLNK(pramn->RAMN_mode)) {
            ...
            } else if (S_ISDIR(pramn->RAMN_mode) && ((iFlags & O_ACCMODE) != O_RDONLY)) {
                __RAMFS_VOL_UNLOCK(pramfs);
                _ErrorHandle(EISDIR);
                return  (PX_ERROR);
...
** 调用模块:
*********************************************************************************************************/
static INT  __ramFsAllocate (PLW_FD_ENTRY  pfdentry, off_t *oftArg)
{
    PLW_FD_NODE   pfdnode = (PLW_FD_NODE)pfdentry->FDENTRY_pfdnode;
    PRAM_NODE     pramn   = (PRAM_NODE)pfdnode->FDNODE_pvFile;
    PRAM_VOLUME   pramfs  = (PRAM_VOLUME)pfdnode->FDNODE_pvFsExtern;
    ...
    if (oftNewSize > pramn->RAMN_stSize) {
        __ram_increase(pramn, (size_t)oftNewSize);
    }
...
static INT  __ramFsReadDir (PLW_FD_ENTRY  pfdentry, DIR  *dir)
{
    ...
    for ((plineTemp  = plineHeader), (i = 0);
        ...
        if (DIR_RESV_DATA_PV0(dir) == plineTemp) {                      /*  当前节点之前有节点被删除    */
            dir->dir_pos = i;
            break;
        }
    }
    ...
    if (plineTemp == LW_NULL) {
        ...
        dir->dir_pos++;
        DIR_RESV_DATA_PV0(dir) = _list_line_get_next(plineTemp);        /*  保存下一个节点位置          */
                          ...
                          LONG          lArg)
{
    ...
    case FIOALLOCATE:                                                   /*  预分配文件空间              */
        return  (__ramFsAllocate(pfdentry, (off_t *)lArg));
