...
/*********************************************************************************************************
  分区类型 SylixOS TpsFs 分区
*********************************************************************************************************/
static const UINT8  _G_ucGptPartTypeTpsFs[16] = {
    0x9a, 0x73, 0x2a, 0xc1, 0x1f, 0xf8, 0xd2, 0x66,
    0xdf, 0x4b, 0x00, 0xa0, 0xc9, 0x3e, 0xc9, 0xd5,
};
/*********************************************************************************************************
  FAT 分区类型判断相关宏
*********************************************************************************************************/
                              ...
                              CPCHAR      cpcName)
{
    ...
    } else if (ucPartType == LW_DISK_PART_TYPE_TPS) {
        lib_memcpy(pgpt->GPT_entry[iIndex].ENT_ucTypeUuid,
                   _G_ucGptPartTypeTpsFs, sizeof(_G_ucGptPartTypeTpsFs));
                                                                        /*  TpsFs分区                   */
    } else {
        lib_memcpy(pgpt->GPT_entry[iIndex].ENT_ucTypeUuid,
...
** 调用模块:
*********************************************************************************************************/
static BOOL  __gptIsFatPartition (GPT_TABLE  *pgpt, UINT64  ui64LbaNum, UINT64  ui64LbaCnt)
{
    BOOL   bRtn = LW_FALSE;
    UINT8 *pucSecBuff;

    pucSecBuff = (UINT8 *)__SHEAP_ALLOC(pgpt->GPT_uiSecSize);
    if (!pucSecBuff) {
        return  (LW_FALSE);
    }
     ...
     */
    if (pgpt->GPT_pblkd) {
        if (pgpt->GPT_pblkd->BLKD_pfuncBlkRd(pgpt->GPT_pblkd,
                                             pucSecBuff,
                                             ui64LbaNum,
                                             1, 1) != ERROR_NONE) {
            goto  __error;
    ...
    } else if (pgpt->GPT_iBlkFd >= 0) {
        if (pread(pgpt->GPT_iBlkFd,
                  pucSecBuff,
                  pgpt->GPT_uiSecSize,
                  (ui64LbaNum * pgpt->GPT_uiSecSize)) != pgpt->GPT_uiSecSize) {
            goto  __error;
        }
     ...
     *  检测分区特征
     */
    if ((pucSecBuff[BS_55AA] != 0x55) || (pucSecBuff[BS_55AA + 1] != 0xAA)) {
        goto  __error;
    }

    if (pucSecBuff[BS_JmpBoot] == 0xE9 ||
        (pucSecBuff[BS_JmpBoot] == 0xEB &&
         pucSecBuff[BS_JmpBoot + 2] == 0x90)) {
        if (!lib_memcmp(pucSecBuff + BS_FilSysType, BS_FilSysStr, 3)) {
            bRtn = LW_TRUE;
            goto  __error;
        }

        if (!lib_memcmp(pucSecBuff + BS_FilSysType32, BS_FilSysStr, 3)) {
            bRtn = LW_TRUE;
            goto  __error;
        }
    }

    if (!lib_memcmp(pucSecBuff + BS_JmpBoot, "\xEB\x76\x90" "EXFAT   ", 11)) {
        bRtn = LW_TRUE;
    }

__error:
    __SHEAP_FREE(pucSecBuff);
                      ...
                      UINT8      *pucPartType)
{
    ...
    *pucPartType = pgpt->GPT_ucBlock[GPT_PART_TYPE_OFF + iIndex];
    ...
    /*
     *  兼容 Windows 分区(未写分区标识位，将 Windows 数据类型分区统一按 FAT32 类型处理)
     */
    if ((*pucPartType) == 0) {
        if (lib_memcmp(pgpt->GPT_entry[iIndex].ENT_ucTypeUuid,
                       _G_ucGptPartTypeEfi, sizeof(_G_ucGptPartTypeEfi)) == 0) {
            *pucPartType = LW_DISK_PART_TYPE_EFI;

        } else if (lib_memcmp(pgpt->GPT_entry[iIndex].ENT_ucTypeUuid,
                              _G_ucGptPartTypeTpsFs, sizeof(_G_ucGptPartTypeTpsFs)) == 0) {
            *pucPartType = LW_DISK_PART_TYPE_TPS;

        } else if (lib_memcmp(pgpt->GPT_entry[iIndex].ENT_ucTypeUuid,
                              _G_ucGptPartTypeData, sizeof(_G_ucGptPartTypeData)) == 0) {
            if (__gptIsFatPartition(pgpt, *pui64LbaNum, *pui64LbaCnt)) {
                *pucPartType = LW_DISK_PART_TYPE_WIN95_FAT32;
            }
        }
    }
...
INT  API_GptPartitionLoad (INT  iBlkFd, GPT_TABLE  *pgpt)
{
    ...
    pgpt->GPT_iBlkFd = iBlkFd;
    pgpt->GPT_pblkd  = LW_NULL;
...
INT  API_GptPartitionBlkLoad (PLW_BLK_DEV  pblkd, GPT_TABLE  *pgpt)
{
    ...
    if (pblkd->BLKD_pfuncBlkRd(pblkd, pgpt->GPT_ucBlock,
                               GPT_HEAD_LBA,
                               GPT_HEAD_LBA_CNT,
                               GPT_HEAD_LBA_CNT) != ERROR_NONE) {       /*  读取 GPT 分区表头           */
        _ErrorHandle(EIO);
        return  (PX_ERROR);
    }
    ...
    if (pblkd->BLKD_pfuncBlkRd(pblkd, (UINT8 *)pgpt->GPT_entry,
                               GPT_ENT_LBA,
                               GPT_ENT_LBA_CNT,
                               GPT_ENT_LBA_CNT) != ERROR_NONE) {        /*  读取分区表项                */
        _ErrorHandle(EIO);
        return  (PX_ERROR);
    }
    ...
    if (__gptVerify(pgpt) != ERROR_NONE) {
        ...
        if (pblkd->BLKD_pfuncBlkWrt(pblkd, pgpt->GPT_ucBlock,           /*  恢复 GPT 分区表头           */
        ...
        if (pblkd->BLKD_pfuncBlkWrt(pblkd, (UINT8 *)pgpt->GPT_entry,    /*  恢复分区表项                */
    ...
    pgpt->GPT_iBlkFd = PX_ERROR;
    pgpt->GPT_pblkd  = pblkd;
