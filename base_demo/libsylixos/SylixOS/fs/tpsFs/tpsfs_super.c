...
** 调用模块:
*********************************************************************************************************/
errno_t  tpsFsMount (PTPS_DEV pdev, UINT uiFlags, INT iTransMode, PTPS_SUPER_BLOCK *ppsb)
{
    ...
    *ppsb = LW_NULL;
    ...
    if (pdev->DEV_ReadSector(pdev, pucSectorBuf,
                             TPS_SUPER_BLOCK_SECTOR, 1, 1) != 0) {      /* 读取超级块                   */
    ...
    if (psb->SB_uiMagic != TPS_MAGIC_SUPER_BLOCK1 &&
        ...
        _DebugHandle(__PRINTMESSAGE_LEVEL, "Magic number error, File system invalid.\r\n");
        TPS_FREE(pucSectorBuf);
        TPS_FREE(psb);
        return  (ERROR_NONE);
    }
    ...
    psb->SB_pbp             = LW_NULL;
    psb->SB_pinodeSpaceMng  = LW_NULL;
    psb->SB_iTransMode      = iTransMode;
...
errno_t  tpsFsFormat (PTPS_DEV pdev, UINT uiBlkSize)
{
    ...
    psb->SB_pbp                 = LW_NULL;
    psb->SB_pinodeSpaceMng      = LW_NULL;
    psb->SB_iTransMode          = TPS_TM_SINGLE;
...
{
    if (psb->SB_dev->DEV_ReadSector(psb->SB_dev, psb->SB_pucSectorBuf, 0, 1, 1) != 0) {
...
** 调用模块:
*********************************************************************************************************/
errno_t tpsFsSetTransMode(PTPS_SUPER_BLOCK psb, INT iTransMode)
{
    psb->SB_iTransMode = iTransMode;
