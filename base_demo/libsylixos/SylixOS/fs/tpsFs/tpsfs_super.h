...
/*********************************************************************************************************
  事务合并模式
  single 模式为常态模式，即一次事务一次提交。
  volume 模式为事务合并模式，对小型事务进行自动合并，减少事务提交次数，提高文件系统速度。该模式存在一定的
  缺陷，可能会损害近期操作过的文件和目录数据的安全性，一般是发生在磁盘 IO 错误或磁盘空间满导致事务回滚的
  情况下。
  file 模式介于 single 模式和 volume 模式之间，即对文件数据操作进行事务合并，不会影响目录安全性，但对文件
  数据安全性的影响比 single 模式大。
*********************************************************************************************************/
#define TPS_TM_SINGLE               0
#define TPS_TM_MERGE_FILE           1
#define TPS_TM_MERGE_VOLUME         2
