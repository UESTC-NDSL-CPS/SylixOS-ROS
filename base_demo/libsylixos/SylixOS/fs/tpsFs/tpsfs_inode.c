...
static BOOL  __tpsFsInodeSerial (PTPS_INODE pinode, PUCHAR pucBuff, UINT uiSize)
{
    ...
    TPS_CPU_TO_LE64(pucPos, pinode->IND_ui64TransNum);
...
static BOOL  __tpsFsInodeUnserial (PTPS_INODE pinode, PUCHAR pucBuff, UINT uiSize)
{
    ...
    TPS_LE64_TO_CPU(pucPos, pinode->IND_ui64TransNum);
...
TPS_RESULT  tpsFsCreateInode (PTPS_TRANS ptrans, PTPS_SUPER_BLOCK psb, TPS_INUM inum, INT iMode)
{
    ...
    pinode->IND_ui64TransNum    = (ptrans ? ptrans->TRANS_ui64SerialNum : 0);
...
** 调用模块:
*********************************************************************************************************/
static TPS_SIZE_T  __tpsFsInodeExtend (PTPS_TRANS ptrans, PTPS_INODE pinode, TPS_SIZE_T size)
{
    TPS_IBLK            blkEnd;
...
#if LW_CFG_DWATCH_EN > 0
    pinode->IND_uiEvent   |= DWATCH_MODIFY;
#endif

    return  (tpsFsFlushInodeHead(ptrans, pinode));
}
/*********************************************************************************************************
** 函数名称: tpsFsTruncInode
** 功能描述: 截断文件
** 输　入  : ptrans           事务
**           pinode           inode指针
**           size             截断后的长度
...
TPS_RESULT  tpsFsTruncInode (PTPS_TRANS ptrans, PTPS_INODE pinode, TPS_SIZE_T size)
{
    ...
    if (size > pinode->IND_szData) {
        return  (__tpsFsInodeExtend(ptrans, pinode, size));
    }
                            ...
                            TPS_SIZE_T szLen, BOOL bTransData)
{
    ...
    while (szLen > 0) {
        ...
        szRmnLen  = (blkPscCnt << psb->SB_uiBlkShift) - offBlk;
        szReadLen = min(szLen, szRmnLen);
...
** 调用模块:
*********************************************************************************************************/
TPS_RESULT  tpsFsInodeBuffInvalid (PTPS_TRANS ptrans, PTPS_INODE pinode, UINT64 ui64SecStart, UINT uiSecCnt)
{
    ...
    if (max(ui64BufStart, ui64SecStart) <
        ...
        if (tpsFsDevBufRead(psb, pinode->IND_inum, 0, pinode->IND_pucBuff,
                            TPS_INODE_MAX_HEADSIZE, psb->SB_uiBlkSize) != TPS_ERR_NONE) {
            return  (TPS_ERR_INODE_SYNC);
        }
    ...
    if (LW_NULL != pinode->IND_pinodeHash) {                            /* 恢复目录哈希节点             */
        return  (tpsFsInodeBuffInvalid(ptrans, pinode->IND_pinodeHash, ui64SecStart, uiSecCnt));
    }

    /*
     * 事务合并模式可能导致正在打开的文件被回滚，需要设置文件为不可操作状态。
     */
    if (psb->SB_iTransMode == TPS_TM_MERGE_VOLUME) {
        if (pinode->IND_ui64TransNum == ptrans->TRANS_ui64SerialNum) {
            pinode->IND_uiOpenCnt = 0;
        }
    }
