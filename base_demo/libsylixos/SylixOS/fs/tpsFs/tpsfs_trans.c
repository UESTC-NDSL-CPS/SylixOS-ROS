...
TPS_RESULT  tpsFsBtreeTransInit (PTPS_SUPER_BLOCK psb)
{
    ...
    ptrans->TRANS_bMergeMode        = LW_FALSE;
...
{
    BOOL bMergeMode = (psb->SB_iTransMode == TPS_TM_MERGE_VOLUME ? LW_TRUE : LW_FALSE);

    return tpsFsTransAllocAndInitEx(psb, bMergeMode);
}
/*********************************************************************************************************
** 函数名称: tpsFsTransAllocAndInit
** 功能描述: 分配并初始化事务
...
** 调用模块:
*********************************************************************************************************/
PTPS_TRANS  tpsFsTransAllocAndInitEx (PTPS_SUPER_BLOCK psb, BOOL bMassMode)
{
    ...
    if ((bMassMode != ptrans->TRANS_bMergeMode) &&
        (ptrans->TRANS_iStatus == TPS_TRANS_STATUS_INIT)) {
        if (tpsFsTransCommitAndFreeEx(ptrans, LW_TRUE) != TPS_ERR_NONE) {
            return  (LW_NULL);
        }
    }

    if (ptrans->TRANS_bMergeMode && (ptrans->TRANS_iStatus == TPS_TRANS_STATUS_INIT)) {
        return  (ptrans);
    }
    ...
    ptrans->TRANS_pdata->TD_uiSecAreaCnt = 0;
    ptrans->TRANS_bMergeMode             = bMassMode;
...
TPS_RESULT  tpsFsTransRollBackAndFree (PTPS_TRANS ptrans)
{
    ...
    for (i = 0; i < ptrans->TRANS_pdata->TD_uiSecAreaCnt; i++) {        /* 无效inode中的事务相关缓冲块  */
        ...
        while (pinode) {
            if (tpsFsInodeBuffInvalid(ptrans, pinode, ui64SecStart, uiSecCnt) != TPS_ERR_NONE) {
                psb->SB_uiFlags |= TPS_TRANS_FAULT;
                return  (TPS_ERR_TRANS_ROLLBK_FAULT);
            }
...
{
    return  tpsFsTransCommitAndFreeEx(ptrans, LW_FALSE);
}
/*********************************************************************************************************
** 函数名称: tpsFsTransCommitAndFreeEx
** 功能描述: 提交并释放事务
** 输　入  : ptrans           事务
**           bForce           强制提交事务
** 输　出  : ERROR
...
** 调用模块:
*********************************************************************************************************/
TPS_RESULT  tpsFsTransCommitAndFreeEx (PTPS_TRANS ptrans, BOOL bForce)
{
    INT     i = 0;
    INT     j;
    ...
    if (ptrans->TRANS_bMergeMode && !bForce) {                          /* 事务合并模式延迟提交事务     */
        if (!tpsFsTransTrigerChk(ptrans)) {
            return  (TPS_ERR_NONE);
        }
    }
    ...
    if (ptranssb->TSB_uiTransSecOff >= psb->SB_uiSectorSize) {          /* 事务计数自增，内部指针调整   */
        ptranssb->TSB_ui64TransCurSec++;
        ptranssb->TSB_uiTransSecOff = 0;
...
TPS_RESULT  tspFsCheckTrans (PTPS_SUPER_BLOCK psb)
{
    ...
    if (transMid.TRANS_iStatus == TPS_TRANS_STATUS_COMPLETE) {          /* 事务为已完成态无需重做       */
        ...
        return (tpsFsTransCommitAndFreeEx(ptranssb->TSB_ptrans, LW_TRUE));
    }
...
** 调用模块:
*********************************************************************************************************/
TPS_RESULT tpsFsTransFlush (PTPS_SUPER_BLOCK psb, BOOL bForce)
{
    PTPS_TRANS       ptrans;

    ptrans = psb->SB_ptranssb->TSB_ptrans;
    if (ptrans->TRANS_bMergeMode &&
        (ptrans->TRANS_iStatus == TPS_TRANS_STATUS_INIT) &&
        (ptrans->TRANS_pdata->TD_uiSecAreaCnt > 0)) {
        if (bForce || ((TPS_UTC_TIME() - ptrans->TRANS_ui64Time) >= 1)) {
            return  (tpsFsTransCommitAndFreeEx(ptrans, LW_TRUE));
        }
    }
