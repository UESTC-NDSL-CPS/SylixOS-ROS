...
typedef struct tps_inode {
    ...
    UINT64              IND_ui64TransNum;                               /* 创建文件的事务序号           */
    ...
    UINT                IND_uiEvent;                                    /* 需要上报的 dwatch event      */
#endif
} TPS_INODE;
...
TPS_RESULT tpsFsInodeAllocBlk(PTPS_TRANS ptrans, PTPS_SUPER_BLOCK psb,
                              ...
                              TPS_IBLK *blkAllocStart, TPS_IBLK *blkAllocCnt);
                                                                        /* 无效inode事务相关缓冲区      */
TPS_RESULT tpsFsInodeBuffInvalid(PTPS_TRANS ptrans, PTPS_INODE pinode,
                                 UINT64 ui64SecStart, UINT uiSecCnt);
