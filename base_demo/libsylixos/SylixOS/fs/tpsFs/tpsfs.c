                    ...
                    PTPS_INODE          *ppinode)
{
    ...
    *ppinode = LW_NULL;
    ...
    if (!S_ISLNK(pentry->ENTRY_pinode->IND_iMode) && (*pcRemain == 0)) {
        ...
        if (S_ISDIR(pentry->ENTRY_pinode->IND_iMode) && ((iFlags & O_ACCMODE) != O_RDONLY)) {
            tpsFsEntryFree(pentry);
            return  (EISDIR);
        }
    }
...
entry_ok:
    *ppinode = tpsFsOpenInode(psb, pentry->ENTRY_inum);
    ...
    if (pentryNew) {
        (*ppinode)->IND_uiEvent |= DWATCH_CREATE;
    }
#endif
                     ...
                     TPS_SIZE_T *pszRet)
{
    ...
    *pszRet = 0;
    ...
    if ((pinode->IND_psb->SB_iTransMode != TPS_TM_SINGLE)||
        (pinode->IND_iFlag & O_LARGEFILE)) {
        bMergeMode = LW_TRUE;
    }

    while (szLen > 0) {                                                 /* 避免事务过大可能将写操作分解 */
        bNeedCommit = LW_FALSE;
        ptrans = tpsFsTransAllocAndInitEx(pinode->IND_psb, bMergeMode); /* 分配事务                     */
        if (ptrans == LW_NULL) {
            return  (ENOMEM);
        }
        ...
        if (tpsFsTransCommitAndFreeEx(ptrans, bNeedCommit) != TPS_ERR_NONE) {          /* 提交事务                     */
            tpsFsTransRollBackAndFree(ptrans);
            return  (EIO);
        }
...
errno_t  tpsFsTrunc (PTPS_INODE pinode, TPS_SIZE_T szNewSize)
{
    ...
    do {
        ...
        if (tpsres != TPS_ERR_NONE && tpsres != TPS_ERR_TRANS_NEED_COMMIT) {
            tpsFsTransRollBackAndFree(ptrans);
            if (tpsres == TPS_ERR_BTREE_DISK_SPACE) {
                return  (ENOSPC);
            }
            return  (EIO);
        }
...
errno_t  tpsFsSync (PTPS_INODE pinode)
{
    ...
    iErr = tpsFsFlushHead(pinode);                                      /* 开启事务同步文件头           */
    if (iErr != TPS_ERR_NONE) {
        return  (iErr);
    }

    if (tpsFsTransFlush(psb, LW_TRUE) != TPS_ERR_NONE) {
        return  (EIO);
    }
