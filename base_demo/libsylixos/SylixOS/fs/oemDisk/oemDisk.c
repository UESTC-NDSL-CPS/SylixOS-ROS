                                     ...
                                     BOOL               bForceFsType)
{
     ...
     *  初始化所有的分区挂载失败
     */
    for (i = 0; i < LW_CFG_MAX_DISKPARTS; i++) {
...
/*********************************************************************************************************
** 函数名称: API_OemDiskGetNPart
** 功能描述: 获得 OEM 磁盘设备分区总数
** 输　入  : poemd              OEM 磁盘控制块
** 输　出  : ERROR CODE or 分区总数
** 全局变量:
** 调用模块:
                                           API 函数
*********************************************************************************************************/
LW_API
INT  API_OemDiskGetNPart (PLW_OEMDISK_CB  poemd)
{
    if (poemd == LW_NULL) {
        _ErrorHandle(EINVAL);
        return  (PX_ERROR);
...
INT  API_OemDiskGetPath (PLW_OEMDISK_CB  poemd, INT  iIndex, PCHAR  pcPath, size_t stSize)
{
    ...
    if (iIndex < 0 || iIndex >= poemd->OEMDISK_uiNPart) {
        _ErrorHandle(EINVAL);
        return  (PX_ERROR);
    }
...
/*********************************************************************************************************
** 函数名称: API_OemDiskFsUmount
** 功能描述: 卸载 OEM 磁盘设备指定下标挂载的文件系统
** 输　入  : poemd              OEM 磁盘控制块
**           iIndex             下标, 如果 0 代表第一个磁盘, 1 代表第二个磁盘...
** 输　出  : ERROR CODE
** 全局变量:
...
*********************************************************************************************************/
LW_API
INT  API_OemDiskFsUmount (PLW_OEMDISK_CB  poemd, INT  iIndex)
{
    ...
    bnprintf(cFullVolName, sizeof(cFullVolName), 0, "%s%d",
             poemd->OEMDISK_cVolName,
             poemd->OEMDISK_iVolSeq[iIndex]);

    if (poemd->OEMDISK_pdevhdr[iIndex] != API_IosDevMatchFull(cFullVolName)) {
        return  (PX_ERROR);
    }

    if (unlink(cFullVolName)) {
        return  (PX_ERROR);                                             /*  卸载文件系统失败            */
    }

    poemd->OEMDISK_iVolSeq[iIndex] = PX_ERROR;
    __oemAutoMountDelete(cFullVolName);
