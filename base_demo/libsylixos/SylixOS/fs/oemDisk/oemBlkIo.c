...
#define BLKIO_READ(buf, sec, n, r)  (pblkDev->BLKD_pfuncBlkRd(pblkDev, buf, sec, n, r))
...
static ssize_t  __blkIoFsPRead (PLW_BLKIO_DEV  pdevblk, PCHAR  pcBuffer, size_t stMaxBytes, off_t oft)
{
    ...
    if (ulStartSec == ulEndSec) {
        if (BLKIO_READ(pdevblk->BLKIO_pucSec, ulStartSec, 1, 0) < 0) {
            return  (PX_ERROR);
        }
        ...
        if (ulEndSec > ulStartSec) {
            if (BLKIO_READ(pcBuffer, ulStartSec, ulEndSec - ulStartSec, 0) < 0) {
                return  (PX_ERROR);
            }
        ...
        if (ulEndOff) {
            if (BLKIO_READ(pdevblk->BLKIO_pucSec, ulEndSec, 1, 0) < 0) {
                return  (PX_ERROR);
            }
...
static ssize_t  __blkIoFsPWrite (PLW_BLKIO_DEV  pdevblk, PCHAR  pcBuffer, size_t stNBytes, off_t oft)
{
    ...
    if (ulStartSec == ulEndSec) {
        if (BLKIO_READ(pdevblk->BLKIO_pucSec, ulStartSec, 1, 1) < 0) {
            return  (PX_ERROR);
        }
        ...
        if (ulEndOff) {
            if (BLKIO_READ(pdevblk->BLKIO_pucSec, ulEndSec, 1, 1) < 0) {
                return  (PX_ERROR);
            }
