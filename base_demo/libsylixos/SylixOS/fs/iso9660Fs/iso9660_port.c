...
/*********************************************************************************************************
  block 函数声明
*********************************************************************************************************/
extern INT  __blockIoDevRead(INT iIndex, VOID *pvBuffer,
                             ULONG ulStartSector, ULONG ulSectorCount, ULONG ulRemainCount);
/*********************************************************************************************************
** 函数名称: cdio_stdio_new
** 功能描述: 新建数据层对象
** 输　入  : drv_num    块设备号
                          ...
                          size_t            nmemb)
{
    ...
    if (oft & ISO_BLOCK_MASK) {                                         /*  起始位置不对齐              */
        ...
        if (__blockIoDevRead(p_cds->drv_num, sec_buff,
                             (oft >> ISO_BLOCK_SHIFT), 1, 0) != ERROR_NONE) {
            cdio_free(sec_buff);
            return  (PX_ERROR);
        }
    ...
    if (__blockIoDevRead(p_cds->drv_num, ptr,
                         start_sec, sec_cnt, 0) != ERROR_NONE) {
        return  (PX_ERROR);
    }
    ...
    if (total_bytes & ISO_BLOCK_MASK) {                                 /*  读取长度不对齐              */
        ...
        if (__blockIoDevRead(p_cds->drv_num, sec_buff,
                             start_sec + sec_cnt, 1, 0) != ERROR_NONE) {
            cdio_free(sec_buff);
