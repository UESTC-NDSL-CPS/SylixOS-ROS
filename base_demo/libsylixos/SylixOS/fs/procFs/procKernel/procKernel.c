...
static ssize_t  __procFsKernelHetrccRead(PLW_PROCFS_NODE  p_pfsn,
                                         PCHAR            pcBuffer,
                                         size_t           stMaxBytes,
                                         off_t            oft);
static ssize_t  __procFsKernelCpuHetrccRead(PLW_PROCFS_NODE  p_pfsn,
                                            PCHAR            pcBuffer,
                                            size_t           stMaxBytes,
                                            off_t            oft);
...
#if LW_CFG_CPU_ARCH_HETRC > 0
static LW_PROCFS_NODE_OP        _G_pfsnoKernelHetrccFuncs = {
    __procFsKernelHetrccRead, LW_NULL
};
static LW_PROCFS_NODE_OP        _G_pfsnoKernelCpuHetrccFuncs = {
    __procFsKernelCpuHetrccRead, LW_NULL
};
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
#endif                                                                  /*  LW_CFG_SMP_EN > 0           */
/*********************************************************************************************************
...
#if LW_CFG_CPU_ARCH_HETRC > 0
    LW_PROCFS_INIT_NODE("hetrcc",
                        (S_IFREG | S_IRUSR | S_IRGRP | S_IROTH),
                        &_G_pfsnoKernelHetrccFuncs,
                        "T",
                        0),
    LW_PROCFS_INIT_NODE("cpu_hetrcc",
                        (S_IFREG | S_IRUSR | S_IRGRP | S_IROTH),
                        &_G_pfsnoKernelCpuHetrccFuncs,
                        "H",
                        0),
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
#endif                                                                  /*  LW_CFG_SMP_EN > 0           */
...
/*********************************************************************************************************
** 函数名称: __procFsKernelHetrccRead
** 功能描述: procfs 读一个内核 hetrcc proc 文件
** 输　入  : p_pfsn        节点控制块
**           pcBuffer      缓冲区
**           stMaxBytes    缓冲区大小
**           oft           文件指针
...
static ssize_t  __procFsKernelHetrccRead (PLW_PROCFS_NODE  p_pfsn,
                                          ...
                                          off_t            oft)
{
    const CHAR      cHetrccInfoHdr[] =
    "       NAME        TID    PID  PRI CPU HETRCC AFFINITY\n"
    "---------------- ------- ----- --- --- ------ --------\n";
          PCHAR     pcFileBuffer;
          size_t    stRealSize;                                         /*  实际的文件内容大小          */
          size_t    stCopeBytes;
    ...
    if (pcFileBuffer == LW_NULL) {                                      /*  还没有分配内存              */
        ...
        stNeedBufferSize  = 80 * API_KernelGetThreadNum();
        stNeedBufferSize += sizeof(cHetrccInfoHdr);
        ...
        stRealSize = bnprintf(pcFileBuffer, stNeedBufferSize, 0, cHetrccInfoHdr);
                                                                        /*  打印头信息                  */
        __KERNEL_ENTER();
        for (i = 0; i < LW_CFG_MAX_THREADS; i++) {
            ptcb = _K_ptcbTCBIdTable[i];                                /*  获得 TCB 控制块             */
            ...
            iPrio   = ptcb->TCB_ucPriority;
            iCpu    = ptcb->TCB_ulCPUId;
            ulHccId = ptcb->TCB_ulHetrccLock;
            __KERNEL_EXIT();

            stRealSize = bnprintf(pcFileBuffer, stNeedBufferSize, stRealSize,
                                  "%-16s %7lx %5d %3d %3d %6lu %s\n",
                                  cName, ulId, pid, iPrio, iCpu, ulHccId,
                                  iMode == LW_HETRCC_WEAK_AFFINITY ? "WEAK" : "STRONG");
...
/*********************************************************************************************************
** 函数名称: __procFsKernelCpuHetrccRead
** 功能描述: procfs 读一个内核 cpu_hetrcc proc 文件
** 输　入  : p_pfsn        节点控制块
**           pcBuffer      缓冲区
**           stMaxBytes    缓冲区大小
**           oft           文件指针
                                             ...
                                             off_t            oft)
{
    ...
    if (pcFileBuffer == LW_NULL) {                                      /*  还没有分配内存              */
        ...
        stNeedBufferSize  = 32 * LW_NCPUS;
        stNeedBufferSize += sizeof(cCpuHetrccInfoHdr);
        ...
        stRealSize = bnprintf(pcFileBuffer, stNeedBufferSize, 0, cCpuHetrccInfoHdr);
                                                                        /*  打印头信息                  */
        LW_CPU_FOREACH (i) {
            pcpu = LW_CPU_GET(i);
            stRealSize = bnprintf(pcFileBuffer, stNeedBufferSize, stRealSize,
                                  "%6lu %6lu %8lu %s\n",
                                  pcpu->CPU_ulCPUId, pcpu->CPU_ulHccId,
                                  LW_HCCCPU_GET(pcpu->CPU_ulHccId)->HCCCPU_uiComputPow,
                                  LW_CPU_IS_ACTIVE(pcpu) ? "YES" : "");
        }
...
#if LW_CFG_CPU_ARCH_HETRC > 0
    API_ProcFsMakeNode(&_G_pfsnKernel[8], "/kernel");
    API_ProcFsMakeNode(&_G_pfsnKernel[9], "/kernel");
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
#endif                                                                  /*  LW_CFG_SMP_EN > 0           */
}
