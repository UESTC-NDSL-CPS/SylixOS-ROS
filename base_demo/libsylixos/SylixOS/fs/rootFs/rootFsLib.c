...
*********************************************************************************************************/
LW_API 
INT  API_RootFsMakeNode (CPCHAR  pcName, INT  iNodeType, INT  iNodeOpts, mode_t  iMode, PVOID  pvValue)
{
    ...
    if (prfsn) {
        ...
        if (prfsnFather && prfsnFather->RFSN_iNodeType) {               /*  父系节点不为目录            */
            ...
            if (prfsnFather) {                                          /*  存在父节点                  */
                if (prfsnFather->RFSN_iNodeOpts &
                    LW_ROOTFS_NODE_OPT_ROOTFS_EXTDIR) {
                    if (iNodeType != LW_ROOTFS_NODE_TYPE_DEV) {         /*  扩展目录只允许安装其他设备  */
                        _ErrorHandle(EACCES);
                        goto    __error;
                    }
                }
