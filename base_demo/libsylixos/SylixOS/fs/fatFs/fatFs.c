...
/*********************************************************************************************************
  Root inode64 值
*********************************************************************************************************/
#define __FAT_ROOT_INODE64              ((UINT64)~0)
/*********************************************************************************************************
                          ...
                          INT             iMode)
{
    ...
    if (pcName == LW_NULL) {                                            /*  无文件名                    */
        ...
        if (fresError != FR_OK) {                                       /*  打开失败                    */
            ...
            if (__PATH_IS_ROOT(pfatfile->FATFIL_cName)) {
                pfatfile->FATFIL_iFileType = __FAT_FILE_TYPE_DEV;       /*  仅仅打开设备, 未格式化      */
                pfatfile->FATFIL_u64Uniq   = __FAT_ROOT_INODE64;        /*  不与任何文件重复(根目录)    */
                goto    __file_open_ok;                                 /*  文件打开正常                */
            }
...
static INT  __fatFsFormat (PLW_FD_ENTRY  pfdentry, LONG  lArg)
{
    ...
    if (!__PATH_IS_ROOT(pfatfile->FATFIL_cName)) {                      /*  检查是否为设备文件          */
        _ErrorHandle(EFAULT);                                           /*  Bad address                 */
        return  (PX_ERROR);
    }
...
** 调用模块:
*********************************************************************************************************/
static INT  __fatFsAllocate (PLW_FD_ENTRY  pfdentry, off_t *oftArg)
{
    PLW_FD_NODE    pfdnode  = (PLW_FD_NODE)pfdentry->FDENTRY_pfdnode;
    PFAT_FILE      pfatfile = (PFAT_FILE)pfdnode->FDNODE_pvFile;
    FRESULT        fresError;
    ...
    if (pfatfile->FATFIL_iFileType == __FAT_FILE_TYPE_NODE) {
        ...
        if (oftNewSize > oftOldSize) {                                  /*  需要放大文件                */
            /*
             * f_lseek 允许直接扩大文件大小并使用随机内容填充,
             * 注意: 不可使用 _USE_FASTSEEK 模式.
             */
...
static INT  __fatFsStatGet (PLW_FD_ENTRY  pfdentry, struct stat *pstat)
{
    ...
    if (ulError != ERROR_NONE) {
        if (__PATH_IS_ROOT(pfatfile->FATFIL_cName)) {                   /*  为根目录                    */
            fileinfo.fsize    = 0;
            /*
                          ...
                          LONG          lArg)
{
    ...
    case FIOALLOCATE:                                                   /*  文件预分配空间              */
        return  (__fatFsAllocate(pfdentry, (off_t *)lArg));
