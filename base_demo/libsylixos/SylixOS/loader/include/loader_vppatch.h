...
typedef struct lw_ld_vproc {
    ...
    VOIDFUNCPTR             VP_pfuncCxaFinalize;                        /*  进程 __cxa_module_finalize  */
    
    size_t                  VP_stTlsSize;                               /*  进程 TLS 大小               */
#if LW_CFG_VMM_EN > 0
    PVOID                   vp_pvTlsMem;                                /*  进程 TLS 内存               */
...
#define __LW_VP_CONTINUE    2
#define __LW_VP_STOP        3
#define __LW_VP_EXIT        4
    INT                     VP_iStatus;                                 /*  当前进程状态                */
    INT                     VP_iExitCode;                               /*  结束代码                    */
    INT                     VP_iSigCode;                                /*  iSigCode                    */
    INT                     VP_iExitSignal;                             /*  结束信号                    */
    
#define __LW_VP_FT_DAEMON       0x01                                    /*  daemon 进程                 */
#define __LW_VP_FT_FPU_EXC_EN   0x02                                    /*  Enabled FPU exception       */
#define __LW_VP_FT_DSP_EXC_EN   0x04                                    /*  Enabled DSP exception       */
    ULONG                   VP_ulFeatures;                              /*  进程 features               */
    ...
    LW_OBJECT_HANDLE        VP_ulKillPrepare;                           /*  清除进程前是否需要 release  */
    INT                     VP_iExitMode;                               /*  退出模式                    */
    BOOL                    VP_bExiting;                                /*  开始退出                    */
...
#define __LW_VP_TCB_PID(ptcb)           vprocGetPidByTcb(ptcb)
#define __LW_VP_TCB_PID_NOLOCK(ptcb)    vprocGetPidByTcbNoLock(ptcb)
...
VOID                vprocIoReclaim(pid_t  pid, BOOL  bIsExec);          /*  进程 exec 与被回收时被调用  */
...
INT                 vprocIoFileCloExec(INT  iFd, INT  iCloExec);
