...
typedef struct {
    ...
    BOOL                    EMOD_bPIE;                                  /*  是否为 PIE 格式可执行文件   */
    ...
    INT64                   EMOD_i64RelaStack[LA_RELA_STACK_DEPTH];     /*  RELA 栈                     */
    size_t                  EMOD_stRelaStackTop;                        /*  RELA 栈顶                   */
#endif                                                                  /*  LW_CFG_CPU_ARCH_CSKY        */
} LW_LD_EXEC_MODULE;
...
VOID  __ldSetMemText(PVOID  pvAddr, size_t  stLen, BOOL  bCanExec);     /*  设置只读段                  */

#define LW_LD_VMSAFESETTEXT(a, len, exec) \
        { if (a) { __ldSetMemText(a, len, exec); } }
...
VOID    __ldProtect(PVOID  pvBase, size_t  stAddrOft, size_t  stLen);   /*  设置代码段保护区域          */
