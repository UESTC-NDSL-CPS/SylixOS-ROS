...
static INT elfModuleMemoryInit (LW_LD_EXEC_MODULE *pmodule, Elf_Shdr *pshdrArr)
{
    ...
    for (i = 0; i <= pmodule->EMOD_ulSegCount; i++, pshdr++) {
        if (pshdr->sh_flags & SHF_ALLOC) {
            ...
            ulAlign = ROUND_UP(ulAlign, LW_CFG_VMM_PAGE_SIZE);
#endif
...
#if (LW_CFG_TRUSTED_COMPUTING_EN > 0) || (LW_CFG_MODULELOADER_TEXT_RO_EN > 0) || defined(LW_CFG_CPU_ARCH_C6X)
            pmodule->EMOD_psegmentArry[i].ESEG_bCanExec = (pshdr->sh_flags & SHF_EXECINSTR) ?
                                                          LW_TRUE : LW_FALSE;
#endif                                                                  /*  LW_CFG_TRUSTED_COMPUTING_EN */
#if LW_CFG_MODULELOADER_TEXT_RO_EN > 0
            pmodule->EMOD_psegmentArry[i].ESEG_bReadOnly = (pshdr->sh_flags & SHF_WRITE) ?
                                                           LW_FALSE : LW_TRUE;
#endif                                                                  /*  LW_CFG_MODULELOADER_TEXT_.. */
        }
    }
...
#if LW_CFG_MODULELOADER_TEXT_RO_EN > 0
    for (i = 0; i <= pmodule->EMOD_ulSegCount; i++) {
        if (pmodule->EMOD_psegmentArry[i].ESEG_bReadOnly) {             /*  保护只读区域                */
            LW_LD_VMSAFESETTEXT((PVOID)pmodule->EMOD_psegmentArry[i].ESEG_ulAddr,
                                       pmodule->EMOD_psegmentArry[i].ESEG_stLen,
                                       pmodule->EMOD_psegmentArry[i].ESEG_bCanExec);
        }
    }
...
__out1:
    ...
    (LW_CFG_MODULELOADER_TEXT_RO_EN == 0) && \
    ...
    for (i = 0; i < iPhdrCnt; i++, pphdr++) {
        if (PT_DYNAMIC == pphdr->p_type) {                              /*  找到dynamic段               */
            ...
            for (j = 0; j < ulItemCount; j++, pdyn++) {
                ...
                case DT_FLAGS_1:
                    pmodule->EMOD_bPIE = (pdyn->d_un.d_val & DF_1_PIE) ? LW_TRUE : LW_FALSE;
                    break;
                        ...
                        INT                iFd)
{
    ...
    for (i = 0; i < pehdr->e_phnum; i++, pphdr++) {
        if (PT_TLS == pphdr->p_type) {                                  /*   TLS 段                     */
            pmodule->EMOD_stTlsSize = pphdr->p_memsz;
            continue;
        }
     ...
     *  分段映射可加载段
     */
#if (LW_CFG_TRUSTED_COMPUTING_EN > 0) || (LW_CFG_MODULELOADER_TEXT_RO_EN > 0) || \
    defined(LW_CFG_CPU_ARCH_C6X)                                        /*  动态度量需要确定内存结构    */
    ...
    if (EM_CSKY_OLD == pehdr->e_machine) {
        ...
        for (i = 0; i < pehdr->e_shnum; i++, pshdr++) {
            if ((pshdr->sh_type == SHT_RELA)   ||
                ...
                pdyndir->ulRelaCount += pshdr->sh_size / sizeof(Elf_Rela);
                                                                        /*  RELA重定位表项数            */
                pdyndir->prelaTable   = (Elf_Rela *)LW_LD_V2PADDR(addrMin,
                                                        pmodule->EMOD_pvBaseAddr,
                                                        pshdr->sh_offset);
