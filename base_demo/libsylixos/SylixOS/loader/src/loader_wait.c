...
/*********************************************************************************************************
  单次等待超时时间
*********************************************************************************************************/
static const struct timespec    _G_tvWaitTimeout = { 1, 0 };
/*********************************************************************************************************
** 函数名称: fork
** 功能描述: posix fork
** 输　入  : NONE
                            ...
                            struct rusage *prusage)
{
    ...
    while (plineList) {                                                 /*  遍历子进程                  */
        ...
        if (pvprocChild->VP_iStatus == __LW_VP_EXIT) {                  /*  子进程已经结束              */
            ...
            if (stat_loc) {
                *stat_loc = SET_EXITSTATUS(pvprocChild->VP_iExitCode) | /*  子进程退出状态              */
                            SET_TERMSIG(pvprocChild->VP_iExitSignal);   /*  子进程退出信号              */
            }
        ...
        } else if (pvprocChild->VP_iStatus == __LW_VP_CONTINUE) {       /*  子进程继续                  */
            pvprocChild->VP_iStatus = __LW_VP_RUN;

            if (option & WCONTINUED) {
                if (pid) {
                    *pid = 0;                                           /*  返回值为 0                  */
                }
                if (stat_loc) {
        ...
        } else if (pvprocChild->VP_ulFeatures & __LW_VP_FT_DAEMON) {    /*  子进程进入 daemon 状态      */
            if (pid) {
                *pid = pvprocChild->VP_pid;
            }
            if (stat_loc) {
    ...
    if (bHasEvent) {
        ...
        if (sigGetAction(pvproc->VP_ulMainThread,
                         SIGCHLD, &sigactionChld) == ERROR_NONE) {
            if ((sigactionChld.sa_flags & SA_NOCLDWAIT) ||
                (sigactionChld.sa_handler == SIG_IGN)) {                /*  无法通过 wait 获取 chld     */
                errno = ECHILD;
                return  (PX_ERROR);
            }
        }
...
pid_t wait (int *stat_loc)
{
    ...
    do {
         ...
         * 注意: 基于以下两点考虑, 使用超时等待
         *
         * 1. 当前线程可能是非主线程，而 SIGCHLD 信号只会发给父进程的主线程,
         *    导致父进程的非主线程一直等不到 SIGCHLD 信号
         * 2. 有可能在这个时间点, 子进程退出了并给父进程的主线程发了 SIGCHLD 信号,
         *    而父进程的主线程也绑定了 SIGCHLD 信号, 从而导致 sigtimedwait 超时
         */
        iError = sigtimedwait(&sigsetSigchld,
                              &siginfoChld,
                              &_G_tvWaitTimeout);                       /*  等待子进程信号              */
        if (iError < 0) {
            if (errno != EAGAIN) {
