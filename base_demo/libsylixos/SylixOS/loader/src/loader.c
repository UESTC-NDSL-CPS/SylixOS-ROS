...
/*********************************************************************************************************
** 函数名称: moduleGetConfLibPath
** 功能描述: 通过加载器配置文件获得库文件的路径
** 输　入  : pcFileName    用户路径参数
**           pcPathBuffer  查找到的文件路径缓冲
**           stMaxLen      缓冲区大小
**           pvproc        进程结构
...
** 调用模块:
*********************************************************************************************************/
static INT moduleGetConfLibPath (CPCHAR  pcFileName, PCHAR  pcPathBuffer,
                                 size_t  stMaxLen, LW_LD_VPROC  *pvproc, struct stat  *pstatFile)
{
    CHAR    cBuffer[MAX_FILENAME_LENGTH];
    PCHAR   pcStart;
    PCHAR   pcDiv;
    ...
    lib_strlcpy(cBuffer, pvproc->VP_pcLdConf, MAX_FILENAME_LENGTH);
    pcPathBuffer[stMaxLen - 1] = PX_EOS;
...
LW_LD_EXEC_MODULE *moduleLoadSub (LW_LD_EXEC_MODULE *pmodule, CPCHAR pcLibName, BOOL bCreate)
{
     ...
     *  获取动态链接库位置
     */
    if (ERROR_NONE != moduleGetConfLibPath(pcLibName, cLibPath, MAX_FILENAME_LENGTH, pmodule->EMOD_pvproc, &statFile)) {
...
static INT finiArrayCall (LW_LD_EXEC_MODULE *pmodule, BOOL  bRunFini)
{
    ...
    pfuncCxaFinalize = pmodTemp->EMOD_pvproc->VP_pfuncCxaFinalize
                     ? pmodTemp->EMOD_pvproc->VP_pfuncCxaFinalize : __cxa_module_finalize;
    ...
    do {
        ...
        if (pmodTemp->EMOD_ulRefs == 0 &&
            ...
            if (bRunFini ||
                (pfuncCxaFinalize == __cxa_module_finalize)) {          /*  不需要运行则不运行进程 fini */
                LW_SOFUNC_PREPARE(pfuncCxaFinalize);
                pfuncCxaFinalize(pmodTemp->EMOD_pvBaseAddr,
                                 pmodTemp->EMOD_stLen,
                         ...
                         PVOID   pvVProc)
{
    ...
    if (LW_NULL == pmodule) {                                           /*  获取动态链接库位置          */
        if (ERROR_NONE != moduleGetConfLibPath(pcFile, cLibPath, MAX_FILENAME_LENGTH, pvproc, &statFile)) {
