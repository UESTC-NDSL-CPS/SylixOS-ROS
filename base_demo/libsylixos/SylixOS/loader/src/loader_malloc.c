...
/*********************************************************************************************************
** 函数名称: __ldSetMemText
** 功能描述: 设置区域内存为 TEXT 段应有属性 (装载 .ko 内核模块时使用此内存).
** 输　入  : pvAddr        起始地址
**           stLen         长度
**           bCanExec      是否可执行
** 输　出  : NONE
...
** 调用模块:
*********************************************************************************************************/
VOID  __ldSetMemText (PVOID  pvAddr, size_t  stLen, BOOL  bCanExec)
{
#if LW_CFG_VMM_EN > 0 && LW_CFG_MODULELOADER_TEXT_RO_EN > 0
    addr_t  adrAddr = (addr_t)pvAddr;
    ULONG   ulPageNum;
    ULONG   ulFlags;

    adrAddr    = ROUND_UP(adrAddr, LW_CFG_VMM_PAGE_SIZE);
    stLen     -= adrAddr - (addr_t)pvAddr;
    ulPageNum  = stLen >> LW_CFG_VMM_PAGE_SHIFT;

    if (ulPageNum) {
        ulFlags = bCanExec
                ? LW_VMM_FLAG_EXEC | LW_VMM_FLAG_READ
                : LW_VMM_FLAG_READ;
        __vmmLibSetFlag(adrAddr, ulPageNum, ulFlags, LW_TRUE);
    }
               ...
               off_t  oftOffset, size_t  stLen,  BOOL  bCanShare, BOOL  bCanExec)
{
    ...
    if (ptcbCur->TCB_ulOption & LW_OPTION_THREAD_NUMA) {
        bCanShare = LW_FALSE;                                           /*  NUMA 进程不 share           */
    }
#endif                                                                  /*  LW_CFG_SMP_NUMA_EN > 0      */

    if (bCanShare == LW_FALSE || LW_CFG_MODULELOADER_TEXT_RO_EN) {      /*  私有或保护区域              */
