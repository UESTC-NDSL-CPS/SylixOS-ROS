...
#define __VPROC_PROCFS_STATE_SIZE           32
...
#define __VPROC_PROCFS_STATE_EN             1
...
#define __VPROC_PROCFS_STATE                "state"                     /*  进程状态                    */
/*********************************************************************************************************
  内核 proc 文件函数声明
*********************************************************************************************************/
...
static ssize_t  __procFsProcIoenvRead(PLW_PROCFS_NODE  p_pfsn, 
                                      ...
                                      size_t           stMaxBytes,
                                      off_t            oft);
static ssize_t  __procFsProcStateRead(PLW_PROCFS_NODE  p_pfsn,
                                      PCHAR            pcBuffer,
                                      size_t           stMaxBytes,
                                      off_t            oft);
/*********************************************************************************************************
...
static LW_PROCFS_NODE_OP        _G_pfsnoProcStateFuncs = {
    __procFsProcStateRead, LW_NULL
};
/*********************************************************************************************************
                                         ...
                                         off_t            oft)
{
    ...
    if (pcFileBuffer == LW_NULL) {                                      /*  还没有分配内存              */
        ...
        for (pringTemp  = pvproc->VP_ringModules, bStart = LW_TRUE;
            ...
            stNameLen = lib_strlen(pmodTemp->EMOD_pcModulePath) + 1;
...
/*********************************************************************************************************
** 函数名称: __procFsProcStateRead
** 功能描述: procfs 读一个进程 state 文件
** 输　入  : p_pfsn        节点控制块
**           pcBuffer      缓冲区
**           stMaxBytes    缓冲区大小
**           oft           文件指针
                                       ...
                                       off_t            oft)
{
    ...
    if (stRealSize == 0) {                                              /*  需要生成文件                */
        ...
        __KERNEL_MODE_PROC(
            usIndex  = _ObjectGetIndex(pvproc->VP_ulMainThread);
            ptcbMain = __GET_TCB_FROM_INDEX(usIndex);                   /*  主线程 TCB                  */
            uiSecReg = ptcbMain->TCB_uiSecReg;
        );
...
INT  vprocProcAdd (LW_LD_VPROC *pvproc, CPCHAR pcWd)
{
...
#if __VPROC_PROCFS_EXE_EN                                               /*  %d/exe 文件                 */
    {
        ...
        PCHAR               pcExe;
        CHAR                cPathBuffer[MAX_FILENAME_LENGTH];
        ...
        if (pcWd && (pvproc->VP_pcName[0] != PX_DIVIDER)) {
            _PathCat(pcWd, pvproc->VP_pcName, cPathBuffer);
        } else {
            lib_strlcpy(cPathBuffer, pvproc->VP_pcName, MAX_FILENAME_LENGTH);
        }
        LW_PROCFS_INIT_SYMLINK_IN_CODE(pfsnExe, pcExe, (S_IFLNK | S_IRUSR | S_IRGRP), LW_NULL, 
                                       (PVOID)pvproc, cPathBuffer);
...
#if __VPROC_PROCFS_STATE_EN                                             /*  %d/state 文件               */
    {
        LW_PROCFS_NODE     *pfsnState;
        PCHAR               pcState;

        pfsnState = (LW_PROCFS_NODE *)__SHEAP_ALLOC(sizeof(LW_PROCFS_NODE) +
                                                    lib_strlen(__VPROC_PROCFS_STATE) + 1);
        if (pfsnState == LW_NULL) {
            return  (PX_ERROR);
        }
        pcState = (PCHAR)pfsnState + sizeof(LW_PROCFS_NODE);
        lib_strcpy(pcState, __VPROC_PROCFS_STATE);

        LW_PROCFS_INIT_NODE_IN_CODE(pfsnState, pcState, (S_IFREG | S_IRUSR | S_IRGRP),
                                    &_G_pfsnoProcStateFuncs,
                                    (PVOID)pvproc, __VPROC_PROCFS_STATE_SIZE);

        if (API_ProcFsMakeNode(pfsnState, pcName) < ERROR_NONE) {       /*  加入 proc 文件系统          */
            __SHEAP_FREE(pfsnState);
        }
    }
#endif                                                                  /*  __VPROC_PROCFS_STATE_EN     */
