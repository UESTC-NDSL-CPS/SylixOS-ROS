...
#define __LW_VP_PATCH_CXA_FINALIZE   "__cxa_module_finalize"
/*********************************************************************************************************
  proc 文件系统操作
*********************************************************************************************************/
#if LW_CFG_PROCFS_EN > 0
INT  vprocProcAdd(LW_LD_VPROC *pvproc, CPCHAR pcWd);
...
VOID __moduleVpPatchInit (LW_LD_EXEC_MODULE *pmodule)
{
    ...
    pvproc->VP_pfuncCxaFinalize = (VOIDFUNCPTR)API_ModuleSym(pmodule, __LW_VP_PATCH_CXA_FINALIZE);

    funcVpCtor = (VOIDFUNCPTR)API_ModuleSym(pmodule, __LW_VP_PATCH_CTOR);
    if (funcVpCtor) {
        LW_SOFUNC_PREPARE(funcVpCtor);
        funcVpCtor(pvproc,
                   &pvproc->VP_pfuncMalloc,
                   &pvproc->VP_pfuncFree);
    }
}
/*********************************************************************************************************
...
INT  vprocDetach (pid_t  pid)
{
    ...
    if (pvproc) {
        ...
        pvproc->VP_ulFeatures |= __LW_VP_FT_DAEMON;                     /*  进入 daemon 状态            */
    }
    LW_LD_UNLOCK();
...
** 调用模块:
*********************************************************************************************************/
LW_LD_VPROC *vprocCreate (CPCHAR  pcFile, CPCHAR  pcWd, ULONG  ulExts)
{
    ...
    if (ulExts & POSIX_SPAWN_EXT_FPU_EXC_EN) {
        pvproc->VP_ulFeatures |= __LW_VP_FT_FPU_EXC_EN;
    }
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */

#if LW_CFG_CPU_DSP_EN > 0
    if (ulExts & POSIX_SPAWN_EXT_DSP_EXC_EN) {
        pvproc->VP_ulFeatures |= __LW_VP_FT_DSP_EXC_EN;
    }
...
INT vprocDestroy (LW_LD_VPROC *pvproc)
{
    ...
    if (pvproc->VP_pcLdConf) {
        LW_LD_SAFEFREE(pvproc->VP_pcLdConf);
    }
    LW_LD_SAFEFREE(pvproc->VP_pcName);
    LW_LD_SAFEFREE(pvproc);
...
** 调用模块:
*********************************************************************************************************/
VOID  vprocKillPrepare (pid_t  pid, LW_OBJECT_HANDLE  ulExcept)
{
    LW_LD_VPROC    *pvproc;
    ...
    LW_LD_LOCK();
    pvproc = _G_pvprocTable[pid - 1];
    if (pvproc && !pvproc->VP_ulKillPrepare) {
        LW_VP_LOCK(pvproc);                                             /*  锁定目标进程                */
        pvproc->VP_ulKillPrepare = ulExcept;                            /*  删除前需要解锁              */
        ...
        for (plineTemp  = pvproc->VP_plineThread;
            ...
            if (ptcb->TCB_ulId == pvproc->VP_ulMainThread ||
                ptcb->TCB_ulId == ulExcept) {
                continue;                                               /*  不停止主任务与排除任务      */
            }
...
__again:
            ...
            if (ptcb->TCB_iDeleteProcStatus == LW_TCB_DELETE_PROC_NONE) {
                ...
                ulError = ERROR_THREAD_NULL;                            /*  已经在删除过程中            */
            }
            __KERNEL_EXIT();                                            /*  退出内核                    */
...
static VOID  vprocKillRelease (PLW_CLASS_TCB  ptcbCur)
{
    ...
    PLW_LIST_LINE     plineTemp;
    PLW_CLASS_TCB     ptcb;
    LW_OBJECT_HANDLE  ulExcept;

    pvproc = (LW_LD_VPROC *)ptcbCur->TCB_pvVProcessContext;
    if (pvproc && pvproc->VP_ulKillPrepare) {
        LW_VP_LOCK(pvproc);                                             /*  锁定目标进程                */
        ulExcept = pvproc->VP_ulKillPrepare;
        pvproc->VP_ulKillPrepare = 0ul;
        ...
        for (plineTemp  = pvproc->VP_plineThread;
            ...
            ptcb = _LIST_ENTRY(plineTemp, LW_CLASS_TCB, TCB_lineProcess);
            if (ptcb == ptcbCur || ptcb->TCB_ulId == ulExcept) {        /*  跳过主线程与例外线程        */
                continue;
            }
...
INT  vprocNotifyParent (LW_LD_VPROC *pvproc, INT  iSigCode, BOOL  bUpDateStat)
{
        ...
        if (sigGetAction(ulFatherMainThread,
            ...
            if ((iSigCode == CLD_STOPPED) || (iSigCode == CLD_CONTINUED)) {
                if ((sigactionChld.sa_flags & SA_NOCLDSTOP) ||
                    ...
                    ulFatherMainThread = LW_OBJECT_HANDLE_INVALID;      /*  不向父进程发送信号          */
                }
    ...
    if (bUpDateStat) {
        if ((iSigCode == CLD_EXITED) || 
            ...
            pvproc->VP_iStatus = __LW_VP_CONTINUE;
...
VOID  vprocReclaim (LW_LD_VPROC *pvproc, BOOL  bFreeVproc)
{
    ...
    if (bFreeVproc) {                                                   /*  exit() or _exit()           */
        ...
        __resPidReclaimOnlyFile(pvproc->VP_pid, LW_FALSE, LW_FALSE);    /*  回收 FILE 资源              */
...
static BOOL vprocAtExit (LW_LD_VPROC *pvproc)
{
    if (pvproc->VP_ringModules) {
        if (pvproc->VP_bRunAtExit && !pvproc->VP_bImmediatelyTerm) {    /*  是否需要运行 atexit 安装函数*/
...
VOID  vprocExit (LW_LD_VPROC *pvproc, LW_OBJECT_HANDLE  ulId, INT  iCode)
{
    ...
    __KERNEL_ENTER();                                                   /*  进入内核                    */
    if (pvproc->VP_bExiting) {
        __KERNEL_EXIT();                                                /*  退出内核                    */
        return;                                                         /*  不允许重复嵌套调用此函数    */

    } else {
        pvproc->VP_bExiting = LW_TRUE;                                  /*  开始退出过程                */
        __KERNEL_EXIT();                                                /*  退出内核                    */
    }
...
__recheck:
    bForce = (pvproc->VP_iExitMode == LW_VPROC_EXIT_FORCE);             /*  是否为强制退出              */

#if LW_CFG_THREAD_EXT_EN > 0
    _TCBCleanupPopExt(ptcbCur, LW_TRUE);                                /*  提前执行 cleanup pop 操作   */
#endif                                                                  /*  LW_CFG_THREAD_EXT_EN > 0    */

#if LW_CFG_POSIX_EN > 0
    _PthreadKeyCleanup(ptcbCur, !pvproc->VP_bImmediatelyTerm);          /*  提前执行 key cleanup 操作   */
#endif                                                                  /*  LW_CFG_POSIX_EN > 0         */
    ...
    vprocKillRelease(ptcbCur);                                          /*  release 被 kill prepare 线程*/

    do {                                                                /*  等待所有的线程安全退出      */
        if (vprocCanExit() == LW_FALSE) {                               /*  进程是否可以退出            */
            iError = API_SemaphoreBPend(pvproc->VP_ulWaitForExit, LW_OPTION_WAIT_INFINITE);
            if (iError == EINTR) {
                goto    __recheck;
            }
        } else {
    ...
    __resPidReclaimOnlyFile(pvproc->VP_pid, LW_FALSE, !bForce);         /*  回收进程的 FILE 资源        */

    vprocIoFileDeinit(pvproc);                                          /*  尝试回收文件                */
...
__recheck:
    ...
    __KERNEL_MODE_PROC(pvproc->VP_bExiting = LW_FALSE;);                /*  结束 exiting 状态           */
}
/*********************************************************************************************************
...
** 调用模块:
*********************************************************************************************************/
INT  vprocSetImmediatelyTerm (pid_t  pid, INT  iExitSignal)
{
    LW_LD_VPROC *pvproc;
    ...
    if (pvproc->VP_iExitSignal == 0) {
        pvproc->VP_iExitSignal = iExitSignal;
    }
    LW_LD_UNLOCK();
...
static FUNCPTR  vprocGetEntry (LW_LD_VPROC  *pvproc)
{
    ...
    for (pringTemp  = pvproc->VP_ringModules, bStart = LW_TRUE;
        ...
        if (bStart) {
            if (pmodTemp->EMOD_bPIE && pmodTemp->EMOD_pfuncEntry) {
                pvproc->VP_pfuncProcess = pmodTemp->EMOD_pfuncEntry;

            } else if (__moduleFindSym(pmodTemp, LW_LD_PROCESS_ENTRY,
...
** 调用模块:
*********************************************************************************************************/
static INT  vprocTlsMemInit (LW_LD_VPROC  *pvproc)
{
    BOOL                bStart;
    LW_LIST_RING       *pringTemp;
    LW_LD_EXEC_MODULE  *pmodTemp;
    ...
    pvproc->VP_stTlsSize = 0;                                           /*  计算模块 TLS 内存总大小      */

    for (pringTemp  = pvproc->VP_ringModules, bStart = LW_TRUE;
         pringTemp && (pringTemp != pvproc->VP_ringModules || bStart);
         pringTemp  = _list_ring_get_next(pringTemp), bStart = LW_FALSE) {
        pmodTemp = _LIST_ENTRY(pringTemp, LW_LD_EXEC_MODULE, EMOD_ringModules);
        pvproc->VP_stTlsSize += pmodTemp->EMOD_stTlsSize;
    }

    pvproc->VP_stTlsSize = ROUND_UP(pvproc->VP_stTlsSize, ARCH_STK_ALIGN_SIZE);

#if LW_CFG_VMM_EN > 0
    if (pvproc->VP_stTlsSize > 0) {
        pvproc->vp_pvTlsMem = API_VmmMmap(LW_NULL, pvproc->VP_stTlsSize * LW_CFG_MAX_THREADS,
                                          LW_VMM_PRIVATE_CHANGE, LW_VMM_FLAG_RDWR, -1, 0);
        if (pvproc->vp_pvTlsMem == LW_VMM_MAP_FAILED) {
            iError = PX_ERROR;
        }
    }
#endif                                                                  /*  LW_CFG_VMM_EN > 0           */
...
** 调用模块:
*********************************************************************************************************/
static VOID  vprocFeaturesConfLoad (LW_LD_VPROC      *pvproc)
{
    PCHAR   pcStart;
    PCHAR   pcDiv;
    ...
    pcDiv = pvproc->VP_pcLdConf;                                        /*  从第一个参数开始找          */
    do {
        pcStart = pcDiv;
        ...
        if (*pcStart == '[') {                                          /*  进程特性                    */
#if LW_CFG_CPU_FPU_EN > 0
            if (lib_strncasecmp(pcStart, "[FPU_EXC_EN]", 12) == 0) {
                pvproc->VP_ulFeatures |= __LW_VP_FT_FPU_EXC_EN;
            }
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */

#if LW_CFG_CPU_DSP_EN > 0
            if (lib_strncasecmp(pcStart, "[DSP_EXC_EN]", 12) == 0) {
                pvproc->VP_ulFeatures |= __LW_VP_FT_DSP_EXC_EN;
            }
#endif                                                                  /*  LW_CFG_CPU_DSP_EN > 0       */
        }
               ...
               CPCHAR            ppcEnv[])
{
    ...
    CHAR               cLdConf[MAX_FILENAME_LENGTH];
    PLW_CLASS_TCB      ptcbCur;
    INTREG             iregInterLevel;

    LW_TCB_GET_CUR_SAFE(ptcbCur);                                       /*  当前任务控制块              */

    _ThreadMigrateToProc(ptcbCur, (PVOID)pvproc, LW_TRUE);              /*  当前线程即为主线程          */
                                                                        /*  此线程不再是全局线程        */
                                                                        /*  而且为此线程重新确定 pid    */
    ...
    if (vprocLoadConfFind(pcFile, cLdConf, MAX_FILENAME_LENGTH) == ERROR_NONE) {
        pvproc->VP_pcLdConf = lib_strdup(cLdConf);                      /*  存在装载器配置              */
        if (!pvproc->VP_pcLdConf) {
            _ErrorHandle(ENOMEM);
            iErrLevel = 0;
            goto    __error_handle;
    ...
    if (pvproc->VP_pcLdConf) {
        vprocFeaturesConfLoad(pvproc);                                  /*  加载进程特性配置            */
    }
    ...
    __ARCH_FPU_CTX_INIT(ptcbCur->TCB_pvStackFP,                         /*  初始化当前任务 FPU 上下文   */
                        pvproc->VP_ulFeatures & __LW_VP_FT_FPU_EXC_EN);

    if (ptcbCur->TCB_ulOption & LW_OPTION_THREAD_USED_FP) {             /*  强制使用 FPU                */
        __ARCH_FPU_RESTORE(ptcbCur->TCB_pvStackFP);                     /*  需要恢复当前任务 FPU CTX    */
    ...
    __ARCH_DSP_CTX_INIT(ptcbCur->TCB_pvStackDSP,                        /*  初始化当前任务 DSP 上下文   */
                        pvproc->VP_ulFeatures & __LW_VP_FT_DSP_EXC_EN);

    if (ptcbCur->TCB_ulOption & LW_OPTION_THREAD_USED_DSP) {            /*  强制使用 DSP                */
        __ARCH_DSP_RESTORE(ptcbCur->TCB_pvStackDSP);                    /*  需要恢复当前任务 DSP CTX    */
        KN_INT_ENABLE(iregInterLevel);                                  /*  打开中断                    */
    }
#endif                                                                  /*  LW_CFG_CPU_DSP_EN > 0       */
    ...
    if (vprocTlsMemInit(pvproc) != ERROR_NONE) {                        /*  进程 TLS 内存区初始化       */
        _ErrorHandle(ENOMEM);
        iErrLevel = 1;
        goto    __error_handle;
    }

    if (vprocTlsAlloc(ptcbCur, 0) != ERROR_NONE) {                      /*  分配主线程 TLS              */
        _ErrorHandle(ENOMEM);
        iErrLevel = 1;
        goto    __error_handle;
    }
    
#ifdef __ARCH_CTX_TLS_LOAD
    __ARCH_CTX_TLS_LOAD(ptcbCur);                                       /*  加载主线程 TLS              */
#endif                                                                  /*  defined(__ARCH_CTX_TLS_LOAD)*/
...
** 调用模块:
*********************************************************************************************************/
BOOL  vprocAddrIsInside (pid_t  pid, PVOID  pvAddr)
{
    BOOL                bStart;
    BOOL                bInside = LW_FALSE;
...
*********************************************************************************************************/
LW_API
INT  vprocSecRegionGet (pid_t pid, UINT *puiSecReg)
{
    LW_LD_VPROC        *pvproc;
    PLW_CLASS_TCB       ptcb;
...
*********************************************************************************************************/
LW_API
INT  vprocSecRegionSet (pid_t pid, UINT uiSecReg)
{
    LW_LD_VPROC        *pvproc;
    PLW_CLASS_TCB       ptcb;
    PLW_LIST_LINE       plineTemp;
...
*********************************************************************************************************/
LW_API
UINT  vprocSecRegionGetCur (VOID)
{
    PLW_CLASS_TCB   ptcbCur;

    LW_TCB_GET_CUR_SAFE(ptcbCur);

    return  (ptcbCur->TCB_uiSecReg);
}
/*********************************************************************************************************
...
*********************************************************************************************************/
LW_API
INT  vprocSecRegionSetCur (UINT  uiSecReg)
{
    LW_LD_VPROC    *pvproc;
    PLW_LIST_LINE   plineTemp;
...
** 功能描述: 在当前线程上下文中加载并执行elf文件. 创建新的进程, 当前线程变为进程的主线程
             (这里初始化了进程的外部补丁库)
** 输　入  : pcFile        文件路径
**           pcEntry       入口函数名，如果为LW_NULL，表示不需要调用初始化函数
**           iArgC         程序参数个数
**           ppcArgV       程序参数数组
**           ppcEnv        环境变量数组
** 输　出  : ERROR_NONE 表示没有错误, PX_ERROR 表示错误
                      ...
                      CPCHAR             ppcEnv[])
{
    ...
    pvproc = vprocCreate(pcFile, LW_NULL, 0ul);                         /*  创建进程控制块              */
    if (LW_NULL == pvproc) {
        return  (PX_ERROR);
    }
