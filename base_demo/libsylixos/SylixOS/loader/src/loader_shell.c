...
static INT  __tshellDlConfig (INT  iArgC, PCHAR  *ppcArgV)
{
    ...
    } else if (lib_strcmp(ppcArgV[1], "soload") == 0) {
        if (!vprocLoadConfRefresh()) {
            fprintf(stderr, LW_VP_LOAD_CONF_PATH " refresh error!\n");
            return  (PX_ERROR);
        }
        return  (ERROR_NONE);
...
static VOID  __ldShellInit (VOID)
{
    ...
    API_TShellHelpAdd("dlconfig",    "config dynamic loader.\n"
                                     ...
                                     "dlconfig soload       refresh process shared lib load config in " LW_VP_LOAD_CONF_PATH "\n"
