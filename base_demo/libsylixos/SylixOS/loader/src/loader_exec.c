...
static VOID  __spawnSecRegion (__PSPAWN_ARG  psarg)
{
    ...
    ptcbCur->TCB_uiSecReg = psarg->SA_spawnattr.SPA_padArea.SPAP_exts.SPAE_uiSecReg;
}
/*********************************************************************************************************
** 函数名称: __spawnGetEnvs
** 功能描述: 获取当前进程环境变量
...
static INT  __execShell (PVOID  pvArg)
{
    ...
    if (psarg->SA_spawnattr.SPA_ulExts & POSIX_SPAWN_EXT_SEC_REGION_EN) {
...
static INT  __processShell (PVOID  pvArg)
{
    ...
    if (iError < ERROR_NONE) {
        /*
         * 注意: 此处不能使用 vprocDestroy 因为此处为进程退出,
         *       需要通知父进程并由父进程进行资源回收.
         */
        vprocExit(psarg->SA_pvproc, ulSelf, iError);                    /*  进程退出, 本线程就是主线程  */
        return  (iError);
    }
...
INT  __processStart (INT  mode, __PSPAWN_ARG  psarg)
{
    ...
    ULONG               ulOption;
    size_t              stStackSize;
    ULONG               ulExts = psarg->SA_spawnattr.SPA_ulExts;
    
    LW_TCB_GET_CUR_SAFE(ptcbCur);
    
    if ((ulExts & POSIX_SPAWN_EXT_SEC_REGION_EN) && ptcbCur->TCB_euid) {
        __spawnArgFree(psarg);
        _ErrorHandle(EACCES);                                           /*  不能更改安全域设置          */
        return  (PX_ERROR);
    }
    ...
    if (mode == P_OVERLAY) {                                            /*  替换当前进程空间            */
        ...
        if (ulExts & POSIX_SPAWN_EXT_FPU_EXC_EN) {
            psarg->SA_pvproc->VP_ulFeatures |= __LW_VP_FT_FPU_EXC_EN;
        } else {
            psarg->SA_pvproc->VP_ulFeatures &= ~__LW_VP_FT_FPU_EXC_EN;
        }
        ...
        if (ulExts & POSIX_SPAWN_EXT_DSP_EXC_EN) {
            psarg->SA_pvproc->VP_ulFeatures |= __LW_VP_FT_DSP_EXC_EN;
        } else {
            psarg->SA_pvproc->VP_ulFeatures &= ~__LW_VP_FT_DSP_EXC_EN;
        }
        ...
        psarg->SA_pvproc = vprocCreate(psarg->SA_pcPath, psarg->SA_pcWd, ulExts);
        if (psarg->SA_pvproc == LW_NULL) {
            __spawnArgFree(psarg);
            return  (PX_ERROR);
        }
