...
2023.07.24  文件描述符传递函数, 不能在进程退出状态进行.
*********************************************************************************************************/
...
/*********************************************************************************************************
  确保文件描述符传递函数已完成
  文件描述符回收时, 此进程确定在 EXITING 状态, 所以文件描述符传递函数不会起作用,
  但如果 文件描述符传递函数 先行调用, 且被抢占, 然后此进程退出, 进行回收操作, 此时需要进行加锁并解锁操作
  以便确保文件描述符传递函数已完成.
*********************************************************************************************************/
#define __LW_VP_FD_FLIGHT_DONE()    { LW_LD_LOCK(); LW_LD_UNLOCK(); }
/*********************************************************************************************************
  函数声明
*********************************************************************************************************/
...
** 全局变量:
** 调用模块:
** 注  意  : 此函数在进程资源回收时被调用 (进程退出, 进程 exec 时) 此时进程处于 EXITING 状态
**
**           此函数确认被顺序调用, 这里不需要加 LD 锁, 进程控制块不会被破坏, 只需确保文件描述符传递已完成.
*********************************************************************************************************/
...
** 全局变量:
** 调用模块:
** 注  意  : 此函数在进程资源回收时被调用 (进程退出时) 此时进程处于 EXITING 状态
**
**           此函数确认被顺序调用, 这里不需要加 LD 锁, 进程控制块不会被破坏, 只需确保文件描述符传递已完成.
*********************************************************************************************************/
...
INT  vprocIoFileDupFrom (pid_t  pidSrc, INT  iFd)
{
    ...
    if (!pvproc || pvproc->VP_bExiting) {                               /*  EXITING 状态不能进行此操作  */
        LW_LD_UNLOCK();
        return  (PX_ERROR);
    }
...
/*********************************************************************************************************
** 函数名称: vprocIoFileCloExec
** 功能描述: 设置 fd FD_CLOEXEC
** 输　入  : iFd           文件描述符
