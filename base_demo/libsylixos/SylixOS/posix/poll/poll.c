...
int   ppoll (struct pollfd fds[], nfds_t nfds, const struct timespec *timeout_ts, const sigset_t *sigmask)
{
    ...
    if (iRet <= 0) {
        iErrNo = errno;
        if (iErrNo == ERROR_IO_SELECT_UNSUPPORT_IN_DRIVER) {
            errno = EIO;
        } else if (iErrNo == ERROR_IO_SELECT_CONTEXT) {
            errno = ENOMEM;
        } else if (iErrNo == ERROR_IO_SELECT_WIDTH) {
            errno = EBADF;
        } else if (iErrNo == ERROR_THREAD_WAIT_TIMEOUT) {
            errno = ETIMEDOUT;
        }
        return  (iRet);
    }
