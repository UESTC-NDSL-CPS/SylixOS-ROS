...
**           pwd           buffer
**           size          buffer size
** 输　出  : ok or errno (if strlen(ppwd) == 0 the new process working directory will inherit creator's)
** 全局变量: 
** 调用模块: 
                                           API 函数
*********************************************************************************************************/
...
** 功能描述: Set flag word to the attribute structure.
** 输　入  : attrp         spawn attr
**           flags         spawn ext flag
** 输　出  : ok or errno
** 全局变量:
...
/*********************************************************************************************************
** 函数名称: posix_spawnattr_getsecreg
** 功能描述: Get security region from the attribute structure.
** 输　入  : attrp         spawn attr
**           sec_region    spawn security region
** 输　出  : ok or errno
** 全局变量:
** 调用模块:
                                           API 函数
*********************************************************************************************************/
LW_API int   posix_spawnattr_getsecreg (const posix_spawnattr_t *attrp,
                                        unsigned int *sec_region)
{
    if (!attrp) {
        errno = EINVAL;
        return  (EINVAL);
    ...
    if (sec_region) {
        *sec_region = attrp->SPA_padArea.SPAP_exts.SPAE_uiSecReg;
    }

    return  (ERROR_NONE);
}
/*********************************************************************************************************
** 函数名称: posix_spawnattr_setsecreg
** 功能描述: Set security region to the attribute structure.
** 输　入  : attrp         spawn attr
**           sec_region    security region
** 输　出  : ok or errno
** 全局变量:
** 调用模块:
                                           API 函数
*********************************************************************************************************/
LW_API int   posix_spawnattr_setsecreg (posix_spawnattr_t *attrp,
                                        unsigned int sec_region)
{
    if (!attrp) {
        errno = EINVAL;
    ...
    if (sec_region && geteuid()) {
        errno = EACCES;
        return  (EACCES);
    }

    attrp->SPA_padArea.SPAP_exts.SPAE_uiSecReg = sec_region;
