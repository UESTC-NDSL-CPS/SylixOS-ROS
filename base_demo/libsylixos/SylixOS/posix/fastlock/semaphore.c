...
/* Fastlock version */
const char *__sylixos_fastlock_version = "1.0.1";
...
int sem_init (sem_t  *psem, int  pshared, unsigned int  value)
{
    ...
    sys_atomic_set(__GET_ATOMIC(psem), value);
...
int sem_destroy (sem_t  *psem)
{
    ...
    sys_atomic_set(__GET_ATOMIC(psem), 0);
    ...
    while ((cur = sys_atomic_cas(patomic, value, value - 1)) != value) {
        if (cur) {
            ...
            ret = API_VutexPendEx(sys_atomic_vutex(patomic),
                                  LW_OPTION_VUTEX_GREATER_EQU |
                                  LW_OPTION_VUTEX_SIGNAL_INTER, 1, LW_OPTION_WAIT_INFINITE);
            if (ret > 0) {
                break;
            } else if (ret == 0) {
                cur   = sys_atomic_get(patomic);
...
int sem_timedwait (sem_t  *psem, const struct timespec *abs_timeout)
{
    ...
    while ((cur = sys_atomic_cas(patomic, value, value - 1)) != value) {
        if (cur) {
            ...
            ret = API_VutexPendEx(sys_atomic_vutex(patomic),
                                  LW_OPTION_VUTEX_GREATER_EQU |
                                  LW_OPTION_VUTEX_SIGNAL_INTER, 1, timeout);
            if (ret > 0) {
                break;
            } else if (ret == 0) {
...
int sem_post (sem_t  *psem)
{
    ...
    cur = sys_atomic_get(patomic);
    while (cur != SEM_VALUE_MAX) {
        pre = sys_atomic_cas(patomic, cur, cur + 1);
        if (pre == cur) {
            break;
        } else {
            cur = pre;
...
int sem_getvalue (sem_t  *psem, int  *pivalue)
{
    ...
    *pivalue = sys_atomic_get(__GET_ATOMIC(psem));
...
int  sem_flush (sem_t  *psem)
{
    ...
    API_VutexPostEx(sys_atomic_vutex(__GET_ATOMIC(psem)), SEM_VALUE_MAX,
