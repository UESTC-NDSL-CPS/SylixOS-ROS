...
/*********************************************************************************************************
** 函数名称: pthread_sethetrcc_np
** 功能描述: 设置线程调度的异构算力簇
** 输　入  : tid           线程 ID
             mode          模式 (PTHREAD_HETRCC_NON_AFFINITY,
                                 PTHREAD_HETRCC_WEAK_AFFINITY,
                                 PTHREAD_HETRCC_STRONG_AFFINITY)
             cluster       算力簇 ID (编号越大算力越强)
** 输　出  : ERROR or OK
** 全局变量:
** 调用模块:
                                           API 函数
...
int  pthread_sethetrcc_np (pthread_t  thread, int mode, unsigned int cluster)
{
    ...
    return  ((int)API_ThreadSetHetrcc(thread, mode, (ULONG)cluster));
...
int  pthread_gethetrcc_np (pthread_t  thread, int *mode, unsigned int *cluster)
{
    ...
    ulError = API_ThreadGetHetrcc(thread, mode, &ulCluster);
    if ((ulError == ERROR_NONE) && cluster) {
        *cluster = (unsigned int)ulCluster;
    }
...
#else
    if (mode) {
        *mode = PTHREAD_HETRCC_NON_AFFINITY;
    }
    if (cluster) {
        *cluster = 0;
    }
