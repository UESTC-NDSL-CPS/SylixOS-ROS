...
static BOOL              __mtracer_suppress_free_null  = LW_FALSE;
...
void mtracer_reset (void)
{
    ...
    MTRACER_LOCK();
    head = __mtracer_node_list.next;
    tail = __mtracer_node_list.prev;
...
/*
 * suppress free NULL pointer warning
 */
void mtracer_suppress_free_null (BOOL  suppress)
{
    __mtracer_suppress_free_null = suppress;
}
...
{
    return  (mtracer_lib_malloc(nbytes, "malloc", __builtin_return_address(0)));
}
...
void lib_free (void *usr)
{
    ...
    if (!usr) {
        if (!__mtracer_suppress_free_null) {
...
/*
 * mtracer_lib_mallocalign
 */
static void *mtracer_lib_mallocalign (size_t  nbytes, size_t align, const char  *operation, void  *ret_addr)
{
    char              *raw;
    char              *usr;
    mtracer_node_t    *node;
    ...
    if (align < 8) {
        return  (mtracer_lib_malloc(nbytes, operation, ret_addr));
    }
...
{
    return  (mtracer_lib_mallocalign(nbytes, align, "mallocalign", __builtin_return_address(0)));
}

/*
 * mtracer_lib_realloc
 */
static void *mtracer_lib_realloc (void *usr, size_t  new_size, const char  *operation, void  *ret_addr)
{
    char              *raw;
    char              *new;
    char              *new_usr;
    ...
    if (!usr) {
        return  (mtracer_lib_malloc(new_size, operation, ret_addr));
    }
...
{
    return  (mtracer_lib_realloc(usr, new_size, "realloc", __builtin_return_address(0)));
}
...
{
    void  *ptr = mtracer_lib_malloc(nbytes, "xmalloc", __builtin_return_address(0));
...
{
    void  *ptr = mtracer_lib_mallocalign(nbytes, align, "mallocalign", __builtin_return_address(0));
...
{
    return  (mtracer_lib_mallocalign(nbytes, align, "memalign", __builtin_return_address(0)));
}
...
{
    void  *ptr = mtracer_lib_mallocalign(nbytes, align, "xmemalign", __builtin_return_address(0));
 ...
 * mtracer_lib_calloc
 */
static void *mtracer_lib_calloc (size_t  num, size_t  nbytes, const char *operation, void *ret_addr)
{
    size_t total = num * nbytes;
    void *pmem;

    pmem = mtracer_lib_malloc(total, operation, ret_addr);
    if (pmem) {
...
{
    return  (mtracer_lib_calloc(num, nbytes, "calloc", __builtin_return_address(0)));
}
...
{
    void  *ptr = mtracer_lib_calloc(num, nbytes, "xcalloc", __builtin_return_address(0));
...
{
    void  *new_ptr = mtracer_lib_realloc(ptr, new_size, "xrealloc", __builtin_return_address(0));
...
int  lib_posix_memalign (void **memptr, size_t align, size_t size)
{
    ...
    *memptr = mtracer_lib_mallocalign(size, align, "posix_memalign", __builtin_return_address(0));
    if (*memptr) {
        return  (ERROR_NONE);
...
{
    return  (mtracer_lib_malloc(nbytes, "malloc_new", __builtin_return_address(0)));
}

/*
 * mtracer_lib_strdup
 */
static char *mtracer_lib_strdup (const char *str, const char *operation, void *ret_addr)
{
    char *mem = NULL;
    size_t size;
    ...
    size = lib_strlen(str);
    mem = (char *)mtracer_lib_malloc(size + 1, operation, ret_addr);
    if (mem) {
        lib_strcpy(mem, str);
    }
...
{
    return  (mtracer_lib_strdup(str, "strdup", __builtin_return_address(0)));
}
...
{
    char *str_ret = mtracer_lib_strdup(str, "xstrdup", __builtin_return_address(0));
 ...
 *  mtracer_lib_strndup
 */
static char *mtracer_lib_strndup (const char *str, size_t size, const char *operation, void *ret_addr)
{
    size_t len;
    char *news;
    ...
    news = (PCHAR)mtracer_lib_malloc(len + 1, operation, ret_addr);
    if (news == NULL) {
        return  (NULL);
    }
...
{
    return  (mtracer_lib_strndup(str, size, "strndup", __builtin_return_address(0)));
}
...
{
    char *str_ret = mtracer_lib_strndup(str, size, "xstrndup", __builtin_return_address(0));
