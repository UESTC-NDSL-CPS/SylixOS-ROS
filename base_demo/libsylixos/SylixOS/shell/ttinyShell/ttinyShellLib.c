...
static const PCHAR      _G_pcResBuildinCmd[] = {
    "help", "ts", "tp", "wjs", "ps", "modules", "lsmod", "ints", "cpuus", "sem", "msgq"
};
#endif                                                                  /*  LW_CFG_MODULELOADER_EN > 0  */
/*********************************************************************************************************
...
static INT  __tshellRedir (PCHAR  pcString, PCHAR  pcRedir, LW_OBJECT_HANDLE ulMe, INT *piPopCnt)
{
    ...
    if (*pcRedir == '>') {                                              /*  标准输出或标准错误          */
        *pcRedir =  PX_EOS;
        ...
        iFd = open(pcFile, O_RDONLY, DEFAULT_FILE_PERM);                /*  以只读方式打开输入文件      */
        if (iFd < 0) {
            return  (PX_ERROR);
        }
...
#if LW_CFG_MODULELOADER_EN > 0                                          /*  记录父进程                  */
    __TTINY_SHELL_SET_FATHER(ptcbShellBg, __LW_VP_TCB_PID_NOLOCK(ptcbCur));
#else
    __TTINY_SHELL_SET_FATHER(ptcbShellBg, 0);
#endif
