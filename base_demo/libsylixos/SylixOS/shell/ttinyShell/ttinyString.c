...
ULONG  __tshellStrConvertVar (CPCHAR  pcCmd, PCHAR  pcCmdOut)
{
        ...
        if (*pcTemp != '$') {                                           /*  不是变量起始符              */
            if (iVarStart) {                                            /*  属于变量名部分              */
                if (__tshellIsSeparator(*pcTemp) == LW_FALSE) {         /*  变量没有结束                */
                    ...
                    CHAR  cVarValue[NAME_MAX + 5] =
                          __TTINY_SHELL_VAR_PREFIX "\"";                /*  变量值                      */
                    ...
                    if (lib_strcmp(cValName, "?") == 0) {               /*  获得最后一次错误            */
                        ...
                        cVarValue[iVarValueLen + 2] = '"';              /*  使用双引号                  */
                        cVarValue[iVarValueLen + 3] = __TTINY_SHELL_VAR_TAIL_CHAR;
                        iVarValueLen += 4;                              /*  两个双引号一个 VAR 特殊前缀 */
    ...
    if (iVarStart) {                                                    /*  最后一个变量替换还没有完成  */
        CHAR  cVarValue[NAME_MAX + 5] = __TTINY_SHELL_VAR_PREFIX "\"";  /*  变量值                      */
...
ULONG  __tshellStrKeyword (CPCHAR  pcCmd, PCHAR  pcBuffer, PCHAR  *ppcParam)
{
    ...
    if (*pcCommand == __TTINY_SHELL_VAR_PREFIX_CHAR) {
        pcCommand++;                                                    /*  起始为变量替换              */
    }
...
ULONG  __tshellStrDelTransChar (CPCHAR  pcCmd, PCHAR  pcCmdOut)
{
    ...
    for (; *pcCommand != '\0'; pcCommand++) {                           /*  拷贝关键字                  */
        if ((UINT8)(*pcCommand) == __TTINY_SHELL_VAR_PREFIX_CHAR) {
            bVarCovert =  LW_TRUE;                                      /*  变量替换串开始              */
            continue;
        } else if ((UINT8)(*pcCommand) == __TTINY_SHELL_VAR_TAIL_CHAR) {
            bVarCovert =  LW_FALSE;                                     /*  变量替换串结束              */
            continue;
        }

        if (*pcCommand == '\\' && !bVarCovert) {                        /*  是否为转义字符              */
            if (iTransCharMarks) {
                iTransCharMarks = 0;
