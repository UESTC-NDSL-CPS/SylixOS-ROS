...
static INT  __tshellSysCmdHetrcc (INT  iArgC, PCHAR  ppcArgV[])
{
    ...
    if (sscanf(ppcArgV[1], "%lx", &ulId) != 1) {
        fprintf(stderr, "tid error.\n");
        return  (PX_ERROR);
    }

    if (ppcArgV[2][0] == 'n') {
        ulError = API_ThreadSetHetrcc(ulId, LW_HETRCC_NON_AFFINITY, 0);

    } else {
        if (sscanf(ppcArgV[3], "%lu", &ulHccId) != 1) {
            fprintf(stderr, "heterogeneous computing cluster ID error.\n");
            return  (PX_ERROR);
        }

        if (ppcArgV[2][0] == 'w') {
            ulError = API_ThreadSetHetrcc(ulId, LW_HETRCC_WEAK_AFFINITY, ulHccId);

        } else if (ppcArgV[2][0] == 's') {
            ulError = API_ThreadSetHetrcc(ulId, LW_HETRCC_STRONG_AFFINITY, ulHccId);
    ...
    if (ulError) {
        fprintf(stderr, "set hetrcc error: %s\n", lib_strerror((INT)ulError));
    }

    return  (ERROR_NONE);
}
/*********************************************************************************************************
** 函数名称: __tshellSysCmdWield
** 功能描述: 系统命令 "wield"
** 输　入  : iArgC         参数个数
**           ppcArgV       参数表
** 输　出  : 0
...
** 调用模块:
*********************************************************************************************************/
static INT  __tshellSysCmdWield (INT  iArgC, PCHAR  ppcArgV[])
{
    INT   iSticky;

    if ((iArgC >= 3) && !lib_strcmp(ppcArgV[1], "-s")) {
        if (sscanf(ppcArgV[2], "%d", &iSticky) != 1) {
            fprintf(stderr, "Scheduler weak affinity sticky set error.\n");
            return  (PX_ERROR);
        }

        if (_CandTableWieldSticky(iSticky) != iSticky) {                /*  设置 wield sticky 参数      */
            fprintf(stderr, "Parameter out of range!\n");
            return  (PX_ERROR);
        }

    } else {
        iSticky = _CandTableWieldSticky(PX_ERROR);
        printf("Scheduler weak affinity sticky: %d\n", iSticky);
    }

    return  (ERROR_NONE);
}
/*********************************************************************************************************
** 函数名称: __tshellSysCmdNumaGo
** 功能描述: 系统命令 "numago"
** 输　入  : iArgC         参数个数
**           ppcArgV       参数表
** 输　出  : 0
...
** 调用模块:
*********************************************************************************************************/
#if LW_CFG_SMP_NUMA_EN > 0

static INT  __tshellSysCmdNumaGo (INT  iArgC, PCHAR  ppcArgV[])
{
    PLW_CLASS_TCB       ptcbCur;
    pid_t               pid;
    INT                 iRet;
    ULONG               ulCluId, ulError;
    ...
    if (sscanf(ppcArgV[1], "%lu", &ulCluId) != 1) {
        fprintf(stderr, "Cluster ID error.\n");
        return  (PX_ERROR);
    }

    LW_TCB_GET_CUR_SAFE(ptcbCur);

    ulError = API_ThreadSetHetrcc(ptcbCur->TCB_ulId, LW_HETRCC_STRONG_AFFINITY, ulCluId);
    if (ulError) {
        fprintf(stderr, "Set cluster affinity error: %s.\n", lib_strerror((INT)ulError));
        return  (PX_ERROR);
    }

    posix_spawnattr_init(&spawnattr);

    ptcbCur->TCB_ulOption             |= LW_OPTION_THREAD_NUMA;
    spawnattr.SPA_opt.SPO_ulMainOption = LW_OPTION_THREAD_NUMA;

    iRet = posix_spawnp(&pid, ppcArgV[2], LW_NULL, &spawnattr, ppcArgV + 2, LW_NULL);

    posix_spawnattr_destroy(&spawnattr);

    if (iRet) {
        fprintf(stderr, "Spawn process error: %s.\n", lib_strerror(iRet));
        return  (PX_ERROR);
    }
...
VOID  __tshellSysCmdInit (VOID)
{
    ...
    API_TShellFormatAdd("open", " [filename flag(hex) mode(oct)]");
...
#if (LW_CFG_SMP_EN > 0) && (LW_CFG_CPU_ARCH_HETRC > 0)
    API_TShellKeywordAdd("hetrcc", __tshellSysCmdHetrcc);
    API_TShellFormatAdd("hetrcc", " [tid] [non | weak | strong] [hccid]");
    API_TShellHelpAdd("hetrcc",   "set or get heterogeneous computing cluster parameter.\n");

    API_TShellKeywordAdd("wield", __tshellSysCmdWield);
    API_TShellFormatAdd("wield", " -s sticky");
    API_TShellHelpAdd("wield",   "set weak affinity sticky parameter (1 ~ 8) of heterogeneous computing scheduler.\n");

#if LW_CFG_SMP_NUMA_EN > 0
    API_TShellKeywordAddEx("numago", __tshellSysCmdNumaGo, LW_OPTION_KEYWORD_SYNCBG);
    API_TShellFormatAdd("numago", " cluster_id program ...args");
    API_TShellHelpAdd("numago",   "launch a process and strongly affinity this process to the specified NUMA cluster.\n");
#endif                                                                  /*  LW_CFG_SMP_NUMA_EN > 0      */
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
}
