...
/*********************************************************************************************************
  ARM64 处理器快速获取线程上下文相关接口
*********************************************************************************************************/

#if LW_CFG_CPU_FAST_TLS > 0
#define __ARCH_FAST_TLS_TID() \
({ \
    REGISTER PLW_CLASS_TCB ptcb asm("x18"); \
    ptcb->TCB_ulId; \
})
...
/*********************************************************************************************************
  ARM64 处理器当前线程上下文 TLS 加载
*********************************************************************************************************/

#define __ARCH_CTX_TLS_LOAD(ptcb)                                                              \
        do {                                                                                   \
            (ptcb)->TCB_archRegCtx.REG_ulTPIDR_EL0 = (ARCH_REG_T)(ptcb)->TCB_pvTlsLowAddr;     \
            __asm__ __volatile__ ("msr TPIDR_EL0, %0\n" : : "r" ((ptcb)->TCB_pvTlsLowAddr) :); \
        } while (0)
