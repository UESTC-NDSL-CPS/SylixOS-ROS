...
#include "../fpu/arm64Fpu.h"
#include "../fpu/vfpv4/arm64VfpV4.h"
...
#define  ARM64_EXC_TYPE_UNKNOWN   10
    ...
    case EXC_DATA_ABORT:
        ...
        if ((uiExcISS & ISS_DAE_DFSC_MASK) == ISS_DAE_DFSC_ALIGN) {
            abtInfo.VMABT_uiMethod = LW_VMM_ABORT_METHOD_EXEC;
            abtInfo.VMABT_uiType   = LW_VMM_ABORT_TYPE_BUS;             /*  非对齐访问错误              */
            break;
        }

        abtInfo.VMABT_uiMethod = (uiExcISS & ISS_DAE_WNR) ? LW_VMM_ABORT_METHOD_WRITE : LW_VMM_ABORT_METHOD_READ;
        abtInfo.VMABT_uiType   = LW_VMM_ABORT_TYPE_MAP;                 /*  数据错误                    */
        if (((uiExcISS & ISS_DAE_DFSC_L3PERM) == ISS_DAE_DFSC_L3PERM) &&
