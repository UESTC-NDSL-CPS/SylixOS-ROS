...
/*********************************************************************************************************
  注意: 以下函数为 PoU 模式 (如果是整片 DCACHE 操作, ARM64 仅支持 PoU, 其他情况使用步进 PoC 操作)
*********************************************************************************************************/
...
#define ARMv8_CACHE_LOOP_OP_MAX_SIZE_S  (16 * LW_CFG_MB_SIZE)           /*  Small scale                 */
#define ARMv8_CACHE_LOOP_OP_MAX_SIZE_L  ((size_t)~0)                    /*  Large scale                 */
/*********************************************************************************************************
...
static INT  arm64CacheFlush (LW_CACHE_TYPE  cachetype, PVOID  pvAdrs, size_t  stBytes)
{
    ...
    if (cachetype == DATA_CACHE) {
        if (stBytes >= ARMv8_CACHE_LOOP_OP_MAX_SIZE_L) {
...
static INT  arm64CacheInvalidate (LW_CACHE_TYPE  cachetype, PVOID  pvAdrs, size_t  stBytes)
{
    ...
    if (cachetype == INSTRUCTION_CACHE) {
        if (stBytes >= ARMv8_CACHE_LOOP_OP_MAX_SIZE_S) {
