...
/*********************************************************************************************************
** 函数名称: leon3CacheEnableSnoop
** 功能描述: LEON3 使能 CACHE 的 snoop 功能
** 输　入  : NONE
** 输　出  : NONE
** 全局变量:
** 调用模块:
** 注  意  : 部分处理器如玉龙 810a 默认不开启此功能，且实际测试此功能关闭时多核 Cache 无法进行同步，
**           且 LEON4/LEON3 此功能的使能位的复位值为 0， 因此需手动开启
**           LEON4/LEON3 手册描述如下：
**           LEON3 is designed to be used in multi-processor systems.
**           Each processor has a unique index to allow processor enumeration.
**           The write-through caches and snooping mechanism guarantees cache coherency
**           in shared-memory systems.
**           23bit - Data cache snoop enable (DS) - if set, will enable data cache snooping
*********************************************************************************************************/
static VOID  leon3CacheEnableSnoop (VOID)
{
    __asm__ __volatile__ ("lda  [%%g0] 2, %%l1\n\t"
                          "set  0x800000, %%l2\n\t"                     /*  使能 snoop 功能             */
                          "or   %%l2, %%l1, %%l2\n\t"
