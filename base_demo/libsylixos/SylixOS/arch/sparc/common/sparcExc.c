...
#include "../fpu/sparcFpu.h"
#include "../fpu/vfp/sparcVfp.h"
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
#if LW_CFG_VMM_EN > 0
#include "../mm/mmu/sparcMmu.h"
#endif                                                                  /*  LW_CFG_VMM_EN > 0           */
#include "./unaligned/sparcUnaligned.h"
/*********************************************************************************************************
  外部引用变量声明
*********************************************************************************************************/
...
#if LW_CFG_CPU_FPU_EN > 0
    sparcVfpGetFsr(&uiFsr);
    if ((uiFsr & FSR_FTT_MASK) == FSR_FTT_IEEE754) {
        if (uiFsr & FSR_NVC) {
            abtInfo.VMABT_uiMethod = FPE_FLTINV;                        /*  非法操作                    */

        } else if (uiFsr & FSR_OFC) {
            abtInfo.VMABT_uiMethod = FPE_FLTOVF;                        /*  上溢异常                    */

        } else if (uiFsr & FSR_UFC) {
            abtInfo.VMABT_uiMethod = FPE_FLTUND;                        /*  下溢异常                    */

        } else if (uiFsr & FSR_DZC) {
            abtInfo.VMABT_uiMethod = FPE_FLTDIV;                        /*  除零异常                    */

        } else if (uiFsr & FSR_NXC) {
            abtInfo.VMABT_uiMethod = FPE_FLTRES;                        /*  不精确异常                  */
        }

        uiFsr &= ~(FSR_AEXC_ALL | FSR_CEXC_ALL);
        sparcVfpSetFsr(&uiFsr);
    }
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
