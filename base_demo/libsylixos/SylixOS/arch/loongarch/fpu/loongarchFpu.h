...
typedef struct {
    ...
    VOIDFUNCPTR     LFPU_pfuncEnableTask;
    VOIDFUNCPTR     LFPU_pfuncEnableException;
} LOONGARCH_FPU_OP;
...
#define LOONGARCH_VFP_ENABLE_TASK(op, tcb)      op->LFPU_pfuncEnableTask((tcb))
#define LOONGARCH_VFP_ENABLE_EXCEPTION(op, ctx) op->LFPU_pfuncEnableException((ctx))
