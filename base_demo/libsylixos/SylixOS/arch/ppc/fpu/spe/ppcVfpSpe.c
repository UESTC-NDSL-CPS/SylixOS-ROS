...
static VOID  ppcVfpSpeEnableTask (PLW_CLASS_TCB  ptcbCur)
{
    ...
    pregctx->REG_uiMsr |= ARCH_PPC_MSR_SPE;
}
/*********************************************************************************************************
** 函数名称: ppcVfpSpeEnableException
** 功能描述: 使能 VFP 上下文异常
...
** 调用模块:
*********************************************************************************************************/
static VOID  ppcVfpSpeEnableException (PVOID pvFpuCtx)
{
    LW_FPU_CONTEXT  *pfpuCtx    = (LW_FPU_CONTEXT *)pvFpuCtx;
    ARCH_FPU_CTX    *pcpufpuCtx = &pfpuCtx->FPUCTX_fpuctxContext;

    pcpufpuCtx->SPECTX_uiSpefscr |= ARCH_PPC_SPEFSCR_FINVE |
                                    ARCH_PPC_SPEFSCR_FDBZE |
                                    ARCH_PPC_SPEFSCR_FUNFE |
                                    ARCH_PPC_SPEFSCR_FOVFE;
}
/*********************************************************************************************************
...
PPPC_FPU_OP  ppcVfpSpePrimaryInit (CPCHAR  pcMachineName, CPCHAR  pcFpuName)
{
    ...
    ppcVfpSpeRestore(&_G_fpuCtxInit);
    ...
    _G_fpuopVfpSpe.PFPU_pfuncEnableException = ppcVfpSpeEnableException;
