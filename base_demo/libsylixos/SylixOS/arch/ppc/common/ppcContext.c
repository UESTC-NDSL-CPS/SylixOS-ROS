...
** MSR 寄存器位定义
*********************************************************************************************************/
#define ARCH_PPC_MSR_VEC            0x02000000                          /*  ALTIVEC: Bit 6 of MSR       */
#define ARCH_PPC_MSR_SPE            0x02000000                          /*  SPE: Bit 6 of MSR           */
                              ...
                              ULONG                  ulOpt)
{
    ...
    uiMsr &= ~(ARCH_PPC_MSR_FP |                                        /*  禁能 FPU                    */
               ARCH_PPC_MSR_FE0 | ARCH_PPC_MSR_FE1 |
               ARCH_PPC_MSR_SPE |                                       /*  禁能 SPE(E500v1 E500v2)     */
               ARCH_PPC_MSR_VEC);                                       /*  禁能 ALTIVEC                */
    ...
    if (pParam->PP_bGenericPPC32) {                                     /*  通用的 32 位 PowerPC 处理器 */
        ...
        if (ulOpt & LW_OPTION_THREAD_USED_FP) {
            if (archFpuTypeGet() == PPC_FPU_TYPE_SPE) {
                uiMsr |= ARCH_PPC_MSR_SPE;                              /*  使能 SPE(E500v1 E500v2)     */
            } else {
    ...
    pregctx->REG_uiR18 = (ARCH_REG_T)ptcb;                              /*  FAST_TLS                    */
#else
...
/*********************************************************************************************************
** 函数名称: archCrtCtxEnableFpu
** 功能描述: 使能协程寄存器上下文的 FPU
** 输　入  : pregctx    寄存器上下文
** 输　出  : NONE
** 全局变量:
** 调用模块:
...
{
    if (archFpuTypeGet() == PPC_FPU_TYPE_SPE) {
        pregctx->REG_uiMsr |= ARCH_PPC_MSR_SPE;                         /*  使能 SPE(E500v1 E500v2)     */
    } else {
        pregctx->REG_uiMsr |= ARCH_PPC_MSR_FP |                         /*  使能 FPU(E500mc E5500 E6500)*/
                              ARCH_PPC_MSR_FE0 | ARCH_PPC_MSR_FE1;      /*  Floating-point precise mode */
    }
}
...
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
/*********************************************************************************************************
** 函数名称: archCrtCtxEnableDsp
** 功能描述: 使能协程寄存器上下文的 DSP
** 输　入  : pregctx    寄存器上下文
** 输　出  : NONE
** 全局变量:
** 调用模块:
...
VOID  archCrtCtxEnableDsp (ARCH_REG_CTX  *pregctx)
{
    pregctx->REG_uiMsr |= ARCH_PPC_MSR_VEC;                             /*  使能 DSP                    */
}
