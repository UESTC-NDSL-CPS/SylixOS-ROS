...
#include "../../param/ppcParam.h"
#if LW_CFG_CPU_FPU_EN > 0
#include "../../fpu/ppcFpu.h"
#include "../../fpu/spe/ppcVfpSpe.h"
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
#if LW_CFG_VMM_EN > 0
#include "../../mm/mmu/e500/ppcMmuE500.h"
#include "../../mm/mmu/ppc460/ppcMmu460.h"
#endif                                                                  /*  LW_CFG_VMM_EN > 0           */
#include "../unaligned/ppcUnaligned.h"
/*********************************************************************************************************
  汇编函数声明
*********************************************************************************************************/
...
VOID  archE500FpDataExceptionHandle (addr_t  ulRetAddr)
{
     ...
     */
    uiFscr = ppcVfpSpeFscrGet();
    ppcVfpSpeFscrSet(uiFscr & (~ARCH_PPC_SPEFSCR_EXC_MASK));            /*  清除浮点异常                */

    if ((uiFscr & ARCH_PPC_SPEFSCR_FINVH) ||
        (uiFscr & ARCH_PPC_SPEFSCR_FINV)) {                             /*  非法操作异常                */
        abtInfo.VMABT_uiMethod = FPE_FLTINV;

    } else if ((uiFscr & ARCH_PPC_SPEFSCR_FDBZH) ||
               (uiFscr & ARCH_PPC_SPEFSCR_FDBZ)) {                      /*  除零异常                    */
        abtInfo.VMABT_uiMethod = FPE_FLTDIV;

    } else if ((uiFscr & ARCH_PPC_SPEFSCR_FOVFH) ||
               (uiFscr & ARCH_PPC_SPEFSCR_FOVF)) {                      /*  上溢异常                    */
        abtInfo.VMABT_uiMethod = FPE_FLTOVF;

    } else if ((uiFscr & ARCH_PPC_SPEFSCR_FUNFH) ||
               (uiFscr & ARCH_PPC_SPEFSCR_FUNF)) {                      /*  下溢异常                    */
        abtInfo.VMABT_uiMethod = FPE_FLTUND;

    } else if ((uiFscr & ARCH_PPC_SPEFSCR_OVH) ||
               (uiFscr & ARCH_PPC_SPEFSCR_OV)) {                        /*  整型溢出                    */
        abtInfo.VMABT_uiMethod = FPE_INTOVF;
    }
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
...
VOID  archE500FpRoundExceptionHandle (addr_t  ulRetAddr)
{
     ...
     */
    uiFscr = ppcVfpSpeFscrGet();
    ppcVfpSpeFscrSet(uiFscr & (~(ARCH_PPC_SPEFSCR_FINXS |
                                 ARCH_PPC_SPEFSCR_FGH |
                                 ARCH_PPC_SPEFSCR_FXH |
                                 ARCH_PPC_SPEFSCR_FG  |
                                 ARCH_PPC_SPEFSCR_FX)));                /*  清除浮点异常                */
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
