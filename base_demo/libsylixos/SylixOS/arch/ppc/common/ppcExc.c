...
#include "../fpu/vfp/ppcVfp.h"
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
#if LW_CFG_VMM_EN > 0
#include "../mm/mmu/hash/ppcMmuHash.h"
#endif
...
#if LW_CFG_CPU_FPU_EN > 0
    UINT32  uiSRR1 = ppcHashMmuGetSRR1();
    if (uiSRR1 & ARCH_PPC_SRR1_PROGFPE) {
        UINT32  uiFpscr = ppcVfpFpscrGet();

        ppcVfpFpscrSet(uiFpscr & (~ARCH_PPC_FPSCR_EXC_MASK));           /*  清除浮点异常                */

        abtInfo.VMABT_uiMethod = FPE_FLTINV;
        abtInfo.VMABT_uiType   = LW_VMM_ABORT_TYPE_FPE;                 /*  终止类型默认为 FPU 异常     */

        if (uiFpscr & ARCH_PPC_FPSCR_VX) {
            abtInfo.VMABT_uiMethod = FPE_FLTINV;                        /*  非法操作异常                */

        } else if (uiFpscr & ARCH_PPC_FPSCR_ZX) {
            abtInfo.VMABT_uiMethod = FPE_FLTDIV;                        /*  除零异常                    */

        } else if (uiFpscr & ARCH_PPC_FPSCR_OX) {
            abtInfo.VMABT_uiMethod = FPE_FLTOVF;                        /*  上溢异常                    */

        } else if (uiFpscr & ARCH_PPC_FPSCR_UX) {
            abtInfo.VMABT_uiMethod = FPE_FLTUND;                        /*  下溢异常                    */

        } else if (uiFpscr & ARCH_PPC_FPSCR_XX) {
...
VOID  archFpAssistExceptionHandle (addr_t  ulRetAddr)
{
    ...
    abtInfo.VMABT_uiMethod = FPE_FLTUND;                                /*  非规格数输入异常            */
    API_VmmAbortIsr(ulRetAddr, ulRetAddr, &abtInfo, ptcbCur);
}
/*********************************************************************************************************
