...
#include "../fpu/x86Fpu.h"
#include "../fpu/fpusse/x86FpuSse.h"
...
static CPCHAR   _G_pcX86ExceName[] = {                                  /*  异常名字表                  */
    ...
    [X86_EXCEPT_SPURIOUS]                    = "Spurious Interrupt",
    ...
    [X86_EXCEPT_SIMD_FP_ERROR]               = "SIMD FP Exception",
    [X86_EXCEPT_VIRTUALIZATION_ERROR]        = "Virtualization Exception",
    [X86_EXCEPT_CONTROL_PROTECTION]          = "Control protection Exception",
    ...
    [X86_EXCEPT_VMM_COMM_ERROR]              = "VMM Communication Exception",
...
/*********************************************************************************************************
  FPSR
*********************************************************************************************************/
#define FPSR_PE     0x0020                                              /*  Precision flag              */
#define FPSR_UE     0x0010                                              /*  Underflow flag              */
#define FPSR_OE     0x0008                                              /*  Overflow flag               */
#define FPSR_ZE     0x0004                                              /*  Divide-by-zero flag         */
#define FPSR_DE     0x0002                                              /*  Denormal flag               */
#define FPSR_IE     0x0001                                              /*  Invalid operation flag      */
/*********************************************************************************************************
  MXCSR
*********************************************************************************************************/
...
static VOID  x86FpErrorExceptHandle (ULONG  ulX86Vector, ARCH_REG_CTX  *pregctx)
{
    ...
    x86FpuSseFpsrGet(&usFpsr);
    x86FpuSseExceptionClear();

    if (usFpsr & FPSR_IE) {
        abtInfo.VMABT_uiMethod = FPE_FLTINV;                            /*  非法操作                    */

    } else if (usFpsr & FPSR_ZE) {
        abtInfo.VMABT_uiMethod = FPE_FLTDIV;                            /*  除零异常                    */

    } else if (usFpsr & FPSR_OE) {
        abtInfo.VMABT_uiMethod = FPE_FLTOVF;                            /*  上溢异常                    */

    } else if (usFpsr & (FPSR_DE | FPSR_UE)) {
        abtInfo.VMABT_uiMethod = FPE_FLTUND;                            /*  非规格数输入, 下溢异常      */

    } else if (usFpsr & FPSR_PE) {
...
/*********************************************************************************************************
** 函数名称: x86SimdFpErrorExceptHandle
** 功能描述: SIMD 浮点错误异常处理函数
** 输　入  : ulX86Vector   x86 异常向量
**           ulESP         异常保存上下文后的 ESP
** 输　出  : NONE
** 全局变量:
** 调用模块:
*********************************************************************************************************/
static VOID  x86SimdFpErrorExceptHandle (ULONG  ulX86Vector, ARCH_REG_CTX  *pregctx)
{
    PLW_CLASS_TCB   ptcbCur;
    LW_VMM_ABORT    abtInfo;
    addr_t          ulRetAddr;
    ...
    x86FpuSseMxcsrGet(&uiMxcsr);

    if (uiMxcsr & MXCSR_IE) {
        abtInfo.VMABT_uiMethod = FPE_FLTINV;                            /*  非法操作                    */

    } else if (uiMxcsr & MXCSR_ZE) {
        abtInfo.VMABT_uiMethod = FPE_FLTDIV;                            /*  除零异常                    */

    } else if (uiMxcsr & MXCSR_OE) {
        abtInfo.VMABT_uiMethod = FPE_FLTOVF;                            /*  上溢异常                    */

    } else if (uiMxcsr & (MXCSR_UE | MXCSR_DE)) {
        abtInfo.VMABT_uiMethod = FPE_FLTUND;                            /*  非规格数输入, 下溢异常      */

    } else if (uiMxcsr & MXCSR_PE) {
        abtInfo.VMABT_uiMethod = FPE_FLTRES;                            /*  不精确异常                  */
    ...
    uiMxcsr &= ~(MXCSR_PE | MXCSR_UE | MXCSR_OE | MXCSR_ZE | MXCSR_DE | MXCSR_IE);
    x86FpuSseMxcsrSet(&uiMxcsr);
...
** 调用模块:
*********************************************************************************************************/
static VOID  x86DivideExceptHandle (ULONG  ulX86Vector, ARCH_REG_CTX  *pregctx)
{
    PLW_CLASS_TCB   ptcbCur;
    LW_VMM_ABORT    abtInfo;
    addr_t          ulRetAddr;
    ...
    abtInfo.VMABT_uiType   = LW_VMM_ABORT_TYPE_FPE;
    abtInfo.VMABT_uiMethod = FPE_INTDIV;                                /*  整型除零异常               */
    API_VmmAbortIsr(ulRetAddr, ulRetAddr, &abtInfo, ptcbCur);
}
/*********************************************************************************************************
** 函数名称: x86IrqHandle
...
VOID  x86ExceptIrqInit (VOID)
{
    ...
    _G_pfuncX86IntHandleArray[X86_EXCEPT_DIVIDE_ERROR]   = x86DivideExceptHandle;
    ...
    _G_pfuncX86IntHandleArray[X86_EXCEPT_SIMD_FP_ERROR]        = x86SimdFpErrorExceptHandle;
#endif                                                                  /*  LW_CFG_CPU_FPU_EN > 0       */
