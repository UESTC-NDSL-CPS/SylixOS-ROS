...
/*********************************************************************************************************
** 函数名称: x86CpuIdHetrccProbe
** 功能描述: 当前 CPU 算力簇 ID 探测
** 输　入  : NONE
** 输　出  : 当前 CPU 算力簇 ID
** 全局变量:
** 调用模块:
...
ULONG  x86CpuIdHetrccProbe (VOID)
{
    UINT32  uiExtFeatures[4];
    UINT32  uiNativeModelId;

    /*
     * Check for hybrid architecture
     * From Intel 64 and IA-32 Architectures Software Developer Manual Combined Volumes
     * Available at https://cdrdv2.intel.com/v1/dl/getContent/671200
     *
     * - CPUID[7h] is Structured Extended Feature Flags Enumeration Leaf (EAX = 7H, ECX = 0)
    ...
    uiExtFeatures[3] = 0;
    x86CpuIdProbeExtFeature(uiExtFeatures);
    if (uiExtFeatures[3] & (1 << 15)) {                                 /*  Hybrid                      */
        uiNativeModelId = x86CpuIdProbeNativeModelId();
        switch (uiNativeModelId >> 24) {                                /*  Core type                   */
