...
static ULONG      _G_ulCpuHetrccTable[LW_CFG_MAX_PROCESSORS];           /*  CPU 算力簇 ID 表            */
...
{
#if LW_CFG_CPU_ARCH_HETRC > 0
    ULONG          ulCPUId = LW_CPU_GET_CUR_ID();                       /*  获得当前 CPU ID             */
    LW_CLASS_CPU  *pcpu    = LW_CPU_GET(ulCPUId);                       /*  获得当前 CPU                */

    _G_ulCpuHetrccTable[ulCPUId] = x86CpuIdHetrccProbe();               /*  当前 CPU 算力簇 ID 探测     */

    pcpu->CPU_ulHccId = _G_ulCpuHetrccTable[ulCPUId];                   /*  记录当前 CPU 算力簇 ID      */

    if (LW_NHETRCCS < (pcpu->CPU_ulHccId + 1)) {
        LW_NHETRCCS =  pcpu->CPU_ulHccId + 1;                           /*  重新统计算力簇个数          */
    }
#endif                                                                  /*  LW_CFG_CPU_ARCH_HETRC > 0   */
    ...
    if (X86_HAS_SMT) {                                                  /*  存在超线程                  */
        LW_CPU_FOREACH_EXCEPT(ulCPUId, 0) {
            ...
            if (X86_APICID_PRESEND(ucLocalApicId)) {
                if (X86_LOGICID_IS_HT(ulCPUId)) {                       /*  先启动所有物理核            */
                    continue;
                }
                API_CpuUp(ulCPUId);
            }
        ...
        LW_CPU_FOREACH_EXCEPT(ulCPUId, 0) {
            ucLocalApicId = X86_LOGICID_TO_APICID(ulCPUId);
            if (X86_APICID_PRESEND(ucLocalApicId)) {
                if (X86_LOGICID_IS_HT(ulCPUId)) {                       /*  再启动所有逻辑核            */
                    API_CpuUp(ulCPUId);
                }
            }
        }
...
{
    if (ulCPUId == 0) {
        if (LW_NCPUS > 1) {                                             /*  保证算力簇 ID 不超过 NCPUS  */
            _G_ulCpuHetrccTable[0] = x86CpuIdHetrccProbe();             /*  初始化 CPU 0 算力簇 ID      */
        }
    }

    return  (_G_ulCpuHetrccTable[ulCPUId]);
}
