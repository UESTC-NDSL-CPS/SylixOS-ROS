...
#define FPXREG_MXCSR        0x18                                    /*  Offset to MXCSR in FPREG_SET    */
#define FPXREG_MXCSR_MASK   0x1c                                    /*  Offset to MXCSR_MASK            */
#define FPXREG_FPX(n)       (0x20 + (n) * sizeof(X86_FPU_DOUBLEX_SSE))  /*  Offset to FPX(n)            */
/*********************************************************************************************************
  FPCR
*********************************************************************************************************/
#define FPCR_PM             0x0020                                  /*  Precision exception enable      */
#define FPCR_UM             0x0010                                  /*  Underflow exception enable      */
#define FPCR_OM             0x0008                                  /*  Overflow exception enable       */
#define FPCR_ZM             0x0004                                  /*  0-divide exception enable       */
#define FPCR_DM             0x0002                                  /*  Denormalized operand enable     */
#define FPCR_IM             0x0001                                  /*  Invalid operation enable        */
/*********************************************************************************************************
  MXCSR
*********************************************************************************************************/
#define MXCSR_PM            0x1000                                  /*  Precision exception enable      */
#define MXCSR_UM            0x0800                                  /*  Underflow exception enable      */
#define MXCSR_OM            0x0400                                  /*  Overflow exception enable       */
#define MXCSR_ZM            0x0200                                  /*  0-divide exception enable       */
#define MXCSR_DM            0x0100                                  /*  Denormalized operand enable     */
#define MXCSR_IM            0x0080                                  /*  Invalid operation enable        */
/*********************************************************************************************************
  类型定义
*********************************************************************************************************/
typedef struct {
...
static FPU_REG_INFO _G_fpXCtrlRegName [] = {
    ...
    { "ds",          FPXREG_DS         },
    { "mxcsr",       FPXREG_MXCSR      },
    { "mxcsr_mask",  FPXREG_MXCSR_MASK },
    { LW_NULL,       0                 },
};

static CPCHAR       _G_pcFpuTaskRegsCFmt = "\n%-10.10s = %08x";
#if LW_KERN_FLOATING > 0
...
** 调用模块:
*********************************************************************************************************/
static VOID  x86FpuSseEnableException (PVOID  pvFpuCtx)
{
    LW_FPU_CONTEXT  *pfpuCtx    = (LW_FPU_CONTEXT *)pvFpuCtx;
    ARCH_FPU_CTX    *pcpufpuCtx = &pfpuCtx->FPUCTX_fpuctxContext;

    if ((X86_FEATURE_HAS_XSAVE && X86_FEATURE_HAS_AVX) || X86_FEATURE_HAS_FXSR) {
        pcpufpuCtx->FPUCTX_XCtx.XCTX_sFpcr &= ~(FPCR_UM | FPCR_OM | FPCR_ZM |
                                                FPCR_DM | FPCR_IM);

        pcpufpuCtx->FPUCTX_XCtx.XCTX_iMxcsr &= ~(MXCSR_UM | MXCSR_OM | MXCSR_ZM |
                                                 MXCSR_DM | MXCSR_IM);

        pcpufpuCtx->FPUCTX_XCtx.XCTX_iMxcsrMask |= (MXCSR_PM | MXCSR_UM | MXCSR_OM | MXCSR_ZM |
                                                    MXCSR_DM | MXCSR_IM);

    } else {
        pcpufpuCtx->FPUCTX_oldCtx.OCTX_iFpcr &= ~(FPCR_UM | FPCR_OM | FPCR_ZM |
                                                  FPCR_DM | FPCR_IM);
    }
}
/*********************************************************************************************************
...
PX86_FPU_OP  x86FpuSsePrimaryInit (CPCHAR  pcMachineName, CPCHAR  pcFpuName)
{
    ...
    _G_fpuopFpuSse.PFPU_pfuncEnableException = x86FpuSseEnableException;
    ...
    if (X86_FEATURE_HAS_XSAVE && X86_FEATURE_HAS_AVX) {
        ...
        x86Cr4Set(x86Cr4Get()     |
                  X86_CR4_OSXSAVE |
                  X86_CR4_OSFXSR  |
                  X86_CR4_OSXMMEXCEPT);                                 /*  设置OSFXSR位, 准备 SSE 环境 */
        ...
        x86Cr4Set(x86Cr4Get() | X86_CR4_OSFXSR | X86_CR4_OSXMMEXCEPT);  /*  设置OSFXSR位, 准备 SSE 环境 */
