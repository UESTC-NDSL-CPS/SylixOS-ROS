...
({ \
    REGISTER ULONG             cpuid; \
    REGISTER PLW_CLASS_TCB     tcb; \
    __asm__ __volatile__(" cli \n" \
                         " movq %%gs:%P1, %q0 \n" \
                         : "=r" (cpuid) \
                         : "i" (0)); \
    tcb = LW_CPU_GET(cpuid)->CPU_ptcbTCBCur; \
...
/*********************************************************************************************************
  x86-64 处理器当前线程上下文 TLS 加载
*********************************************************************************************************/

#if LW_CFG_CPU_WORD_LENGHT == 64
#define __ARCH_CTX_TLS_LOAD(ptcb)   archTaskCtxTlsLoad(ptcb)

VOID    archTaskCtxTlsLoad(PLW_CLASS_TCB  ptcb);
...
VOID    bspIntVectorEoi(ULONG  ulVector);
...
#define __ARCH_INT_VECTOR_EOI       bspIntVectorEoi
