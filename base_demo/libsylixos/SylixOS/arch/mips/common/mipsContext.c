                              ...
                              ULONG                  ulOpt)
{
    ...
    if (ulOpt & LW_OPTION_THREAD_USED_FP) {
        ...
        ulCP0Status |= ST0_FR;                                          /*  32 个浮点寄存器             */
#else
        ulCP0Status &= ~ST0_FR;                                         /*  16 个浮点寄存器             */
...
{
#if LW_CFG_CPU_WORD_LENGHT == 64
    pregctx->REG_ulCP0Status |= ST0_CU1 |                               /*  使能 CU1(FPU)               */
                                ST0_FR;                                 /*  32 个浮点寄存器             */
#else
    pregctx->REG_ulCP0Status |= ST0_CU1;                                /*  使能 CU1(FPU)               */
...
VOID  archCrtCtxEnableDsp (ARCH_REG_CTX  *pregctx)
{
    ...
    pregctx->REG_ulCP0Status |= ST0_CU2;                                /*  使能向量运算单元            */
#else
    pregctx->REG_ulCP0Status |= ST0_MX;                                 /*  使能 MDMX ASE 或 DSP        */
