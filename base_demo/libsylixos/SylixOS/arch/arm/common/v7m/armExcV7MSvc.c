...
ARCH_REG_CTX  *armv7mSvcHandle (ARCH_HW_SAVE_REG_CTX  *pHwSaveCtx, ARCH_SW_SAVE_REG_CTX  *pSwSaveCtx)
{
    ...
    case SVC_archSigCtxSave: {
        ARCH_REG_CTX  *pregctxDest = (ARCH_REG_CTX *)pHwSaveCtx->REG_uiR2;

        archTaskCtxCopy(pregctxDest, pSwSaveCtx, pHwSaveCtx);
        pregctxDest->REG_uiR0 = (ARCH_REG_T)pregctxDest;                /*  返回非 0                    */

        pcpuCur     = LW_CPU_GET_CUR();
        pregctxDest = &pcpuCur->CPU_ptcbTCBCur->TCB_archRegCtx;
        archTaskCtxCopy(pregctxDest, pSwSaveCtx, pHwSaveCtx);
        return  (pregctxDest);
    }
