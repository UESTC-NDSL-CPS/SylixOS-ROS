   ...
   struct winsize *winp)
{
    ...
    CHAR  cPtyName[PATH_MAX];

    snprintf(cPtyName, PATH_MAX, "/dev/pty/ssh_%d", getpid());

    if (ptyDevCreate(cPtyName, LW_CFG_NET_TELNET_RBUFSIZE, LW_CFG_NET_TELNET_WBUFSIZE) < 0) {
        error("pty ptyDevCreate failed : %s.", strerror(errno));
    }
    strlcpy(name, cPtyName, PATH_MAX);
    {
        CHAR cPtyHostName[PATH_MAX];

        snprintf(cPtyHostName, PATH_MAX, "%s.hst", cPtyName);
        *aslave = open(cPtyHostName, O_RDWR);
        if (*aslave < 0) {
            error("pty host can not open: %s.", strerror(errno));
            return -1;
        }
    }
	
    {
        CHAR cPtyDevName[PATH_MAX];

        snprintf(cPtyDevName, PATH_MAX, "%s.dev", cPtyName);
        *amaster = open(cPtyDevName, O_RDWR);
        if (*amaster < 0) {
            close(*aslave);
            error("pty dev can not open: %s.", strerror(errno));
            return -1;
        }
    }
	
    ioctl(*aslave, FIOSETOPTIONS, OPT_TERMINAL);
    return 0;
#else
	/* BSD-style pty code. */
