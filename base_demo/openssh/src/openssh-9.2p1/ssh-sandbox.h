...
#ifdef SYLIXOS
static inline struct ssh_sandbox *ssh_sandbox_init(struct monitor *m)
{
    return NULL;
};
void ssh_sandbox_child(struct ssh_sandbox *s) { return; };
void ssh_sandbox_parent_finish(struct ssh_sandbox *s) { return; };
void ssh_sandbox_parent_preauth(struct ssh_sandbox *s, pid_t i) { return; };
#else
