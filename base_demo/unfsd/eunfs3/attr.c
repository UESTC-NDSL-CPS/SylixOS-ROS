...
post_op_attr get_post_buf(backend_statstruct buf, struct svc_req * req)
{
    ...
    result.post_op_attr_u.attributes.fsid = (uint32)buf.st_dev;
	...
	if (backend_lstat(export_path, &epbuf) != -1 &&
	    (uint32)buf.st_dev == (uint32)epbuf.st_dev) {
				...
				struct svc_req *req)
{
    ...
    if (dev != (uint32)buf.st_dev || ino != buf.st_ino)
	return error_attr;
...
{
    return get_post_ll(path, (uint32)st_cache.st_dev, st_cache.st_ino, req);
}
...
static nfsstat3 set_attr_unsafe(const char *path, nfs_fh3 nfh, sattr3 new)
{
    ...
    /* check local fs race */
    if ((uint32)buf.st_dev != fh->dev || buf.st_ino != fh->ino)
	return NFS3ERR_STALE;
...
nfsstat3 set_attr(const char *path, nfs_fh3 nfh, sattr3 new)
{
    ...
    /* check local fs race */
    if (fh->dev != (uint32)buf.st_dev || fh->ino != buf.st_ino ||
...
#else /* windows nfs client will set to client time, avoid return invalid error */
    if ((attr.uid.set_it == TRUE && used_uid != backend_geteuid()) ||
    (attr.gid.set_it == TRUE && used_gid != backend_getegid()) ||
    (attr.size.set_it == TRUE && attr.size.set_size3_u.size != 0))
#endif
