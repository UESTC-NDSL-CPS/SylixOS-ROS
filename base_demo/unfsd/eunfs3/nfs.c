...
LOOKUP3res *nfsproc3_lookup_3_svc(LOOKUP3args * argp, struct svc_req * rqstp)
{
	...
	if (res == -1)
		...
		fh = fh_extend(argp->what.dir, (uint32)buf.st_dev, buf.st_ino, gen);
		fh_cache_add((uint32)buf.st_dev, buf.st_ino, obj);
	    }
...
#define NFS_IN_READ   (0x55555555)
#define NFS_IN_WRITE  (0xaaaaaaaa)

void  change_socket_nagle(struct svc_req *rqstp)
{
    static uint32 *xflags = NULL;
    int pad = 0;

    if (xflags == NULL) {
        xflags = (uint32 *)calloc(_rpc_dtablesize(), sizeof(uint32));
        if (xflags == NULL) {
            return;
        }
    }

    if (rqstp->rq_proc == NFSPROC3_READ) {
        if (xflags[rqstp->rq_xprt->xp_sock] != NFS_IN_READ) {
            pad = 1;
            __setsockopt(rqstp->rq_xprt->xp_sock, IPPROTO_TCP, TCP_NODELAY, (void *)&pad, sizeof(pad));
            xflags[rqstp->rq_xprt->xp_sock] = NFS_IN_READ;
        }

    } else if (rqstp->rq_proc == NFSPROC3_WRITE) {
        if (xflags[rqstp->rq_xprt->xp_sock] != NFS_IN_WRITE) {
            if (get_socket_type(rqstp) == SOCK_STREAM) {
                pad = 0;
                __setsockopt(rqstp->rq_xprt->xp_sock, IPPROTO_TCP, TCP_NODELAY, (void *)&pad, sizeof(pad));
            }
            xflags[rqstp->rq_xprt->xp_sock] = NFS_IN_WRITE;
        }
    }
}
...
CREATE3res *nfsproc3_create_3_svc(CREATE3args * argp, struct svc_req * rqstp)
{
	...
	if (res != -1) {
		...
		fh_extend_post(argp->where.dir, (uint32)buf.st_dev, buf.st_ino, gen);
	...
	if (argp->how.mode == EXCLUSIVE && errno == EEXIST) {
		...
		if (backend_check_create_verifier
		    ...
		    result.CREATE3res_u.resok.obj =
			fh_extend_post(argp->where.dir, (uint32)buf.st_dev,
				       buf.st_ino, gen);
		    result.CREATE3res_u.resok.obj_attributes =
			get_post_buf(buf, rqstp);
		} else {
