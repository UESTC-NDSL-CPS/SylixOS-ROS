...
namespace rclcpp
{
  ...
  protected:
    ...
    RCLCPP_PUBLIC
    bool
    PiCAS_get_next_ready_executable(AnyExecutable &any_executable);

    RCLCPP_PUBLIC
    bool
    PiCAS_get_next_ready_executable_from_map(
        AnyExecutable &any_executable,
        const WeakCallbackGroupsToNodesMap &weak_groups_to_nodes);

    RCLCPP_PUBLIC
    bool
    PiCAS_get_next_executable(
        AnyExecutable &any_executable,
        std::chrono::nanoseconds timeout = std::chrono::nanoseconds(-1));
    

    std::map<rclcpp::SubscriptionBase::SharedPtr, int> subscription_priority_map_;
    std::map<rclcpp::ServiceBase::SharedPtr, int> service_priority_map_;
    std::map<rclcpp::ClientBase::SharedPtr, int> client_priority_map_;
    std::map<rclcpp::TimerBase::SharedPtr, int> timer_priority_map_;
    std::map<rclcpp::Waitable::SharedPtr, int> waitable_priority_map_;
