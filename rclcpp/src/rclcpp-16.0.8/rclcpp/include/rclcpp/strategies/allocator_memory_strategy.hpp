  ...
  namespace memory_strategies
  {
    namespace allocator_memory_strategy
    {
      ...
      public:
        ...
        void
        PiCAS_get_next_service(
            rclcpp::AnyExecutable & any_exec,
            std::map<rclcpp::ServiceBase::SharedPtr, int> service_priority_map,
            const WeakCallbackGroupsToNodesMap & weak_groups_to_nodes) override
        {
          ...
          while (it != service_handles_.end())
          {
            ...
            if (service)
            {
              ...
              // Otherwise it is safe to set and return the any_exec
              if(service_priority_map.count(service)) {
                priority = service_priority_map[service];
              }
              if(priority > highest_priority || idx == 1) {
        ...
        void
        PiCAS_get_next_client(
            rclcpp::AnyExecutable &any_exec,
            std::map<rclcpp::ClientBase::SharedPtr, int> client_priority_map_,
            const WeakCallbackGroupsToNodesMap &weak_groups_to_nodes) override
        {
          int highest_priority = -1;
          int idx = 0;
          ...
          while (it != client_handles_.end())
          {
            ...
            if (client)
            {
              ...
              // Otherwise it is safe to set and return the any_exec
              if(client_priority_map_.count(client)) {
                priority = client_priority_map_[client];
              }
              if (priority > highest_priority || idx==1) {
                highest_priority = priority;
                any_exec.client = client;
        ...
        void
        PiCAS_get_next_subscription(
            rclcpp::AnyExecutable &any_exec,
            std::map<rclcpp::SubscriptionBase::SharedPtr, int> subscription_priority_map,
            const WeakCallbackGroupsToNodesMap &weak_groups_to_nodes) override
        {
          int highest_priority = -1;
          int idx = 0;
          ...
          auto res = it;
          int ss = subscription_handles_.size();
          while (it != subscription_handles_.end())
          {
            idx++;
            if(idx > ss)  break;
            ...
            if (subscription)
            {
              ...
              // Otherwise it is safe to set and return the any_exec
              if(subscription_priority_map.count(subscription)) {
                priority = subscription_priority_map[subscription];
              }
              if (priority > highest_priority || idx==1) {
                std::cout<<"---------get the subscription---------"<<std::endl;
                highest_priority = priority;
                any_exec.subscription = subscription;
                any_exec.callback_group = group;
                any_exec.node_base = get_node_by_group(group, weak_groups_to_nodes);
            ...
            if(res != subscription_handles_.end()) {
              subscription_handles_.erase(it);
            }
          }
        }
        ...
        void
        PiCAS_get_next_timer(
            AnyExecutable &any_exec,
            std::map<rclcpp::TimerBase::SharedPtr, int> timer_priority_map,
            const WeakCallbackGroupsToNodesMap &weak_groups_to_nodes) override
        {
          bool success = false;
          int highest_priority = -1;
          ...
          while (it != timer_handles_.end())
          {
            ...
            if (timer)
            {
              ...
              // Otherwise it is safe to set the any_exec and compare the priority
              if (timer_priority_map.count(timer))
              {
                priority = timer_priority_map[timer];
              }             
              if (priority > highest_priority || idx==1)
              {
                success = true;
                std::cout << "--------------get the timer---------------" << std::endl;
                highest_priority = priority;
                any_exec.timer = timer;
                any_exec.callback_group = group;
                any_exec.node_base = get_node_by_group(group, weak_groups_to_nodes);
          ...
          if(res != timer_handles_.end()) {
            timer_handles_.erase(res);
          }
        }

        void
        PiCAS_get_next_waitable(
            rclcpp::AnyExecutable &any_exec,
            std::map<rclcpp::Waitable::SharedPtr, int> waitable_priority_map,
            const WeakCallbackGroupsToNodesMap &weak_groups_to_nodes) override
        {
          int highest_priority = -1;
          int idx = 0;
          ...
          while (it != waitable_handles_.end())
          {
            ...
            if (waitable)
            {
              ...
              // Otherwise it is safe to set and return the any_exec
              if (waitable_priority_map.count(waitable))
              {
                priority = waitable_priority_map[waitable];
              }             
              if (priority > highest_priority || idx==1) {
                highest_priority = priority;
            ...
            if(res != waitable_handles_.end()) {
              waitable_handles_.erase(res);
            }
          }
        }
