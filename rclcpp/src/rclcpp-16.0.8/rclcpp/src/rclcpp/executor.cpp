...
bool
Executor::PiCAS_get_next_ready_executable(AnyExecutable & any_executable)
{
  // std::cout << "-----------start PiCAS_get_next_ready_executable()-----------------" << std::endl;
  bool success = PiCAS_get_next_ready_executable_from_map(any_executable, weak_groups_to_nodes_);
  return success;
}

bool
Executor::PiCAS_get_next_ready_executable_from_map(
  AnyExecutable & any_executable,
  const rclcpp::memory_strategy::MemoryStrategy::WeakCallbackGroupsToNodesMap &
  weak_groups_to_nodes)
{
  // TRACEPOINT(rclcpp_executor_get_next_ready);
  // std::cout << "-----------start PiCAS_get_next_ready_executable_from_map()-----------------" << std::endl;
  bool success = false;
  int max_priority = -1;
  std::lock_guard<std::mutex> guard{mutex_};
  // Check all the timers and if there are some ready then select the one having the highest priority
  // possible bug: timer's period < current operation time.Maybe searching is not over, next expiration of some timer has come
  AnyExecutable timer_exec;
  memory_strategy_->PiCAS_get_next_timer(timer_exec, this->timer_priority_map_, weak_groups_to_nodes);
  if (timer_exec.timer) { 
    int priority = 0;
    if(this->timer_priority_map_.count(timer_exec.timer)) {
      priority = this->timer_priority_map_[timer_exec.timer];
    }
    std::cout<<"----------get timer,priority:"<<priority<<"-------------"<<std::endl;
    // use >= because it needs to set a ready callback first
    if(priority >= max_priority) {
      std::cout<<"+++++++++++set timer,priority:"<<priority<<"++++++++++++++"<<std::endl;
      success = true;
      any_executable = timer_exec; 
  ...
  // Check the subscriptions to see if there are any that are ready
  AnyExecutable subscription_exec;
  memory_strategy_->PiCAS_get_next_subscription(subscription_exec, this->subscription_priority_map_, weak_groups_to_nodes);
  if (subscription_exec.subscription) {    
    int priority = 0;
    if(this->subscription_priority_map_.count(subscription_exec.subscription)) {
      priority = subscription_priority_map_[subscription_exec.subscription];
    }
    std::cout<<"----------get subscription,priority:"<<priority<<"-------------"<<std::endl;
    // use > because a subscription can be executed before timers' execution only when subscription's priority was set higher statically
    // callbacks below follow the reason above
    if(priority > max_priority) {
      std::cout<<"+++++++++++set subscription,priority:"<<priority<<"++++++++++++++"<<std::endl;
      success = true;
      any_executable = subscription_exec;
      max_priority = priority;
    }
  }

  // Check the services to see if there are any that are ready
  AnyExecutable service_exec;
  memory_strategy_->PiCAS_get_next_service(service_exec, this->service_priority_map_, weak_groups_to_nodes);
  if (service_exec.service) {
    int priority = 0;
    if(this->service_priority_map_.count(service_exec.service)) {
      priority = this->service_priority_map_[service_exec.service];
    }
    std::cout<<"----------get service,priority:"<<priority<<"-------------"<<std::endl;
    if(priority > max_priority) {
      std::cout<<"+++++++++++set service,priority:"<<priority<<"++++++++++++++"<<std::endl;
      success = true;
      any_executable = service_exec;
      max_priority = priority;
    }
  }
  ...
  // Check the clients to see if there are any that are ready
  AnyExecutable client_exec;
  memory_strategy_->PiCAS_get_next_client(client_exec, this->client_priority_map_, weak_groups_to_nodes);
  if (client_exec.client) {  
    int priority = 0;
    if(this->client_priority_map_.count(client_exec.client)) {
      priority = this->client_priority_map_[client_exec.client];
    }
    std::cout<<"----------get client,priority:"<<priority<<"-------------"<<std::endl;
    if(priority > max_priority) {
      std::cout<<"+++++++++++set client,priority:"<<priority<<"++++++++++++++"<<std::endl;
      success = true;
      any_executable = client_exec;
      max_priority = priority;
    }
  }
  ...
  // Check the waitables to see if there are any that are ready
  AnyExecutable waitable_exec;
  memory_strategy_->PiCAS_get_next_waitable(waitable_exec, this->waitable_priority_map_, weak_groups_to_nodes);
  if (waitable_exec.waitable) {
    int priority = 0;
    if(this->waitable_priority_map_.count(waitable_exec.waitable)) {
      priority = this->waitable_priority_map_[waitable_exec.waitable];
    }
    std::cout<<"----------get waitable,priority:"<<priority<<"-------------"<<std::endl;
    if(priority > max_priority) {
      std::cout<<"+++++++++++set waitable,priority:"<<priority<<"++++++++++++++"<<std::endl;
      success = true;
      any_executable = waitable_exec;
      any_executable.data = any_executable.waitable->take_data();
      max_priority = priority;
    }
  }
...
bool
Executor::PiCAS_get_next_executable(AnyExecutable & any_executable, std::chrono::nanoseconds timeout)
{
  // std::cout << "----------start PiCAS_get_next_executable()" << std::endl;
  bool success = false;
  // Check to see if there are any subscriptions or timers needing service
  success = PiCAS_get_next_ready_executable(any_executable);
  // If there are none
  if (!success) {
    // Wait for subscriptions or timers to work on
    wait_for_work(timeout);
